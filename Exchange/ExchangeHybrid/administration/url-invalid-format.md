...
This article describes an issue in hybrid Exchange Online environments where the Microsoft Remote Connectivity Analyzer returns the error "ActiveSync URL was in an Invalid format" during an Exchange ActiveSync test. The issue occurs when the `ExternalURL` value of the `Microsoft-Server-ActiveSync` virtual directory on the on-premises Exchange server doesn't use the required URL format. To resolve the issue, verify and correct the `ExternalURL` setting so that it points to the appropriate external hostname and ActiveSync path.
...
1. Expand **Server Configuration**, select **Client Access**, select the appropriate Client Access server, and then select the **Exchange Active Sync** tab.
1. Select the Microsoft-Server-ActiveSync virtual directory, and then select **Properties**.
1. Make sure that the **ExternalURL** property uses the following format:
