...
This behavior is application to other similar scenarios as well. For example, if email is sent from OnPremises to a distribution group which is hosted in Exchange Online and the distribution group has an external recipient in it, the email to that external recipient will be routed directly instead of following centralized mail routing because the message originated in the on-premises environment.
