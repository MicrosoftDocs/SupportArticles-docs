...
|---|---|---|---|
|Migration Exchange Web Services (EWS) throttling policy|Verify that the EWS throttling policy isn't too restrictive for a mailbox data migration that uses third-party tools. (Does not apply to Microsoft tools for Hybrid, IMAP, G Suite, or Public Folder migrations.)|Diag: EWS Throttling||
