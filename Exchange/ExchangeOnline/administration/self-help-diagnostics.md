...
|Test user's Exchange Online RBAC permissions|If a user has issues running specific PowerShell cmdlets or commands (or is getting an error indicating they do not have correct roles or permissions) - check if the user has permissions to run the cmdlet and a specific parameter.|[Run Tests: EXO RBAC test user](https://aka.ms/PillarEXORBACTest)||
|Compare Exchange Online RBAC permissions of two users|If one user has issues running specific Exchange Online PowerShell cmdlets or commands (or is getting an error indicating they do not have correct roles or permissions) - and another does not have those problems, compare RBAC roles of those two users.|[Run Tests: EXO RBAC compare users](https://aka.ms/PillarEXORBACCompare)||
