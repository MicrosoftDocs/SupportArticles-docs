...
This issue may also occur if the conference room is hidden from the Global Address List (GAL).
...
To verify that a room is visible in the GAL, use this command and verify that ```HiddenFromAddressListsEnabled``` is set to false:
