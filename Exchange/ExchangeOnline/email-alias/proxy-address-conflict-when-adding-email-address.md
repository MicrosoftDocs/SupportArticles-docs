...
The first error might occur if a mail-enabled object in Exchange Online uses the proxy address that you want to assign.

### Cause 2

The second error might occur if an MEPF object that exists only in Azure Active Directory (Azure AD) uses the proxy address that you want to assign.

## Resolution

Use the appropriate resolution, depending on the error message that you receive. Then, try again to assign the proxy address.
...
2. For each on-premises MEPF that's identified in step 1, remove the conflicting SMTP address by running the following command in the on-premises EMS:
   ...
   This step removes any proxy address conflicts on the on-premises side to make sure that they aren't synced to Azure AD or Exchange Online.

3. If you removed a conflicting SMTP address from any on-premises MEPF in step 2, wait until the next scheduled sync runs on the Azure AD Connect server, or manually start a [sync cycle](/azure/active-directory/hybrid/how-to-connect-sync-feature-scheduler#full-sync-cycle) by running the following PowerShell command:
...
6. Rerun the search from step 4 in Azure AD. This time, the search results shouldn't return any MEPFs.
