...
|5.7.236|`Your message can't be sent because your tenant has exceeded its daily limit for sending email to external recipients from your tenant's onmicrosoft.com domains`|Your organization has exceeded the limit of sending emails to 100 external recipients from your organization’s onmicrosoft.com (MOERA) domain within a 24‑hour rolling window. This limit is enforced per tenant.|MOERA (onmicrosoft.com) domains are intended only for testing, not for regular email sending. Messages sent from these domains are limited to 100 external recipients per rolling 24-hour period. Inbound emails are not affected. External recipients are counted after any recipient list expansion. To avoid this limitation, purchase and migrate to a custom domain and ensure that only custom domains are used for sending emails to external recipients.|
