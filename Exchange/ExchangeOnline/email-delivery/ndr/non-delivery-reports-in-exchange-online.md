---
title: Email non-delivery reports (NDRs) and SMTP errors in Exchange Online
ms.date: 03/25/2026
author: cloud-writer
ms.author: meerak
...
description: Administrators can learn about SMTP errors and non-delivery reports (also known as NDRs or bounce messages) that are generated in Exchange Online.
---
...
|4.7.850-899|`Access denied, please try again later`|Suspicious activity was detected on the IP in question, and it's temporarily restricted while being further evaluated.|If this activity is valid, this restriction islifted shortly.|
...
|5.4.300|`Message expired`|The email message took too long to be successfully delivered, either because the destination server never responded or the sent message generated an error but an NDR couldn't be delivered to the original sender.|
...
|5.7.507|`Access denied, rejected by recipient`|The recipient's organization blocked the IP from which you're trying to send.|Contact the recipient to resolve this issue.|
