...
This issue occurs because the **MonitoringMailboxCleaner** process incorrectly deletes the monitoring mailboxes. This process runs every hour on every server that's running Exchange Server. It removes monitoring mailboxes that the server doesn't use. To build the list of mailboxes to check, the process examines the DisplayName value of the accounts by using a pattern match for the **HealthMailbox-SERVERNAME-** string. Then, it evaluates each item on the list to determine whether that item should be deleted.
...
1. Repeat steps 1–6 for each **HealthMailbox\<GUID>** object in the **Monitoring Mailboxes** container.
