...
This article describes an issue in which Active Directory site redirection for Exchange Control Panel (ECP) fails in Exchange Server 2016. Users who access ECP from a remote site receive an "Error 400: The length of the query string for this request exceeds the configured maxQueryStringLength value" message. The article explains that the issue is caused by an incorrect redirection response generated by ECP and provides a workaround that uses proxying instead of redirection by removing the `ExternalURL` setting from the ECP virtual directory.
