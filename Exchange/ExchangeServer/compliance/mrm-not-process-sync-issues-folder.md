---
title: MRM policy doesn't process or delete items in the Sync Issues folder in Outlook
description: Describes an issue in which a Messaging Records Management (MRM) policy doesn't process or delete items in the Sync Issues folder in Microsoft Outlook.
author: cloud-writer
ms.author: meerak
...
ms.custom: 
  ...
  - CI 9823
  - CI 12269
ms.reviewer: benwinz, v-kegunn, v-six, v-kccross
appliesto: 
  - Exchange Server SE
...
Messaging Records Management (MRM) policies don’t process or delete items in the Sync Issues folder because the folder exists only on the Outlook client and isn’t managed by Exchange server-side processing. Although items might display an expiration date based on retention settings, the Managed Folder Assistant doesn’t act on them. To manage or remove items in this folder, use Outlook client features such as AutoArchive instead of relying on MRM.

## Symptoms

When you use Microsoft Office Outlook in Cached Exchange mode, messages are created in the Sync Issues folder. Exchange Administrators can create a retention policy tag (RPT) for the Sync Issues folder and can then add that RPT to an existing retention policy.

When you apply a Messaging Records Management (MRM) policy to the mailbox, the items in the Sync Issues folder might have an expiration date stamped on the items. However, MRM doesn't process or delete the items.
...
To work around this issue, don't implement an RPT for the Sync Issues folder because no action is taken on the contents of the Sync Issues folder itself. This retention tag is applied automatically to the contents of the subfolders under the Sync Issues folder.
...
For more information about retention policies and retention tags, see [Understanding Retention Tags and Retention Policies](/Exchange/policy-and-compliance/mrm/retention-tags-and-retention-policies).
