...
This article describes an issue in which email messages received from external senders or generated by applications aren't delivered and generate a "554 5.6.0 STOREDRV.Deliver; Corrupt message content" error. The issue occurs when messages contain attachments that are incorrectly encoded as `application/mac-binhex40` content by using Base64 encoding instead of the standard BinHex format. The article explains the cause of the message rejection and provides guidance for ensuring that attachments are sent in a supported format.
...
As per the **Appendix A. The BinHex format** part in [RFC 1741 - MIME Content Type for BinHex Encoded Files](http://www.ietf.org/rfc/rfc1741/):
