---
title: How to install a second KMS host for Office in an environment that has an existing KMS host for Windows
...
You can follow the steps in [this article](http://technet.microsoft.com/library/ff793405.aspx) to configure DNS for multiple KMS hosts. Be aware that the SRV resource record that you want to edit permissions in DNS is the _VLMCS record of your KMS host. Editing this record’s permissions to grant the global security control will enable additional _VLMCS records to be created by the other KMS hosts as needed.
