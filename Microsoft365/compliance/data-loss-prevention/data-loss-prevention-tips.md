...
|Sensitive data threshold for the DLP policy isn't met. Therefore, the associated DLP policy tips aren't displayed.|Modify the conditions of the applicable DLP policy by using the steps in [Manage DLP Policies](/microsoft-365/compliance/data-loss-prevention-policies). For example, you may have to adjust the minimum or maximum sensitive item count values or adjust the kind of sensitive data that triggers the policy.|
