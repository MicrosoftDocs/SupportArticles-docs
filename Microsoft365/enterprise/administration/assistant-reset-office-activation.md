...
## Script to run the Reset Office Activation scenario

To show you how the Enterprise version of the Assistant can be scripted to run the Reset Office Activation scenario, we created a sample Windows PowerShell script.

> [!NOTE] 
> The script requires an elevated Windows PowerShell session (use 'Run as administrator').
> 
...
The script provides the following basic capabilities, but it can be edited to better fit your own workflows.
...
2. Launch SaraCmd.exe to run the Reset Office Activation scenario with the required switches and parameters

3. Gather log files generated during the scenario

   The script collects all log files generated by the Assistant.

4. Create a consolidated zip file with all of the logs
