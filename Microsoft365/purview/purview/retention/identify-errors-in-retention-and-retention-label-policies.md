...
In the [Microsoft Purview portal](https://compliance.microsoft.com/):
...
> [!NOTE]
> When you select the policy to open the policy details pane, make sure that you select the policy name and not the corresponding checkbox. Otherwise, the details pane doesn't open.
...
- Check whether the sync and distribution status of a policy is pending before you make any further updates to the policy. If you use the Microsoft Purview portal, check the **Status** section in the policy details pane. If you use PowerShell, run the following command:
...
- Merge your updates to a policy in a single bulk request instead of repeatedly running each policy update command. For example, if you want to add multiple locations to a policy, use the [Set-RetentionCompliancePolicy](/powershell/module/exchange/set-retentioncompliancepolicy) cmdlet to add all locations in a single bulk request, as shown in the following example.
