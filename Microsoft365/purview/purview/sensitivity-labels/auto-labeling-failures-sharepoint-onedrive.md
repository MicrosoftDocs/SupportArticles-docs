...
- If `EncryptedFileNotSupported` persists when the observed file properties don't show encryption or a relevant restriction, collect multiple representative failing samples and normalized test copies before you contact Microsoft Support. Record the failure codes, timestamps, restrictions, and test outcomes for each sample.
