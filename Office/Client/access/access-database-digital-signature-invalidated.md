...
description: Describes database usage scenarios that are known to invalidate the digital signature of Access database and provides workarounds.
author: helenclu
...
- You open a digitally signed database outside of a trusted folder location.

In this scenario, you may receive one or more of the following  messages indicating that the [digital signature](https://support.microsoft.com/office/show-trust-by-adding-a-digital-signature-5f4ebff3-360d-4b61-b2f8-ce0dfb53adf6) applied to the database is no longer valid:
...
To work around the issues, select the appropriate options for different scenarios.

### Action queries and passthrough queries

You can use VBA code to create, modify, or delete action queries and passthrough queries in Access without invalidating the digital signature, as long as any changes to objects are reverted in the same database session. 
...
- You use a [QueryDefs collection](/office/client-developer/access/desktop-database-reference/querydefs-collection-dao) to locate a named passthrough query and edit the `QueryDef.SQL` property. Before you close the database, you revert the `QueryDef.SQL` property to its original value.
- You create and execute an action query or passthrough query by using a temporary [QueryDef](/office/client-developer/access/desktop-database-reference/querydef-object-dao) object.

### The Perform name AutoCorrect option is enabled

Before applying a digital signature, make sure that there are no pending name corrections by opening and saving any objects that may contain pending changes.

**Note:** For databases with many objects, consider using a code loop to perform this task.
...
Consider using a split database where the front-end database has a digital signature applied. After each use, replace the front-end database with a new copy by using a batch file or other scripts.
