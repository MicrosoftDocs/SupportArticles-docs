...
The Jet Database Engine was designed to be a general-purpose database engine supplied with Windows. The Access Database Engine, also called Access Connectivity Engine (ACE), is Office’s database engine built from the foundations of Jet. ACE is provided in both 32-bit and 64-bit architecture, but only one architecture is allowed on a device. The architecture of ACE must match the architecture of Office installed on that machine. Although both engines provide similar functionality, ACE follows the design principles of Office and is not suited for some of the previous scenarios supported by Jet.  Please read through all the intended use scenarios provided on the download page [Download Microsoft Access Database Engine 2016 Redistributable](https://www.microsoft.com/download/details.aspx?id=54920) to make sure using ACE is the best solution for your scenario.  
