# [Office Products Troubleshooting](../client.md)

## Access
### ["Enter Parameter Value" dialog box appears](../access/enter-parameter-value-when-run-query.md)
### ["Linked table connection failed" when using Azure AD](../access/linked-table-connection-failed.md)
### [.LDB files](../access/ldb-file-description.md)
### [0x80004005 Microsoft Jet Database Engine cannot open the file](../access/error-80004005-connecting-access-database.md)
### [Access cache formats for SharePoint](../access/access-cache-formats.md)
### [Access crashes after you install Office 2010 SP1](../access/access-2010-crashes.md)
### [Access database engine cannot find input table](../access/database-engine-not-find-input-table.md)
### [Access error “Your network access was interrupted” when using a mapped drive](../access/your-network-access-was-interrupted-error.md) 
### [Access freezes when opening a linked table](../access/access-freezes-when-open-linked-table.md)
### [ADODB code errors with adBiglnt data](../access/adBigInt-data-type-errors.md)
### [Automate the process of selecting printer for a report](../access/automate-selecte-printer-for-report.md)
### [Build a dynamic query with values from a search form](../access/build-dynamic-query-from-search-form.md)
### [Cannot update. Database or object is read-only](../access/database-or-object-is-read-only.md)
### [Can't add or delete records with ADO methods](../access/cannot-add-delete-records-ado.md)
### [Can't enter a large amount of text in Memo field with an index](../access/run-time-error-3709.md)
### [Can't import attribute-centric XML](../access/import-attribute-centric-xml.md)
### [Can't open a previous created database](../access/database-previous-version.md)
### [Can't open any wizard](../access/open-access-wizard.md)
### [Can't open databases compiled as MDE, ACCDE or ADE files](../access/error-run-compiled-mde-accde-ade.md)
### [Can't perform an operation on a table](../access/error-when-perform-operation-on-table.md)
### [Can't process transactions in a multi-user environment](../access/file-sharing-lock-count-exceeded.md)
### [Can't save a table after updating a field](../access/too-many-fields-defined-error.md)
### [Can't use CDec() function in Access query](../access/cdec-function-error.md)
### [Can't use ODBC driver or OLEDB provider](../access/cannot-use-odbc-or-oledb.md)
### [Change dates by using functions and operators](../access/change-dates-by-functions-operators.md)
### [Change the connection of a  project programmatically ](../access/change-the-connection-of-project.md)
### [Convert data values of Text data type to proper case format](../access/convert-data-values-of-text-data-type.md)
### [Could not deploy package when deploying DACPAC files](../access/deploy-dacpac-access-web-app-package.md)
### [Count occurrences of Yes or No in a field with Yes/No data type](../access/expressions-to-count-yes-no-other-reponses.md)
### [Crashes after install security update MS16-004](../access/access-crashes-update-ms16-004.md)
### [Create DSN-less connection to SQL Server for linked tables](../access/create-dsn-less-connection-linkted-table.md)
### [Create SQL pass-through query with ADO](../access/adox-create-sql-pass-through-query.md)
### [Database begins to bloat after you use DAO](../access/prevent-database-bloat-dao.md)
### [Database requires a newer version of Access](../access/database-requires-newer-access.md)
### [Decimal value  is rounded up or down to integer value](../access/decimal-value-rounded-up-down-integer.md)
### [Determine who is logged on to a database](../access/determine-who-is-logged-on-to-database.md)
### [Enforce or disable startup options in a database](../access/disable-database-startup-options.md)
### [Error when you work in Access table linked to a list](../access/cant-update-database-or-object-error.md)
### [Errors during importing/exporting to Excel](../access/import-export-access-to-excel.md)
### [Extra Access report page prints](../access/extra-access-report-page-prints.md)
### [Find a record by using ADO and Jet OLE DB provider](../access/find-record-by-ado-and-jet-ole-db-provider.md)
### [Force a new line in a MsgBox message](../access/force-new-line-msgbox-message.md)
### [Functions return a wrong week number](../access/functions-return-wrong-week-number.md)
### [Functions to calculate age in months and in years](../access/calculate-age-in-months-years.md)
### [How Table Analyzer Wizard works](../access/table-analyzer-wizard.md)
### [How to create a running totals query](../access/create-a-running-totals-query.md)
### [ID is increment when adding a new field](../access/id-field-increment-by-one.md)
### [Incorrect registry key for current version of Access](../access/incorrect-current-version-registry-key.md)
### [Issue when loading an Access web app](../access/load-access-web-app-in-ie.md)
### [Issues when you retrieve SharePoint list data](../access/retrieve-sharepoint-list-in-access.md)
### [Jet compact utility is available in download center](../access/jet-compact-utility-download.md)
### [Linked table to a SharePoint list returns #Deleted](../access/access-linked-table-return-deleted.md)
### [Linked table to SQL Server database returns #Deleted](../access/access-linked-table-deleted.md)
### [Memory leak when Access connects to database files](../access/access-engine-memory-leak.md)
### [Move to a specific record from Combo Box selection](../access/move-to-specific-record-from-combo-box-selection.md)
### [Not a valid Teradata SQL token when running outer join queries](../access/invalid-teradata-token.md)
### [ODBC Data Source Administrator crashes when adding a system DSN](../access/odbc-administrator-crashes.md)
### [OLE DB Provider for Jet and ODBC driver](../access/jet-odbc-driver-available-32-bit-version.md)
### [Optimize Access when using ODBC data sources](../access/optimize-odbc-data-sources.md)
### [Populate text boxes with multicolumn Combo Box selections](../access/text-box-multicolumn-combox-selections.md)
### [Query fails because of double-byte arabic number](../access/error-run-query-in-access.md)
### [Query must have at least one destination field](../access/query-must-have-at-least-one-destination-field.md)
### [Register or unregister ActiveX controls](../access/register-activex-controls.md)
### [Reset page number on group level in a report](../access/reset-page-number-on-a-group-level.md)
### [Runtime error 2147319779 (8002801d) library not registered](../access/library-unregistered-runtime-error.md)
### [Scroll records with mouse wheel](../access/scroll-records-with-mouse-wheel.md)
### [Security Update impairs Access Database](../access/update-impairs-access-database.md)
### [Shut down a custom Access application remotely](../access/shut-down-custom-application-remotely.md)
### [Split a database manually](../access/manually-split-database.md)
### [SQL Server Native Client drivers used by Access](../access/sql-server-native-client-drivers.md)
### [Store items from multiple selection list box as comma-delimited string](../access/retrieve-items-in-multiple-selection-list-box.md)
### [Store, calculate, and compare Date/Time data](../access/store-calculate-compare-datetime-data.md)
### [Use SetOption to disable warning messages](../access/setoption-disable-warning-messages.md)
### [Use SQLConfigDataSource to create a system DSN](../access/sqlconfigdatasource-access-system-dsn.md)
### [Use the Workgroup Administrator utility ](../access/use-workgroup-administrator-utility.md)



## Activation/Installation
### [Cannot verify the license for this product](../activation-installation/license-issue-when-start-office-application.md)
### [Can't activate Office on a new computer](../activation-installation/cant-activate-on-new-computer.md)
### [Can't pass the Welcome Screen in Office for Mac](../activation-installation/office-for-mac-welcome-screen.md)
### [Enable Office 2010 to run on a terminal server](../activation-installation/office-pro-plus-2010-terminal-server.md)
### [Error 1935 when install Office 2010](../activation-installation/error-1935-when-install-office-2010.md)
### [Install and activate Office 2013](../activation-installation/install-and-activate-office-2013.md)
### [Issue when activate Office 365 ProPlus](../activation-installation/issue-when-activate-office-365-proplus.md)
### [Issue when activate Office from Office 365](../activation-installation/issue-when-activate-office-from-office-365.md)
### [Issue with changing Office 2010 product key](../activation-installation/office-2010-product-key-change-error.md)
### [Known issues with Office for Mac](../activation-installation/office-for-mac-issues-macos-10-7.md)
### [Office setup issues](../activation-installation/office-setup-issues.md)
### [PowerPoint for Mac is closed unexpectedly ](../activation-installation/powerpoint-mac-closes-unexpectedly.md)
### [Something went wrong 30016-4 error ](../activation-installation/something-went-wrong-30016-4-error.md)
### [We can't sign you into your company portal](../activation-installation/something-configured-fails-activating-office.md)


## Error Messages
### ["Missing file" error with a web page file](../error-messages/web-page-missing-file-error.md)
### ["This file can’t be previewed" error](../error-messages/this-file-cant-be-previewed.md)
### [0x80004005: General Error Unable to Open Registry Key](../error-messages/fails-accessing-page-connected-access-database.md)
### [0x80041015 when starting an Office application](../error-messages/0x80041015-start-office-program.md)
### [Another account is already signed in](../error-messages/another-account-already-signed-in.md)
### [Apps for Office don't start ](../error-messages/apps-for-office-not-start.md)
### [Background task activation is spurious](../error-messages/background-task-activation-spurious.md)
### [Can't create user name that contains special characters](../error-messages/username-contains-special-character.md)
### [Can't install 64-bit version of Office with Click-to-Run](../error-messages/64-bit-cant-install-with-click-to-run.md)
### [Can't locate proxy server when clicking hyperlink](../error-messages/cannot-locate-server-when-click-hyperlink.md)
### [Can't open a file from UNC share](../error-messages/file-not-found-when-open-file-from-unc.md)
### [Can't open Click-to-run version of Office on terminal server](../error-messages/click-to-run-office-on-terminal-server.md)
### [Can't open Excel, Word or PowerPoint file](../error-messages/cannot-open-excel-word-or-powerpoint-file.md)
### [Can't open this add-in from localhost when loading an add-in](../error-messages/cannot-open-add-in-from-localhost.md)
### [Can't start an Office Starter program](../error-messages/cannot-be-opened-error-when-start.md)
### [Can't uninstall Office](../error-messages/error-when-uninstall-office.md)
### [Error 25090 with an Office application](../error-messages/error-25090-starting-office-program.md)
### [Error when edit an equation in Office](../error-messages/issue-when-edit-equation-in-office.md)
### [Error when you install Office 2013](../error-messages/office-2013-installation-error.md)
### [Error when you install VS2010 Toos for Office Runtime](../error-messages/generic-trust-failure-(0x800b010b)-error.md)
### [Error when you open a presentation](../error-messages/error-when-you-open-presentation.md)
### [Excel cannot complete this task](../error-messages/excel-cannot-complete-task-with-available.md)
### [Files necessary to run the Office Customization Tool were not found](../error-messages/fails-running-office-customization-tool.md)
### [Incorrect function with saving to a CD-RW or DVD/CD-RW drive](../error-messages/incorrect-function-saving-office-file.md)
### [Installing Office 2010 from Office 365 Subscription fails](../error-messages/install-office-from-office-365-subscription.md)
### [Invalid product key or license mismatch](../error-messages/invalid-product-key-or-license-mismatch-error.md)
### [Issue when launching Office in Microsoft Store app](../error-messages/windows-cannot-access-device.md)
### [Issue when Office 365 activates Skype for Business](../error-messages/office-365-activates-skype.md)
### [Issue when opening a file with a long folder name](../error-messages/fails-opening-long-folder-name-files.md)
### [Issue with starting Office applications](../error-messages/microsoft-program-cannot-be-opened.md)
### [Issue with updating a data connection or change PivotTable fields](../error-messages/connection-failed-to-refresh.md)
### [Mobile viewing PDF files with Office Web Apps](../error-messages/mobile-viewing-pdf-files.md)
### [Office 2010 installation doesn't finish](../error-messages/install-and-use-this-product-error.md)
### [Office has detected a problem with this file](../error-messages/office-file-fails-validation.md)
### [Repair process of click-to-run app](../error-messages/click-to-run-app-repair-process.md)
### [Run-time Error 13: Type Mismatch](../error-messages/run-time-error-13-type-mismatch.md)
### [Selected a single-byte font to replace a double-byte font with Japanese](../error-messages/single-byte-font-replace-error.md)
### [The document caused a serious error the last time it was opened](../error-messages/error-open-word-excel.md)
### [The password you supplied is not correct](../error-messages/incorrectly-password-fails-verifying.md)
### [View error signatures for Office](../error-messages/view-error-signatures.md)



## Excel
### ["Center Across Columns" doesn't appear on the toolbar](../excel/center-across-columns-button-disappear.md)
### ["Margins do not fit page size" error with print](../excel/fails-printing-a-workbook.md)
### ["Total" row isn't shown in waterfall charts](../excel/table-total-row-not-shown-in-waterfall.md)
### [.SpecialCells(xlCellTypeBlanks) VBA function doesn't work](../excel/specialcells(xlcelltypeblanks)-vba-function.md)
### [A file is updated without user interaction](../excel/file-updated-without-user-interaction.md)
### [A serious error the last time it was opened](../excel/opening-file-cause-error.md)
### [Access data connections are slow to refresh](../excel/access-data-connections-refresh-slowly.md)
### [Algorithm is used by the XIRR() function](../excel/algorithm-of-xirr-funcation.md)
### [An active process continues to run](../excel/quitting-process-continues-running.md)
### [Apply permissions to separate ranges](../excel/apply-password-to-separate-range.md)
### [AutoComplete may not work](../excel/autocomplete-not-work.md)
### [AutoRecover functions in Excel](../excel/autorecover-functions-in-excel.md)
### [Blank pages are unexpectedly printed ](../excel/blank-pages-unexpectedly-printed.md)
### [Border missing when pasting table in email for Mac](../excel/table-border-missing.md)
### [Calculate ages before 1/1/1900](../excel/calculate-age-before-1-1-1900.md)
### [Calculate interpolation step value](../excel/calculate-interpolation-step-value.md)
### [Cannot use object linking and embedding](../excel/cant-use-object-linking-and-embedding-error.md)
### [Can't add refedit control to VBA userform](../excel/cant-add-refedit-control-to-vba-userform..md)
### [Can't create a PivotTable in Excel 2013](../excel/cannot-create-pivottable.md)
### [Can't export to Excel from SharePoint Online](../excel/cannot-export-to-excel.md)
### [Can't open .xls files](../excel/cannot-open-xls-file.md)
### [Can't open a workbook saved in Excel version 1805](../excel/cant-open-workbook-saved-in-excel-1805.md)
### [Can't open IRM protected workbook](../excel/cannot-open-irm-protected-workbook.md)
### [Can't paste any attributes into a workbook in another instance](../excel/cannot-paste-attributes.md)
### [Can't remove error tracing arrow](../excel/removing-error-tracing-arrow-fails.md)
### [Can't resize the Chart Area of a chart](../excel/cannot-resize-chart-area.md)
### [Can't scroll to view other areas](../excel/fails-scrolling-view-other-areas.md)
### [Can't use external references with data validation](../excel/external-references-data-validation-fails.md)
### [Category labels are missing or too small to read](../excel/chart-category-labels-missing.md)
### [Cell content is displayed as #N/A! ](../excel/cell-content-shows-na-enable-editing.md)
### [Cell shading continually darkens](../excel/cell-shading-continually-darkens.md)
### [Change formula from absolute to relative or mixed references](../excel/change-formular-reference-type-.md)
### [Change the Software License Registration Information for Mac](../excel/change-software-license-registration.md)
### [CHIINV statistical functions](../excel/statistical-functions-chiinv.md)
### [Clean up workbooks with less memory](../excel/clean-workbook-less-memory.md)
### [Columns and rows are labeled numerically](../excel/numeric-columns-and-rows.md)
### [COM add-ins are not displayed ](../excel/com-add-ins-not-displayed.md)
### [Command bars of add-ins aren't displayed or removed](../excel/add-ins-command-bars-removed.md)
### [Command can't be used on multiple selections](../excel/command-cannot-be-used-on-selections.md)
### [Conditional formatting is set incorrectly with VBA](../excel/conditional-formatting-set-incorrectly.md)
### [Connection to ODBC driver fails in PowerPivot](../excel/connection-to-odbc-driver-fails.md)
### [Convert degrees/minutes/seconds angles](../excel/convert-degrees-minutes-seconds-angles.md)
### [Convert Excel column numbers](../excel/convert-excel-column-numbers.md)
### [Crashing issues with Excel](../excel/crashing-issues-with-excel.md)
### [Create a conditional format to hide errors](../excel/hide-errors.md)
### [Create a dynamic defined range](../excel/create-dynamic-defined-range.md)
### [Create a formula to correctly evaluate blank cells](../excel/use-formula-evaluate-blank-cell.md)
### [Create a monthly calendar](../excel/create-monthly-calendar.md)
### [Create a sequential numbered list](../excel/create-sequential-numbered-list.md)
### [Create and user two-input data tables](../excel/create-two-input-data-tables.md)
### [Create application-level event handlers](../excel/create-application-level-event-handler.md)
### [Custom function may not calculate expected value](../excel/custom-function-calculate-wrong-value.md)
### [Data collected by Excel Baseline Diagnostic](../excel/data-collected-by-excel-support-diagnostic.md)
### [Date format is incorrect when converting a CSV text file with macros](../excel/incorrect-date-format-converted-csv-file.md)
### [Delete cells with a "For Each...Next" loop](../excel/use-for-each-next-loop-to-delete-cell.md)
### [Delete every other row on a worksheet](../excel/delete-every-other-row.md)
### [Determine the current mode of calculation](../excel/current-mode-of-calculation.md)
### [Determine whether a year is a leap year](../excel/determine-a-leap-year.md)
### [Difference between STDEVPA and STDEVP functions](../excel/statistical-functions-differences.md)
### [Differences between 1900 and 1904 date system](../excel/1900-and-1904-date-system.md)
### [Digital signatures and code signing](../excel/digital-signatures-code-signing.md)
### [Display more digits in trendline equation coefficients](../excel/trendline-equation-coefficient-digits.md)
### [Dsum and Dcount functions not working](../excel/dsum-and-dcount-not-work-with-pivottables.md)
### [Embedded macros are blocked from running](../excel/embedded-macros-not-run.md)
### [Endlessly loop calculating cells when inserting several subtotals](../excel/endlessly-loop-calculating.md)
### [Error when opening Excel files signed in newer version](../excel/document-contains-invalid-signatures-error.md)
### [Error when sending commands in Excel](../excel/error-when-send-commands-in-excel.md)
### [Error when you copy formulas over large area](../excel/copy-formulas-over-large-area.md)
### [Error when you import data to a workbook](../excel/index-outside-bounds-of-array-error.md)
### [Error when you save a workbook in OneDrive](../excel/excel-file-path-too-long.md)
### [Excel 2013 crashes when you scroll down](../excel/excel-crashes-mouse-wheel.md)
### [Excel 2013 or Word 2013 crashes on close](../excel/excel-word-crash-on-close.md)
### [Excel 2016 for Mac crashes](../excel/excel-2016-for-mac-crashes.md)
### [Excel crashes when moving Excel sheet tab](../excel/crash-when-move-excel-sheet-tab.md)
### [Excel crashes when working on multiple workbooks](../excel/crash-when-work-on-multiple-workbooks.md)
### [Excel crashing issues are fixed in recent updates](../excel/excel-crash-issues-fixed-in-recent-updates.md)
### [Excel doesn't recognize data as Date type data](../excel/not-recognize-data-as-date-type-data.md)
### [Excel doesn't shut down after calling Quit method](../excel/jscript-not-shut-down-excel.md)
### [Excel hangs, freezes or stops working](../excel/excel-not-working.md)
### [Excel incorrectly assumes 1900 is a leap year](../excel/wrongly-assumes-1900-is-leap-year.md)
### [Excel opening blank](../excel/excel-opens-blank.md)
### [Excel stops working after KB3118373 is installed](../excel/excel-has-stopped-working-error.md)
### [Excel takes long time to insert or delete rows](../excel/excel-takes-long-time-to-insert-delete-rows.md)
### [Export a text file with both comma and quote delimiters](../excel/export-text-file-with-comma-quote.md)
### [F4 keyboard shortcut doesn't work in Excel](../excel/f4-keyboard-shortcut-not-work.md)
### [FALSE result with the ISBLANK() function](../excel/isblank-function-return-false.md)
### [First sheet name displays different language](../excel/first-sheet-name-displays-different-language.md)
### [Floating-point arithmetic gives inaccurate results](../excel/floating-point-arithmetic-inaccurate-result.md)
### [Force Excel to open in a new instance by default](../excel/force-excel-to-open-new-instance.md)
### [Format Cells settings](../excel/format-cells-settings.md)
### [Formatted text is limited to 240 characters per line](../excel/prn-file-limited-to-240-characters.md)
### [Formula errors when list separater isn't set](../excel/formula-errors.md)
### [Formulas to count text occurrences in Excel](../excel/formulas-to-count-occurrences-in-excel.md)
### [Gridlines are not printed](../excel/gridlines-not-print.md)
### [GROWTH statistical functions](../excel/statistical-functions-growth.md)
### [Hide sheets and use xlVeryHidden constant in a macro](../excel/hide-sheet-and-use-xlveryhidden.md)
### [How column widths are determined](../excel/determine-column-widths.md)
### [How Excel works with two-digit year numbers](../excel/two-digit-year-numbers.md)
### [How to obtain the latest Excel Viewer](../excel/get-latest-excel-viewer.md)
### [Hyperlink to mht file fails to load](../excel/hyperlink-to-mht-file-fails-to-load.md)
### [Hyperlinks stop working when saving as a PDF](../excel/hyperlinks-not-work-in-pdf.md)
### [Incorrect icon shows when inserting an object](../excel/inserted-object-icon-incorrect.md)
### [Inquire COM add-in options don't respond](../excel/inquire-com-add-in-not-respond.md)
### [INTERCEPT statistical functions](../excel/statistical-functions-intercept.md)
### [Issue when save Excel workbooks](../excel/issue-when-save-excel-workbooks.md)
### [Issue when setting the Fit to option](../excel/not-fit-worksheet-to-page.md)
### [Issue when sorting a range that has merged cells](../excel/error-sort-a-range-merged-cell.md)
### [Issue when starting Excel for Mac](../excel/fails-starting-excel-mac.md)
### [Issue when updating external links](../excel/update-external-links.md)
### [Issue with using different print quality for sheets](../excel/different-print-quality.md)
### [Large Address Aware capability change](../excel/laa-capability-change.md)
### [Legacy UML features are missing](../excel/legacy-uml-features-missing.md)
### [Limitations for working with arrays](../excel/arrays-limitations.md)
### [Link a Visio drawing to a specific region](../excel/link-visio-drawing-to-a-specific-region.md)
### [LOGEST statistical functions](../excel/statistical-functions-logest.md)
### [Long numbers appear incorrectly in Excel](../excel/long-numbers-incorrectly-in-excel.md)
### [Loop through a list of data by using macros](../excel/loop-through-data-using-macro.md)
### [Macro to extract data from a chart](../excel/macro-retrieves-chart-data.md)
### [Macros run slowly](../excel/macros-run-slowly.md)
### [Make Paste Options button disappear](../excel/make-paste-options-button-disappear.md)
### [Manage multiple windows](../excel/manage-multiple-windows.md)
### [Memory usage in the 32-bit edition of Excel](../excel/memory-usage-32-bit-edition-of-excel.md)
### [Minimize the size of an XML Spreadsheet file](../excel/minimize-xml-spreadsheet-file-size.md)
### [Misleading labels in the output of Analysis ToolPak t-Test tools](../excel/output-misleading-labels.md)
### [Network Mapped Drive hyperlinks resolve as UNC](../excel/network-mapped-drive-hyperlinks-unc.md)
### [Office 365 users can't open or synchronize files](../excel/cannot-operate-files.md)
### [Operation Must Use an Updateable Query](../excel/fails-accessing-excel-through-odbc.md)
### [Parts of Excel turn white or gray](../excel/parts-of-excel-turn-white-or-gray.md)
### [Paster special option is missing or not working](../excel/paste-special-missing-or-not-working.md)
### [Power View sheet is not displayed in a workbook ](../excel/power-view-sheet-not-displayed.md)
### [PowerPivot controls disabled with non-default Excel file format](../excel/powerpivot-controls-disabled-with-non-default.md)
### [Prevent files from opening automatically](../excel/files-open-automatically.md)
### [Print preview misses lines](../excel/print-preview-miss-lines.md)
### [Programmatically copy all range names](../excel/copy-range-names.md)
### [Programmatically print all embedded charts](../excel/print-embedded-charts.md)
### [Programmatically save a file to a network drive](../excel/save-file-to-network-drive.md)
### [Query and update data with ADO from ASP](../excel/query-update-data.md)
### [Recalculation with a SUMIF formula takes a longer time](../excel/worksheet-recalculation-sumif-formula.md)
### [Registry keys that control the File Repair feature](../excel/file-repair-registry-key.md)
### [Remove smart tags from a workbook](../excel/remove-workbook-smart-tags.md)
### [Representing ties by using RANK](../excel/representing-ties-using-rank.md)
### [RSQ statistical functions](../excel/statistical-functions-rsq.md)
### [Run a macro with certain cells change](../excel/run-macro-cells-change.md)
### [Run-time Error 1004 with legend entries in charts](../excel/run-time-error-1004.md)
### [Run-time error '-2147467259 (80004005)' when setting a chart property](../excel/run-time-error-2147467259-80004005.md)
### [Sample macro to insert/delete rows or columns](../excel/macro-insert-delete-rows-columns.md)
### [Sample user-defined function to hide formula errors](../excel/hide-formula-error.md)
### [SAP add-ins don't display controls after update to version 1806](../excel/sap-add-ins-not-display-control.md)
### [Save the addresses from a Word mail-merge label document](../excel/save-mail-merge-label-document-addresses.md)
### [Set page setup attributes for sheets](../excel/set-page-setup-attributes.md)
### [Set starting page in header or footer](../excel/set-starting-page-number.md)
### [Set up and use the RTD function](../excel/set-up-realtimedata-function.md)
### [Smart tag functionality](../excel/smart-tag-functionality.md)
### [Startup message about updating linked workbooks ](../excel/control-startup-message.md)
### [STEYX statistical functions](../excel/statistical-functions-steyx.md)
### [Too many different cell formats in Excel](../excel/too-many-different-cell-formats-in-excel.md)
### [Toolbar missing or reset menus in Excel for Mac](../excel/toolbar-missing-reset-menu.md)
### [Transfer data to Excel from Visual Basic](../excel/transfer-data-to-excel-from-vb.md)
### [Troubleshoot available resources issues](../excel/available-resources-errors.md)
### [Turn off Function Argument ToolTips](../excel/turn-off-function-argument-tooltips.md)
### [Unable to cast COM object](../excel/team-explorer-export-fails.md)
### [Unexpected value with the DAYS360 function](../excel/days360-function-returns-unexpected-value.md)
### [Use a worksheet function in a VB macro](../excel/worksheet-function-in-vb-macro.md)
### [Use cell references and defined names in criteria](../excel/use-cell-references-defined-names.md)
### [Use defined names to update a chart range](../excel/defined-names-to-update-chart-range.md)
### [Use Draw Border tools with cell borders](../excel/use-draw-border-tools.md)
### [Use INDIRECT function to create references](../excel/indirect-function-to-create-reference.md)
### [Use left, right, mid, and len functions in VB](../excel/use-left-right-mid-len-function.md)
### [Use logical AND or OR in a SUM+IF statement ](../excel/logical-and-or-in-sum-if.md)
### [Use macro to apply cell shading format to other rows](../excel/apply-cell-shading-format-by-macro.md)
### [Use OnEntry macro to create a running total in cell comment](../excel/use-onentry-to-create-running-total.md)
### [Use saved property to determine if workbook is changed](../excel/saved-property-determine-workbook-is-changed.md)
### [Use shared workbooks with different versions](../excel/use-shared-workbook.md)
### [Use startup folders](../excel/use-startup-folders.md)
### [Use the "Insert Options" button](../excel/insert-options-button.md)
### [Use the AutoRepublish feature](../excel/autorepublish-feature.md)
### [Use the IF worksheet function to suppress the #DIV/0! Error](../excel/if-worksheet-function-div0.md)
### [VBA writes to cells slowly when ActiveX controls are invisible](../excel/vba-writes-to-cells-slowly.md)
### [Workbook isn't activated when you run a macro](../excel/workbook-not-activated.md)
### [Workbook loads slowly the first time that it is opened](../excel/workbook-loads-slowly.md)
### [Workbooks not open ](../excel/workbooks-not-open.md)
### [Worksheets may be printed at an unexpected different size](../excel/worksheet-printed-different-size.md)
### [Wrap text doesn't adjust row height](../excel/wrap-text-not-adjust-row-height.md)


## InfoPath
### [Disable code-behind prompt for InfoPath forms](../infopath/disabling-code-behind-prompt-for-infopath-forms.md)
### [Generated .PDB files don't match InfoPath form name](../infopath/generated-pdb-file-name-does-not-match-infopath-name.md)

## Office
### ["No default mail client" error](../office/no-default-mail-client-error.md)
### ["Your system is low on virtual memory" error](../office/your-system-is-low-on-virtual-memory.md)
### [Applications crash when opening IRM doccument](../office/office-applications-crash-when-open-irm-doc.md)
### [Can't change the number of places in Recent Places](../office/change-number-of-recent-places.md)
### [Can't embed Adobe OpenType fonts](../office/fails-embedding-adobe-opentype-font.md)
### [Can't install 64-bit Office with 32-bit](../office/cannot-install-64-bit-office-with-32-bit.md)
### [Can't install Office Starter 2010 on Windows 8](../office/cant-install-office-on-windows-8.md)
### [Can't use "Convert bibliography to static text"](../office/convert-bibliography-to-static-text.md)
### [Compile Error: Procedure too large](../office/compile-error-procedure-too-large.md)
### [Connection to a network document is lost](../office/connect-to-network-document-lost.md)
### [Document files in Office Document Cache not deleted](../office/office-temporary-files-not-delete.md)
### [Edit OLE document property with Dsofile.dll ](../office/dsofile.dll-allow-edit-document-property.md)
### [Error when you install an Office program](../office/error-run-office-setup-program.md)
### [Error when you move mailboxes to or from Office 365](../office/error-move-mailboxes-to-from-o365.md)
### [FAQ about Office Activation Wizard](../office/faq-about-office-activation-wizard.md)
### [Information about Office Click-to-Run installation](../office/office-click-to-run-installation.md)
### [Introduce Click-to-Run for Office 2010](../office/office-click-to-run-faq.md)
### [Issues when you click hyperlink to SSO Web site in a document](../office/click-hyperlink-to-sso-website.md)
### [Loss of image quality occurs in a document](../office/office-docuemnt-image-quality-loss.md)
### [Manage templates in Office program ](../office/manage-office-program-templates.md)
### [No Office entitlement found on device](../office/office-not-entitled-on-device.md)
### [Office 2010 Installation failure](../office/office-installation-failure.md)
### [Office 2010 installation requires MSXML ](../office/office-installtion-need-msxml.md)
### [Office 2010 Setup program not restart](../office/office-suite-setup-not-restart.md)
### [Office crash with SendToBluetooth add-in installed](../office/office-crash-with-sendtobluetooth.md)
### [Office End User License Agreement always appears](../office/office-end-user-license-agreement.md)
### [Office files with RMS protection crash](../office/rms-protected-files-crash.md)
### [Office installtion Error 1401, 1402 or 1406](../office/office-install-error-1401-1402-1406.md)
### [Office stops responding when you open a file](../office/office-stops-responding.md)
### [Prevent Office 2010 from disabling printers](../office/prevent-office-from-disabling-printers.md)
### [Saving documents to network server is slow](../office/saving-file-to-network-server-slow.md)
### [Services display incorrectly in Research pane](../office/duplicate-service-listed.md)
### [Some VB Editor settings disabled in Office 2010](../office/vb-editor-settings-disabled.md)
### [The ability to uninstall Office updates](../office/ability-to-uninstall-office.md)
### [Unable to create DSN for Office System Driver](../office/cant-create-dsn-for-office-system-driver.md)
### [Use GetObject and CreateObject with automation server](../office/getobject-createobject-behavior.md)
### [Win32API_PtrSafe with 64-bit Support](../office/win32api_ptrsafe-with-64-bit-support.md)
### [Windows Installer service could not be accessed](../office/windows-installer-service-not-be-accesse.md)


## Office for Mac
### ["wkresources" files in Trash](../office-for-mac/wkresources-files-moved-to-trash.md)
### [Error when you open files from SharePoint in Office client](../office-for-mac/files-fail-to-open.md)
### [Office 2016 for Mac 64-bit upgrade](../office-for-mac/office-2016-for-mac-64-bit-upgrade.md)
### [Office 2016 for Mac prompts you for authentication](../office-for-mac/office-for-mac-prompted-for-authentication.md)
### [Office 365 Enterprise E3 setup is incomplete](../office-for-mac/o365-e3-setup-incomplete.md)

## OneNote
### ["Open new side note" missing from  Windows notification](../onenote/open-new-side-note-missing.md)
### [Locate Onenote notebooks](../onenote/locate-onenote-notebooks.md)
### [Warning when a notebook is in an offline location](../onenote/notebook-offline-location.md)

## Performance
### [Office applications are crashing](../performance/office-applications-stop-working.md)
### [Open network file slow or stop responding](../performance/office-slow-or-stop-responding.md)

## PowerPoint
### ["Play CD Audio Track" command is not available](../powerpoint/play-cd-audio-track-not-available.md)
### [A .pps file is saved as a .pptx file](../powerpoint/pps-file-saved-as-pptx-file.md)
### [Add custom values to animation effect options](../powerpoint/custom-value-animation-effect-option.md)
### [Add section command is unavailable](../powerpoint/add-section-not-available.md)
### [Audio stops playback after you leave original slide](../powerpoint/audio-stops-playback.md)
### [Cannot edit the footer information in the Header and Footer](../powerpoint/fails-editing-footer-information.md)
### [Cannot edit video or audio in a presentation](../powerpoint/cannot-edit-video-or-audio.md)
### [Cannot load an add-in with a low macro security level](../powerpoint/powerpoint-fails-loading-add-in.md)
### [Change the export resolution of PowerPoint slide](../powerpoint/change-export-slide-resolution.md)
### [Create a mailto hyperlink in a presentation in PowerPoint for Mac](../powerpoint/create-powerpoint-mailto-hyperlink.md)
### [Embedded videos don't play](../powerpoint/embedded-video-not-play.md)
### [File size of an existing presentation increases](../powerpoint/presentation-file-size-increases.md)
### [Hlink.dll Can't Be Loaded](../powerpoint/viewing-presentation-fails.md)
### [Not enough printer memory available to print page](../powerpoint/not-enought-printer-memory-available.md)
### [Office opens blank from SharePoint](../powerpoint/office-opens-blank-from-sharepoint.md)
### [Only the audio plays with a video file](../powerpoint/powerpoint-video-file-only-audio-plays.md)
### [Password to modify a presentation is required](../powerpoint/require-password-to-modify-presentation.md)
### [PowerPoint 2010 not responding](../powerpoint/powerpoint-not-responding.md)
### [Powerpoint doesn't print in pure black and white](../powerpoint/powerpoint-not-print-in-pure-black-white.md)
### [PowerPoint files not rendering in Office Online Server after Security Update](../powerpoint/files-not-rendering-office-online-server-after-security-update.md)
### [PowerPoint for Mac startup error](../powerpoint/fails-starting-powerpoint-mac.md)
### [PowerPoint stops responding (spinning wheel)](../powerpoint/powerpoint-stops-responding.md)
### [Shortcuts to control display of grid and guides](../powerpoint/grid-guides-keyboard-shortcuts.md)
### [Slide transition sounds are missing ](../powerpoint/slide-transition-sounds-missing.md)
### [Troubleshoot a damaged presentation](../powerpoint/damaged-presentation.md)
### [You can't embed an online video in a presentation](../powerpoint/cant-embed-online-video.md)
### [Videos are listed as "Unsupported](../powerpoint/videos-listed-as-unsupported.md)


## Project
### ["Can't save to global.mpt" when existing Project](../project/cant-save-to-global.mpt-error.md)
### [Calculate work and duration with resource assignment changes](../project/calculate-work-duration-assignment.md)
### [Can't add or edit resources in Enterprise Resource Pool](../project/error-when-add-edit-resource.md)
### [Can't enter User Logon Account for new resource](../project/cannot-enter-user-logon-account.md)
### [Can't view a timesheet in PWA](../project/iexplorer.exe-encountered-problem.md)
### [Circular relationship error when indenting task](../project/circular-relationship-error-when-indenting-task.md)
### [Create a report template](../project/create-report-template.md)
### [Definition of project constraints](../project/definition-of-project-constraints.md)
### [Error when accessing Project Web App](../project/error-when-access-pwa.md)
### [Error when opening a visual report](../project/error-when-open-visual-report-in-project.md)
### [Error when you click project link](../project/error-when-click-on-project-links.md)
### [Errors when you access PWA](../project/error-accessing-project-web-app.md)
### [File size increases when inserting picture](../project/file-size-increase-when-insert-picture.md)
### [Gantt Chart or Usage views print on too many pages](../project/gantt-chart-usage-view-print-issues.md)
### [How to remove leading split in a task](../project/remove-leading-split-in-task.md)
### [How to upgrade Project global template](../project/upgrade-project-global-template.md)
### [Import task outline structure into-project](../project/import-task-outline-structure.md)
### [Issue when change user's status to inactive in Project](../project/eventing-service-error-when-changing-user-status-to-inactive-microsoft-project.md)
### [Make a custom toolbarribbon available to all users](../project/custom-toolbarribbon-available-to-all-users.md)
### [Operatio times out when connecting to Odata](../project/operation-times-out-if-connect-odata.md)
### [Print Gantt Chart view without table information](../project/print-gantt-chart-view-without-table.md)
### [Progress bars not drawn for tasks that are in progress](../project/progress-bars-not-drawn-for-tasks.md)
### [Project opens calendar from a different instance](../project/project-opens-calendar-from-different-instance-when-editing.md)
### [Project Publish job process remains at 80%](../project/project-publish-jobs-not-complete.md)
### [Removed users are not inactivated](../project/removed-erp-group-user-activated.md)
### [SharePoint task list sync job fails when Project is published](../project/sharepoint-task-list-sync-job-fails-when-project-published.md)
### [Show fiscal dates on a timescaled view](../project/fiscal-dates-on-timescaled-view.md)
### [Sync Jobs take a long time to complete](../project/sync-jobs-take-long-time-to-complete.md)
### [Sync with SharePoint tasks list](../project/sync-with-tasks-from-project.md)
### [Tasks are not scheduled](../project/tasks-not-scheduled.md)


## Publisher
### [Publication is printed incorrectly](../publisher/print-publication-incorrectly.md)
### [Publisher can't open file with OLE objects](../publisher/publisher-cannot-open-the-file-error.md)
### [Text boxes in Publisher-created email are converted to bitmap](../publisher/text-boxes-coverted-to-bitmap.md)

## Reference
### [Document Information Panel is deprecated](../reference/document-information-panel-deprecate.md)
### [Enable co-authoring for OneDrive-synced files](../reference/enable-co-authoring-for-onedrive-synced-file.md)
### [Features to enable collaboration](../reference/enable-collaboration.md)
### [Incompatibilities between Office and Adob](../reference/incompatibilities-between-office-and-adob.md)
### [Numbering scheme for product code GUIDs](../reference/numbering-scheme-for-product-guid.md)
### [Numbering scheme for product code GUIDs](../reference/numbering-scheme-product-code-guids.md)
### [Office activation process issues](../reference/office-activation-process-issues.md)
### [Updates are offered for nonexistent Office programs ](../reference/updates-provided-for-nonexistent-office.md)
### [Use different versions of Office on the same PC](../reference/use-multiple-versions-of-office.md)


## Settings
### [Display issues in Office applications](../settings/office-display-issues.md)
### [Error in Office when file is blocked](../settings/file-blocked-in-office.md)
### [Task panes in Office](../settings/office-program-task-panes.md)
### [Use registry to clear the most recently used files list](../settings/clear-most-recently-used-file.md)

## End of Support
### [Office 2007 has reached end of extended support](../support/office-2007-not-support.md)
### [Support for 2013 versions of Office 365 ProPlus ends](../support/2013-versions-of-office-365.md)


## Visio
### ["Fit to Drawing" not working](../visio/fit-to-drawing-command-not-work.md)
### [Can't add visio drawing control](../visio/cannot-add-visio-drawing-control.md)
### [Can't change text or shape fill color](../visio/cant-change-text-or-shape-fill-color.md)
### [Can't open file type blocked by registry policy settings](../visio/file-blocked-by-registry-policy.md)
### [Can't open Visio files in Visio Web Access](../visio/open-visio-files-in-visio-web-access.md)
### [Enable Windows Search service to use Search for Shapes](../visio/search-for-shapes-use-windows-search.md)
### [Search for Shapes feature is discontinued](../visio/cannot-search-shapes-feature.md)
### [Text not displayed when the drawing opens in a browser](../visio/text-not-displayed-in-drawing.md)
### [This is not a valid win32 application when installing Office 365](../visio/not-a-valid-win32-application-error.md)
### [Troubleshoot damaged drawings in Visio](../visio/troubleshoot-damaged-drawings.md)
### [Visip doesn't have shapes and stencils](../visio/visio-not-have-certain-shapes-and-stencils.md)
### [Visio Services does not render diagrams](../visio/visio-services-not-render-diagrams.md)
### [Visio Services was unable to load Web Drawing](../visio/cannot-load-visio-web-drawing.md)

## Word
### ["End tag" error when opening docx file](../word/end-tag-error-when-open-docx.md)
### ["relationship rID8 was not found" in Word](../word/image-part-relationship-rld8-not-found-error-microsoft-word.md)
### [A connector doesn't move when moving a shape](../word/connector-not-move-with-shapes.md)
### [Background is not printed in Word](../word/background-is-not-printed-in-word.md)
### [Can't edit text in print preview](../word/cannot-edit-text-print-preview.md)
### [Can't insert QuickTime media into presentations](../word/cannot-insert-quicktime-media.md)
### [Can't modify oData connection in PowerPivot](../word/cannot-modify-odata-connection.md)
### [Can't open documents with unsupported XML elements](../word/custom-xml-elements-not-supported.md)
### [Can't open this file because it is larger than 512 Megabytes](../word/file-larger-than-512-megabytes.md)
### [Can't start the converter SSPDFCG_X64.cnv](../word/converter-sspdfcg-x64.cnv.md)
### [Can't use  mouse to draw selection rectangle](../word/cant-use-mouse-draw-selection-rectangle.md)
### [Change default view to Draft view](../word/change-default-view-to-draft-view.md)
### [Changes in Word content layout](../word/changes-in-content-layout.md)
### [Changes to default font not retained](../word/default-font-changes-not-retained.md)
### [Changes to settings aren't updated in the Preview window](../word/change-to-print-setting-not-update.md)
### [Configure the default search engine](../word/configure-search-engine-in-word.md)
### [Cross-reference links don't update to the correct heading number](../word/cross-reference-links-update.md)
### [Custom XML markup is removed](../word/custom-xml-markup-is-removed.md)
### [Customize the color of the spelling and grammar checker underlines](../word/spelling-grammar-checker-underline-color.md)
### [Damaged documents in Word](../word/damaged-documents-in-word.md)
### [Faq about hyperlinks in Word](../word/faq-about-hyperlinks-in-word.md)
### [Faq about the Comments feature in Word](../word/faq-about-comments-feature-in-word.md)
### [Fonts in Word](../word/troubleshoot-font-problems.md)
### [Image DPI is changed after you print to PDF](../word/image-dpi-change-after-print-to-pdf.md)
### [Issue when opening a .wps file](../word/open-wps-file-word.md)
### [Issues when start or use Word](../word/issues-when-start-or-use-word.md)
### [Mail merge issue](../word/mail-merge-error.md)
### [Merge multiple Word documents into one](../word/merge-word-documents.md)
### [Object may not appear in Word 2010](../word/object-not-appear-in-word-2010.md)
### [On-screen keyboard opens repeatedly](../word/on-screen-keyboard-open-repeatedly.md)
### [Opening file is blocked by registry policy](../word/opening-file-blocked-by-registry-policy.md)
### [Operating parameter limitations and specifications](../word/operating-parameter-limitation.md)
### [Overtype mode is disabled](../word/overtype-mode-is-disabled.md)
### [Performance issues with AutoRecover location](../word/performance-issue-on-autorecover-location.md)
### [Print a general field](../word/print-general-field.md)
### [Print a range of pages in a multiple-section document](../word/print-a-range-of-pages.md)
### [Print failures in Word](../word/print-failures-in-word.md)
### [Printer settings are ignored](../word/printer-settings-not-used.md)
### [Recover lost documents](../word/recover-lost-document.md)
### [Reset user options and registry settings in Word](../word/reset-options-and-settings-in-word.md)
### [Scan and insert images into a document](../word/insert-image-into-document.md)
### [Scroll bars are missing in Word for Mac](../word/word-scroll-bars-missing.md)
### [Spell checker doesn't check text in the form fields](../word/spell-checker-not-check-form-field-text.md)
### [Spell Checker not recognizes misspelled words in Word 2010](../word/not-recognize-mispespelled-words-in-word.md)
### [Tabbing form fields content controls not working](../word/tabbing-form-fields-content-controls-not-work.md)
### [Text is not copied from the Clipboard](../word/text-not-copied-from-clipboard.md)
### [Third-party installed fonts isn't in the font list in Word for Mac](../word/third-party-installed-font-fails-appearing.md)
### [Toolbars missing in Word for Mac](../word/word-mac-toolbars-missing.md)
### [Track Changes functionality disabled when RMS is disabled](../word/track-changes-disabled.md)
### [Troubleshooting guide for unwanted page breaks](../word/troubleshoot-unwanted-page-breaks.md)
### [Word could not create the work file](../word/word-could-not-create-the-work-file.md)
### [Word for Mac crashes with saving](../word/word-for-mac-crashes-or-quits.md)
### [Word for Mac is closed unexpectedly](../word/word-mac-closes-unexpectedly.md)
### [Work with toolbars in Word](../word/toolbars-and-toolbar-buttons-in-word.md)
### [Wrong paper size in the Printer Properties](../word/paper-size-printer-property.md)
### [You're prompted to save changes to global template](../word/save-changes-to-global-template.md)



## Office Developer
### [Add a button in Word](../office-developer/add-button-in-word.md)
### [Automate Access using Visual C#](../office-developer/automate-access-using-visual-c.md)
### [Automate create presentation using Visual Basic .NET](../office-developer/automate-create-presentation-using-visual-basic-net.md)
### [Automate Excel from C++](../office-developer/automate-excel-from-c.md)
### [Automate Excel from client-side VBScript](../office-developer/automate-excel-from-client-side-vbscript.md)
### [Automate Excel from HTML Web page](../office-developer/automate-excel-using-jscript.md)
### [Automate Excel from MFC and Visual C++ to fill data](../office-developer/automate-excel-from-mfc-fill-data.md)
### [Automate Excel From Visual Basic .NET](../office-developer/automate-excel-from-visual-basic-net.md)
### [Automate Excel From Visual C#.NET](../office-developer/automate-excel-from-visual-c.md)
### [Automate Excel from Visual Basic](../office-developer/automate-excel-from-visual-basic.md)
### [Automate Excel using Visual C# to fill data](../office-developer/automate-excel-using-visual-c-fill-data.md)
### [Automate Outlook using C++ in Visual Stuido 2010](../office-developer/automate-outlook-2010-using-c-with-mfc.md)
### [Automate Visio using VB .NET](../office-developer/how-to-automate-visio-with-visual-basic-.net.md)
### [Automate Word create mail merge](../office-developer/automate-word-create-mail-merge.md)
### [Automate Word for mail merge using Visual C#](../office-developer/automate-word-mail-merge-using-visual-c.md)
### [Automate Word from VB create mail merge for mailing labels](../office-developer/automate-word-create-mail-merge-for-mailing-labels.md)
### [Automate Word perform a client-side mail merge using XML](../office-developer/automate-word-mail-merge-using-xml.md)
### [Automate Word perform a mail merge fro VB .NET](../office-developer/automate-word-perform-mail-merge-from-vb-net.md)
### [Automate Word perform mail merge using C++ and MFC](../office-developer/automate-word-mail-merge-using-visual-c-mfc.md)
### [Automate Word using Visual Basic to create files](../office-developer/automate-word-create-file-using-visual-basic.md)
### [Automate Word using Visual C# to create files](../office-developer/automate-word-create-file-using-visual-c.md)
### [Call Excel macros](../office-developer/call-excel-macros.md)
### [Count cells number with cell color using VBA](../office-developer/count-cells-number-with-color-using-vba.md)
### [Create a RealTimeData server for Excel](../office-developer/create-realtimedata-server-in-excel.md)
### [Create script for Outlook Rules Wizard](../office-developer/create-script-for-rule-wizard-in-outlook.md)
### [Declare the return type explicitly in 64-bit macro](../office-developer/declare-return-type-in-64-bit-vba.md)
### [Display progress bar with user form in Excel](../office-developer/display-progress-bar-with-user-form-in-excel.md)
### [Embed and automate documents with VB](../office-developer/embed-automate-document-using-vb.md)
### [Encode attachment using Visual C# in InfoPath 2003](../office-developer/encode-and-decode-attachment-using-visual-c-in-infopath-2003.md)
### [Encode attachment using Visual C# in InfoPath 2010](../office-developer/encode-and-decode-attachment-using-visual-c-in-infopath-2010.md)
### [Export data in DataGrid on ASP .NET WebForm to Excel](../office-developer/export-data-in-datagrid-on-asp.net-webform-to-excel.md)
### [Find Office installation path](../office-developer/find-office-installation-path.md)
### [Handle events for Excel using VB .NET](../office-developer/handle-excel-event-using-vb-net.md)
### [Limit of Asynchronous Programming to Object Model](../office-developer/asynchronous-programming-to-object-model.md)
### [Macro delete duplicate items in Excel](../office-developer/macro-delete-duplicate-items-in-excel.md)
### [Office COM add-in using Visual C# .NET](../office-developer/office-com-add-in-using-visual-c.md)
### [Print comments in Visio](../office-developer/print-comment-in-visio.md)
### [Retrieve macro names from Excel using VB 6.0](../office-developer/retrieve-macro-name-in-excel-using-vb.md)
### [Select ranges or cells with Visual Basic](../office-developer/select-cells-rangs-with-visual-basic.md)
### [Transfer data to Excel workbook using Visual C#](../office-developer/transfer-data-to-excel-using-visual-c.md)
### [Transfer excel data from ADO Recordset](../office-developer/transfer-excel-data-from-ado-recordset.md)
### [Turn off Visual Basic for application](../office-developer/turn-off-visual-basic-for-application.md)
### [Use a type library for Office from Visual C++.NET](../office-developer/use-type-library-for-office-from-visual-c-net.md)
### [Use early binding and late binding in Automation](../office-developer/binding-type-available-to-automation-clients.md)
### [Use Office Web Components using ASP.NET](../office-developer/use-office-web-components-using-aps-net.md)
### [Use VB ActiveX for Word from Internet Explorer](../office-developer/use-vb-activex-for-word-automation.md)
### [Use VB to count occurrences in Excel](../office-developer/use-vb-count-occurrences-in-excel.md)
### [Use Visual C# to automate running program instance](../office-developer/use-visual-c-automate-run-program-instance.md)
### [Use WebBrowser in Visual Basic to open document](../office-developer/webbrowser-open-office-document-in-visual-basic.md)
### [Use WebBrowser in Visual C# to open document](../office-developer/webbrowser-open-office-document-in-visual-c.md)
### [Use XML Web Service using ASP.NET in Word or Excel](../office-developer/use-xml-web-service-in-word-or-excel.md)
### [Word automation to count page numbers](../office-developer/use-word-automation-count-page-number.md)
