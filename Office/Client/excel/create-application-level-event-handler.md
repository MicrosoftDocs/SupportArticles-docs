...
If you want a particular event handler to run whenever a certain event is triggered, you can write an event handler for the Application object. Event handlers for the Application object are global, which means that as long as Microsoft Excel is open, the event handler runs when the appropriate event occurs, regardless of which workbook is active when the event occurs.
...
3. Click Class Module on the Insert menu. This operation inserts a module titled "\<book name> - Class1 (Code)" into your project.
...
5. In the Class1 (Code) module window, click the Object drop-down and then select appevent in the list.   
6. In the Class1 (Code) module window, select the Procedure drop-down and then select WindowResize in the list. This operation adds the following to the Class1 (Code) module sheet:
...
7. Add code to the Class1 (Code) module sheet so that it appears as follows:
    ...
    Next, you have to create an instance of the class and then set the appevent object of the instance of the Class1 to Application. This happens because when you declare a variable, WithEvents, at design time, there's no object associated with it. A WithEvents variable is just like any other object variable - you have to create an object and assign a reference to the object to the WithEvents variable.

8. On the Insert menu, select Module to insert a general type module sheet into your project.   
...
11. On the File menu, select **Close and Return to Microsoft Excel**.   
12. Resize a workbook window. A message box with "you resized a window" will be displayed.   

### How to Turn Off the Event Handler

If you close the workbook that contains the above project, the application-level event handler is turned off. To programmatically turn off the event handler, do the following steps:
