...
You have an Excel workbook that's stored in a SharePoint Online or SharePoint Server document library. When you open the workbook in the Excel desktop app and [view the file properties](https://support.microsoft.com/office/view-or-change-the-properties-for-an-office-file-21d604c2-481e-4379-8e54-1dd4622c6b75), it shows the old property values instead of the values that are stored in SharePoint. In this case, when Excel performs a **Save** or **AutoSave** operation, the old properties that are stored in the local cache will overwrite the current properties that are stored in SharePoint.

## Cause

This issue may occur if the Excel workbook already exists in the app local cache of the Excel desktop app. Currently, it's a product limitation.
...
**Note:** Setting this option may cause files to open more slowly and increase network usage, because Office applications always download files completely each time they're opened.  
...
**Note:** You must have the SharePoint administrator permission to perform the following step.
...
> [!IMPORTANT]
> If the `ParserDisabled` property is later set to **$false**, there may be a significant side effect where the old property values stored in the file may overwrite the property values of all documents stored in the document library on SharePoint.
