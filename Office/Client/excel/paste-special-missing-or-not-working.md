...
Ensure that all instances of the web browsers that you use are closed and try to use the **Paste Special** option again. Paste special will not function if your web browser is causing the conflict. Windows Internet Explorer (8-9) don't conflict with the **Paste Special** option in Excel 2010. Third party Add-ins can cause a variety of issues, one of then being, conflicting with the **Paste Special** option. To determine if an add-in is causing the issue:
...
4. If the **Paste Special** option works in the Safe Mode as follows, enable your Add-ins one by one until you hit that one that's causing the conflict. Leave that one disabled or uninstall it.
