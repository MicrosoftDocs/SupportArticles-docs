...
description: Resolve common errors that you might experience when you try to add a file upload question to a form or upload a file.
author: Cloud-Writer
ms.reviewer: zakirh, v-kccross
...
Microsoft Forms stores uploaded files in either the form owner's OneDrive for work or school or a Microsoft 365 group's SharePoint site. File upload functionality can fail if SharePoint or OneDrive settings prevent Forms from creating folders, sharing uploaded files, or accessing the storage location. This article describes common file upload error messages, explains the underlying SharePoint and OneDrive configuration issues that cause them, and provides steps to resolve problems related to sharing settings, information barriers, expired sharing links, site permissions, managed device restrictions, library settings, and folder creation restrictions.

## Symptoms

Files that you upload through Microsoft Forms are stored in Microsoft SharePoint or in OneDrive for work or school, as follows:

- Files that you upload to group forms are stored in the document library on the Microsoft 365 group's SharePoint site.
- Files that you upload to individual forms are stored in the form author's personal OneDrive folder.
...
1. Enable the *People in your organization* sharing links if they're disabled.
    ...
    You must be a SharePoint Online administrator and a site collection administrator to run the following PowerShell commands in [SharePoint Online Management Shell](/powershell/sharepoint/sharepoint-online/connect-sharepoint-online).
...
- If the *People in your organization* sharing links are disabled, enable the links by using the [Set-SPOSite](/powershell/module/sharepoint-online/set-sposite?view=sharepoint-ps&preserve-view=true) PowerShell cmdlet in [SharePoint Online Management Shell](/powershell/sharepoint/sharepoint-online/connect-sharepoint-online).

  You must be a SharePoint Online administrator and a site collection administrator to run the following PowerShell commands.
  ...
  - For a group form, run the following PowerShell cmdlet:
...
- **If the *People in your organization* sharing links are expired**
  ...
  Delete the file upload questions, and then re-create them. The new files are uploaded to a new folder, and the previously uploaded files remain in the original folder.
  ...
  As long as the expiration timeframe remains configured, the links expire after the number of days specified in the policy, and the file upload questions need to be re-created.
...
- **If an individual form is moved to a Microsoft 365 group**
  ...
  After you re-create the file upload questions, the new questions are linked to the group's SharePoint site, and all files are uploaded to the SharePoint site.

## Error: Access denied. You don't have permissions to perform this action or access this resource
...
1. Select the **Settings** icon in the upper-right corner of the screen.
1. Select **OneDrive settings** > **More settings**.
1. Under **Can't find what you're looking for?**, select **Return to the old Site settings page**.
...
This issue occurs if the **New Folder** command is disabled in the site settings. This situation prevents you from creating folders in the document library where the form is stored.
