...
1. Start Registry Editor. To do this, press the Windows key+R to open the Run window, type *regedit*, and then press Enter.
...
1. For each subkey under **Providers**, follow these steps:
   ...
   1. Select the subkey, and check the value data for its `UniqueId` entry.
   1. If the value of `UniqueId` is one of the following values, delete the subkey:
     ...
     For example, the subkey under `HKEY_CURRENT_USER\Software\Microsoft\Office\16.0\WEF\Providers` is `KpwDSnL9jumf9ZJTx_XF_Q==`, and its `UniqueId` value is `3a975b5d-ad3e-4e3d-84b0-a7e6776ba6a6_ADAL`. In this case, delete the `KpwDSnL9jumf9ZJTx_XF_Q==` subkey.
