...
Resave the Web Page from Visio with the output format set to something other than XAML, like VML. Specifically, when choosing Web Page (*.htm;*.html) click on the Publish... button in the Save As dialog, click on the Advanced tab and choose something other than XAML (Extensible Application Markup Language). For example, this won't occur when you choose VML (Vector Markup Language), the default in Visio 2007. The drawback to this workaround is the web page will not be using Silverlight, if you have Silverlight installed on the client machine opening the web page.

Chosing XAML as the output format requires a Microsoft Silverlight browser plug-in or a browser capable of displaying XAML. If you choose another output format when publishing the Visio Drawing you will not benefit from Silverlight rendering of the Visio Web Page.
