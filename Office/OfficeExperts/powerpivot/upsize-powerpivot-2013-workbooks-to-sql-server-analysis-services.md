...
Assume that you're using PowerPivot as a data source, so no UI is present in the workbook that's hosting the PowerPivot data model. The data displays from another workbook that connects to the PowerPivot workbook as a data source or from some other reporting tool such as Power View or Excel Services.

## What's upsizing PowerPivot? (Also known as convert PowerPivot to Tabular model, also known as convert PowerPivot to SSAS)

Upsizing PowerPivot is moving the solution from a SharePoint centric storage mode to an SSAS centric storage mode.  The result is that the PowerPivot model and data aren't stored in SharePoint.
...
- SharePoint isn't optimized to host large files and it has a hard limit of 2 gigabytes (GB) for a single file.
- You'll find that SharePoint features may fail or behave incorrectly when you use large files. It's also a burden on the SharePoint system that could adversely affect other users.
...
- If users are experiencing intermittent poor performance, it might be because the cache time has expired and the back-end SSAS database has to be rebuilt.
- In SharePoint 2013, the default behavior of Excel Services data \ "Refresh all connections" is to have the PowerPivot data completely reloaded itself from source data.  For large workbooks, it could cause poor user experience.

**Have a PowerPivot workbook larger than 2 GB**

The only option to have a PowerPoint workbook larger than 2 GB is to upsize or only use it in the 64-bit of Excel. SharePoint only supports 2GB files or smaller.
...
1. SSAS doesn't use the SharePoint security system. If you want to restrict who can access the workbook data, you need configure it on the SSAS database.
   - It consists of assigning Windows users or groups' access to the database.
   - You can do so by using **SQL Server Data Tools**.
1. If you don't use the workbook as a data source, you need create a separate workbook to view the data. It would be in cases where both the data and the pivot tables that view the data are stored in the same workbook.
...
1. Have your administrator set up a tabular instance of SSAS in your network.
   ...
   1. Have the SSAS service run under a domain account. It needs to access the file share that's made in the step 2 below.
   1. Install [SQL Server 2012 Service Pack 1](https://www.microsoft.com/en-us/download/details.aspx?id=35575) if you're using Excel 2013 workbook as the data source.
...
1. Enter a project name. This project holds the model and build the database on the SSAS server.
...
1. Select **Yes** in the following warning window. It just asks that you trust where the PowerPivot workbook is getting data from and informs you that the data won't be imported. It's fine because the server fetches data from the data sources that's defined in the PowerPivot model. If you have data in the model that's from a linked sheet that needs to be copied manually into this solution through the **Past Append** function.
...
1. Set up an automated processing schedule by using the **SQL Server Management Studio** tool because the new SSAS database isn't getting refreshed with the latest data and it's a static snapshot of the data at the time you deployed it to the server.  
   ...
   1. Create a new SQL Server Agent job, as follows:
      ...
      **Server**: Name of your SSAS Tabular server
