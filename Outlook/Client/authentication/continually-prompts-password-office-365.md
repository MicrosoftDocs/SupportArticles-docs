...
You may be an Office 365 customer who is experiencing the error that is described in the "Symptoms" section when you try to connect to an Exchange Online mailbox. You may also already be running newer versions of Outlook 2013 or Outlook 2016. If both conditions are true, you have access to diagnostics that can be run to troubleshoot issues in which Outlook continually prompts for a password. To do this, select **Run Tests: Outlook keeps asking for my password** to launch a diagnostic. The diagnostic does automated checks and returns possible solutions for you to use to try to fix any detected issues.
