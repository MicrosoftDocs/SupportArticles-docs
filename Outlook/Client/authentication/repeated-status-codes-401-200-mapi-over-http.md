...
The [HTTP sessions authentication](https://datatracker.ietf.org/doc/html/rfc4559) Request for Comment (RFC) describes the expected protocol sequence. This sequence includes sending an empty authentication request so that the server responds by using the authentication protocols that it supports. It lets the client choose the appropriate authentication type. The repeated "401" and "200" status codes are expected as part of this process.

## Workaround

You can disable the automatic proxy setting to reduce the number of HTTP "401" responses. To do so, change or add the following registry value:
