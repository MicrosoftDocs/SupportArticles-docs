...
## Solution: Delete corrupted rules using MFCMAPI
...
1. Select the **Force the use of Outlook's MAPI files** check box, and then save the change.
1. On the **Session** menu, select **Logon**. If you receive an error, select **OK** and the public folder store will load.
...
1. To delete the corrupted rule, right-click the message, and then select **Delete Message**.
1. After the corrupted rule is deleted, try to access the Folder Assistant again in Outlook.

If there are multiple public folders that contain corrupted rules, repeat the process for each of them to locate and delete the corrupted rule.  
