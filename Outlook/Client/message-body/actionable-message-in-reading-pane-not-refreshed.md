...
2. The recipient performs an action on the actionable message in the pop-out window that causes the message to update. For example, a recipient selects an **Approve** button in the message, and the following text appears next to the button shortly afterward: "Your request is approved".
...
Outside the specific combinations that are described in the "More information" section, the reading pane might not refresh.
