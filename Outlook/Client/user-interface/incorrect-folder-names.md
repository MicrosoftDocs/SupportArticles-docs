...
# Issue with folder names or locations in Outlook
...
- Folder names appear in a language other than the default language.
    ...
    This issue occurs if a mobile device or a third-party server application synchronizes with the Microsoft Exchange Server mailbox. To resolve this issue, try Method 1 first. If that doesn't work, try Method 2 or Method 3.

- Folder names are switched. For example, the **Inbox** folder and the **Outbox** folder names are switched.  

  This issue might occur after a maintenance cycle for the Exchange Server mailbox. To resolve this issue, try Method 1 first. If that doesn't work, try Method 2 or Method 3.
...
1. Exit Outlook.
2. Log on to [Outlook on the web](https://support.office.com/article/Sign-in-to-Outlook-Web-App-e08eb8ac-ac27-49f4-a400-a47311e1ee7e) by using your credentials.
...
5. In the **Language** drop-down list, change the language to your preferred language, and then select the **Rename default folders so their names match the specified language** check box.

    :::image type="content" source="media/incorrect-folder-names/owa-settings.png" alt-text="Screenshot of the OWA Settings page in which you need to change language, date and time format.":::

6. Select date and time format that you want to use, and then select **Save**.
...
8. Restart Outlook and check whether the folder names display in the default language.

If you use an older version of Outlook on the web, use the following steps:

1. Exit Outlook.
2. Log on to [Outlook on the web](https://support.office.com/article/Sign-in-to-Outlook-Web-App-e08eb8ac-ac27-49f4-a400-a47311e1ee7e) by using your credentials.
3. Select **Settings** (:::image type="icon" source="media/incorrect-folder-names/settings-icon.png":::) > **Options**.
4. Under **Options**, select **General** > **Region and time zone**.
5. In the **Language** drop-down list, change the language to your preferred language, and then select the **Rename default folders so their names match the specified language** check box.

    :::image type="content" source="media/incorrect-folder-names/old-owa-settings.png" alt-text="Screenshot of the old OWA Settings page in which you need to change language, date and time format.":::
...
1. Right-click the mailbox name at the top of the folder hierarchy in Outlook, select **New Folder**, and then create a folder named *Archive01*.
...
1. Start MFCMAPI in Online mode.

   To check whether that MFCMAPI is in Online mode, select **Tools** > **Options**, and then make sure that the **Use the MDB_ONLINE flag** and **Use the MAPI_NO_CACHE** check boxes are selected.

1. Select **Session** > **Logon**, and select the mailbox.  
1. Expand **Root Container**, and then expand **Top of Information Store**.

    If the user mailbox is set to a language other than English, the name of this folder will be in the default language.
...
1. Sort the entries based on the **Tag** column and search for the **0x35FF0102** property tag. Double-click this property.
...
1. Delete the old archive folder, as the *Archive01* folder is now the default archive folder.
...
1. Right-click the name of your account in Outlook, select **New Folder**, and then create a folder that's named *New Junk* as an example. This will create a new Junk email folder at the same level as the **Inbox** folder.
...
1. Make sure that the **MDB_ONLINE** and **MAPI_NO_CACHE** check boxes are checked, and then select **OK**.
