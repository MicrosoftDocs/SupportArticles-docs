---
title: Enable logging in Outlook for Mac
...
ms.date: 01/30/2024
---
# Enable logging in Outlook for Mac
...
Restart Outlook and when prompted to turn off logging, select **Turn Logging Off**. 
...
> [!NOTE]
> Turn off logging after you complete your troubleshooting and reproduce the issue. If logging is not turned off, the log files size will continue to increase. If you must keep logging enabled for several hours or several days to capture the issue, make sure that the hard disk has sufficient free space.
...
When you collect and send the log file, Microsoft Support uses it to identify issues. If you are a messaging server administrator, you can use the information in the log file to diagnose issues when you troubleshoot.
...
> [!NOTE]
> The log file may also contain user information. This includes user name, sender and receiver email addresses, and the contents of the user's email messages, notes, tasks, calendar, and contacts. Microsoft doesn't use this information to contact users without their consent. If you're concerned that the data file contains sensitive or confidential information, review the contents of the data file by using text-editing software and remove the information from the file before you send the data file to support professionals. Authentication information is not included in the log files.

## EWS SyncState message size

Beginning with the Outlook for Mac 2011 14.3.4 update, the size of the SyncState binary large object (BLOB) of EWS messages increased from 6 kilobytes (KB) to 32 KB and can be increased up to no more than 300 KB. If you're troubleshooting an issue in which you have to view more of this data, you must increase this value. 

To increase the default value of the SyncState binary large object of EWS messages, use the following steps:

1. Exit Outlook.
2. Open **Terminal**.
3. To set the SyncState binary large object of EWS messages to a specific size, type the following command in the **Terminal** window:
