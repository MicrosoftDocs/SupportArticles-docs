...
| **Locked Site**| Validates that the user's OneDrive or SharePoint site exists, and there are no known issues accessing the site due to lock or archived state. | [Run Tests: Locked Site](https://aka.ms/PillarLockedSite) | [SharePoint or OneDrive read-only error messages](https://docs.microsoft.com/sharepoint/troubleshoot/sites/site-is-read-only)
