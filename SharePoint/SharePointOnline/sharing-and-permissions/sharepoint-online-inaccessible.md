...
- In some instances , you may receive a message that states, **“You are now in line to access this site”**. This page replaces 503-Service unavailable error message on SharePoint pages. The waiting room page itself, has built-in retry mechanisms that will automatically confirm whether the page that you are attempting to access is available to users or not. The user does not need to refresh the page as the page handles that automatically. **Manually refreshing the page may further delay the page load, as it will proceed to do the verification after each refresh.** The page also serves to acknowledge that an issue has been identified and work to resolve the issue is in progress, and users should either wait for the page to load or try again later.
