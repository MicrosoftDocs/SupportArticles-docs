...
- The Document Information Panel pauses while loading

## Workaround

To resolve this issue, try one of the following workarounds:

- Update the group logo from Outlook Web UI:
    1. Go to Outlook web. 
...
1. To make changes to the logo, title, description, and other settings, you must have owner or designer permissions on the SharePoint site. For more information, see Manage site permissions.
2. Make sure the O365 group property "HiddenFromAddressListsEnabled" is set to True. Exchange or Global administrators should use Set-UnifiedGroup to set HiddenFromAddressListsEnabled to False.
3. Make sure that Address Book Policies (ABP) are set on the group mailbox. Check with your Exchange admins if Removing an address book policy for a group mailbox is an option. 
4. If the O365 group is deleted but the SharePoint site still exists, the Document Information Panel might show a spinning icon. If so, restore the O365 group from the recycle bin if it was deleted within the past 30 days. 
