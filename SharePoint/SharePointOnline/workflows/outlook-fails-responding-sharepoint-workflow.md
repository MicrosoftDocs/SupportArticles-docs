...
- You created a **List Workflow** in SharePoint Designer 2013 by using the SharePoint 2010 Workflow Platform Type.
- The workflow contains at least one **Start Approval Process** action.
- As expected, each workflow instance sends a notification email message to the approvers. Each approver opens the notification email message in Outlook 2013, and then selects **Open this Task**.
- In the **Approval Task** dialog box, the approver selects the **Approve** button.

In this scenario, Outlook 2013 doesn't respond for about 30 seconds, and then becomes responsive again.

## Solution/Workaround

To work around this issue, perform one of the following actions, as appropriate:

- In the approval notification email message, add a hyperlink to the task. Instruct users to select the hyperlink in the message and then approve the item by using the web browser within SharePoint Online instead of approving the item by using Outlook 2013.
...
It's a known issue in SharePoint Online. This issue occurs because each SharePoint 2010 Workflow is compiled before it's executed. The more complex the SharePoint Designer 2010 workflow is, the longer this compilation requires to finish (up to about 30 seconds).
