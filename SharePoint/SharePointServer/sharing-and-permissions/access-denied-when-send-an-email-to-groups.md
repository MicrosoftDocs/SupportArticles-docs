...
SharePoint 2013 Workflows that attempt to send an email to a SharePoint group can fail with an Access Denied error if the membership of the SharePoint group isn't set to Everyone.  

## Cause  

This issue occurs when the user starting the workflow is not part of the SharePoint group. The following entries are logged ULS and in the Workflow Status page:
...
- Explicitly add the users to the To or CC line if the membership visibility can't be changed for the SharePoint group.
