...
The `-listolddatabases` command lists the content databases that weren't synchronized since 'n' days while the 'deleteolddatabases' command performs the same operation as the listolddatabases parameter, except it deletes old records corresponding to these databases. It doesn't delete the databases themselves. Once the synchronization references are deleted and a new profile synchronization occurs, new references are stored in the database.  
