...
When a user's Microsoft 365 sign-in address (also known as the User Principal Name, UPN, or user ID) is changed, the Skype for Business Online (formerly Lync Online) SIP address for the user is automatically synchronized. Previously, when a user's Microsoft 365 sign-in address was changed, the IT administrator had to update the user's SIP address separately to match the new Microsoft 365 sign-in address. Otherwise, the two values would be mismatched. A mismatch between a user ID and the user's SIP address may cause confusion for the Skype for Business Online user during sign in.
...
Some companies may want a configuration that uses disjointed sign-in addresses for Skype for Business Online and Microsoft 365 and although this configuration is supported, the end-user experience on initial sign-in will be less than ideal for some Lync clients. Skype for Business Online users will have to enter the UPN and SIP address separately in the user interface for a successful sign-in. Running the Microsoft 365 Desktop Setup Tool will resolve this issue on computers that are running Lync 2010.

> [!NOTE]
> The Microsoft 365 Desktop Setup Tool isn't available for mobile devices or Lync 2013.

If you want to manually configure the setting for Lync 2013, see the following Microsoft 365 website:
