...
When you try to sign in to Skype for Business Online (formerly Lync Online) from a particular computer, you receive one of the following error messages:
...
However, you can sign in from another computer that's running the Lync client.
...
This issue may occur if the time is set incorrectly on the computer on which you couldn't sign in. For example, this issue may occur if the time is skewed.
