...
The following Lync Server Edge Server roles won't be installed after the Setup Lync Server components wizard completes the installation process:
...
- Doesn't have an FQDN or primary DNS suffix that matches the FQDN that was used to define the single server Lync Server Edge server pool   
- The FQDN or primary DNS suffix of one or more of the Windows server computers doesn't match the FQDN information that was used to define the multiple server Lync Server Edge server pool    
...
2. Select Start and open Control Panel.
3. Select the System and Security link to open it.
4. Select the System link to open it.
5. Select the Change System Settings link to access the System Properties dialog.
...
7. If the primary DNS suffix needs to be updated, select the change button and then the More button. If the primary DNS suffix doesn't need to be updated then skip this step and move to step 8
  
   1. Enter the correct FQDN for the primary DNS suffix.
   2. Select the OK button to close the DNS Suffix and NetBios Computer Name dialog.

8. If the Windows server computer's host name value needs to be updated, select the Change button,
   1. Enter the correct host name for the Windows server computer.
   2. Select the OK button to close the Computer Name/Domain Changes dialog.
9. Select the OK button to close the System properties dialog.
10. You'll be prompted to restart the Windows server computer before the name changes take effect.
