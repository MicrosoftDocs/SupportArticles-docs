...
> [!NOTE]
> In a [hosted model](/microsoftteams/direct-routing-sbc-multiple-tenants), SIP options should be sent from only the hosted SBC. The status of SBCs in a derived trunk model are based on the main SBC.
...
- This situation might occur if you’re using an older version of TLS. To enforce stricter security, [enable TLS 1.2](/mem/configmgr/core/plan-design/security/enable-tls-1-2).
...
- If you’re using the minimum required TLS, and no problem is affecting your SBC certificate, your issue most likely occurs because the FQDN is misconfigured in your SIP profile, and it isn’t recognized as belonging to the tenant. To resolve this issue, narrow down which configuration is causing the issue. The most likely culprits are the following conditions:
  ...
  - The domain isn’t yet [fully validated](/microsoft-365/admin/setup/add-domain). If you add an FQDN that wasn’t previously validated, you must validate it now.
