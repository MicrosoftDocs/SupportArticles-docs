...
When using Direct Routing, you might experience the following Session Border Controller (SBC) connectivity issues:
...
- A TLS certificate has problems
...
5. Also, the SIP Proxy sends SIP options to the SBC FQDN that is listed in the Contact header of the SBC’s SIP options.
...
If you’re using the minimum required version of TLS and your SBC certificate is valid, then the issue might be occurring because the FQDN is misconfigured in your SIP profile, and isn’t recognized as belonging to any tenant. Check for the following and fix any errors that you find.
...
<summary><b>“200 OK” response received from SIP Proxy but not SIP options</b></summary>

The SBC receives the “200 OK” response from the SIP Proxy but not the SIP options that were sent. If this error occurs, make sure that the FQDN listed in the Record-Route or Contact header is correct and it resolves to the correct  IP address.
...
This issue might occur if the SBC is configured to send SIP options not to FQDNs but to the specific IP addresses they resolve to. During maintenance or outages, these IP addresses might change to a different datacenter, so the SBC will be sending SIP options to an inactive or unresponsive datacenter. Do the following:

- Make sure that the SBC is discoverable and configured to send SIP options only to FQDNs.
- Make sure that all the devices in the route such as SBCs and firewalls are configured to allow communication to and from all Microsoft-signaling FQDNs.
...
<details>
<summary><b>FQDN doesn’t match contents of CN or SAN certificates</b></summary>

This issue will happen when a wildcard doesn't match a lower level subdomain. For example the wildcard *.contoso.com would match sbc1.contoso.com, but not customer10.sbc1.contoso.com. You can't have multiple levels of subdomains under a wildcard. So if the FQDN doesn’t match the contents of the Common Name (CN) certificate or Subject Alternate Name (SAN) certificate, request a certificate that matches your domains.
...
To fully activate a domain for a tenant and distribute it over the Microsoft 365 environment, you must assign at least one licensed user to the sub domain used by the SBC. After all the requirements are met, it may take up to 24 hours for the domain to be activated.
...
<details>
<summary><b>SBC's certificate is invalid</b></summary>

If the Health Dashboard for Direct Routing in the Teams admin center indicates that the SBC's certificate has expired or been revoked, then request or renew the certificate from a trusted Certificate Authority (CA). Then install it on the SBC.
...
<details>
<summary><b>SBC certificate or intermediary certificates are missing in SBC’s TLS “Hello” message</b></summary>
...
Sometimes, even if everything looks correct, a closer check of the packet capture might reveal that the TLS certificate is not being provided to the Teams infrastructure.
...
The TLS connection is interrupted or does not get set up even though there are no issues with the certificates or settings on the SBC.

The TLS connection may have been closed by one of the intermediary devices (for example, a firewall or a router) on the path between the SBC and the Microsoft network. Check for any connection issues within your managed network and fix them.
