...
description: An external participant (user) can't join a Teams meeting and receives an "Only people with access to this org can join its meetings" message. This behavior is caused by anonymous join and external access settings.
author: helenclu
ms.author: luche
...
- Enable anonymous join in the [meeting policies](/microsoftteams/settings-policies-reference#meeting-join--lobby) that are assigned to all users in your organization or to those users who are allowed to organize meetings with anonymous participants.
