...
|Front-of-room display shows a black screen intermittently|On devices that're running version 4.17.x.x of the Teams Rooms for Windows app, you might see a black screen with a spinner in the front-of-room display intermittently after the system has been idle for a period of time.</br></br>Both the Microsoft Teams admin center & the Pro Management portal will indicate that the front-of-room display is functioning normally. This is because the display is not disconnected. However, the app window is hidden.|Select the spinner to restore the front-of-room display. If that doesn't fix the issue, then select either the **Meet now** or the **Call** button on the console of the Teams Rooms device to restore the front-of-room display immediately.|
