...
|Unable to start whiteboard when using Teams Rooms on Android devices |When you try to initiate a Whiteboard session from an Android device that's running Teams Rooms, the following error message displays:<br/><br/> **Something went wrong**.|No workaround is available at this time.
