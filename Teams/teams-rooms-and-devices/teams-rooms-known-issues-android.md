...
This article lists the known issues for the Microsoft Teams Rooms app when it's used on Android devices.
...
|Teams on Android devices experience a delay with signing in after a user signs out.|You sign out of a Teams on Android device either manually or from the Microsoft Teams admin center. When you try to sign in again immediately, there is a delay. This is because the authentication code takes 15 minutes or longer to refresh after a device signs out.<br/><br/>This issue occurs on all Teams on Android devices including those that are running version 5.0.6061.0 of the Company portal app.|Restart the affected device to force the code to refresh.|
