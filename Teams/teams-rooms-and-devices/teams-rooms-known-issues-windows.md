...
|The onscreen keyboard on a touch console device is missing or not responsive.|The onscreen keyboard on the touch console in a Teams Rooms for Windows device either doesn't work or doesn't appear.<br/><br/>Because of this issue, you're unable to perform the following actions:<ul><li>Enter text to call another user.</li><li>Add a user to a meeting.</li><li>Enter a username and password to access the Settings menu.</li></ul>|Use the following steps to work around the issue:<ol><li>In the Teams Rooms device console, select **More** > **Settings**.</li><li>Select **Windows Settings**.</li><li>Sign in as an administrator.</li><li>Search for and select the Registry Editor.</li><li>Select the **HKEY_USERS** folder from the list of machine user SIDs.</li><li>Select **File** > **Load Hive**.</li><li>Browse to the C:\Users\Skype folder, type `NTUSER.dat` in the **File name** field, and then select **Open**.</li><li>When prompted for a key name for the newly loaded hive, type `Skype`.<br/>You will see the registry settings for the user named Skype.</li><li>Browse to the **HKEY_USERS\Skype\SOFTWARE\Microsoft\TabletTip\1.7** folder in the registry.</li><li>Right click (or tap and hold) the **1.7** folder, select **New** > **DWORD (32-bit) Value**.</li><li>Type `EnableDesktopModeAutoInvoke` as the name of the registry entry.</li><li>Double-click the **EnableDesktopModeAutoInvoke** registry entry, type `1` in the **Value data** field, and then tap **OK**.>/li><li>Restart the Teams Rooms device.</li></ol>|
