...
|Content shared by video teleconferencing devices doesn't display in Teams Rooms for Windows devices.|The shared content doesn't display in the following scenario:<br/><br/>A video teleconferencing (VTC) device makes a SIP call to a Teams Rooms on Windows device. The Teams Rooms device accepts it as an audio-only call. When the VTC device shares content during the call, the content doesn't display in the Teams Rooms device.<br/><br/>This issue might also occur when the Teams Rooms device accepts the SIP call as a video call.|No workaround is available at this time.|
