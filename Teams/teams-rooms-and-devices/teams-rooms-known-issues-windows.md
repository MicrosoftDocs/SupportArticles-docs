...
|QR code to join meetings reappears after being disabled.|You have disabled the QR code that is displayed on the device
console and in Front of Room displays to join Teams meetings.<br/><br/>However, the QR code reappears when the device restarts, or after a user either joins or leaves a meeting on the device.|No workaround is available at this time.|
