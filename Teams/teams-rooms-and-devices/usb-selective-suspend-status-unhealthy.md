...
description: Resolve the issue that causes the USB Selective Suspend signal of a Microsoft Teams Rooms device to appear as Unhealthy.
...
In the [Microsoft Teams Rooms Pro Management portal](https://portal.rooms.microsoft.com/), the **USB Selective Suspend** signal of a Microsoft Teams Rooms device is shown as **Unhealthy**, and the incident severity value is shown as **Warning**. And users may experience the following issue:
...
Each new Teams Rooms device includes a Windows configuration to define the power plan and how the connected USB devices behave, depending on the power supply (AC or DC). By default, USB selective suspend is disabled, but sometimes this setting may be changed. In this case, the Teams Rooms Pro agent detects and reports the status of **USB Selective Suspend** as **Unhealthy**.
...
1. Select the **Start** icon, type *edit power plan* in the search box, and then select **Edit power plan** from the result.
