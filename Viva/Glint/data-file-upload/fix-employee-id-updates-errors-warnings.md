...
When you [update employee IDs](/viva/glint/setup/id-update#update-employee-ids-in-bulk) by uploading a file to Microsoft Viva Glint, you may receive one of the following errors or warning messages. Select the message that you receive from the list at the top of the article and follow the appropriate resolution.
...
This error occurs when the uploaded file contains extra columns in addition to the required columns.
...
This error occurs when one of the following required columns is missing from the uploaded file:
...
To fix the issue, add the missing column and its data to the file. Then, upload the file again to Viva Glint.
...
1. Open the employee attribute data file. Use the appropriate method depending on the file type:
    ...
    - If the data file has a *.xls* or a *.xlsx* extension, open it in Microsoft Excel.
...
This error occurs when all employee IDs in the uploaded file are identical to values already in Viva Glint.
...
This error occurs when an employee record in the uploaded file is missing the employee ID value.
...
1. Open the employee attribute data file. Use the appropriate method depending on the file type:
    ...
    - If the data file has a *.xls* or a *.xlsx* extension, open it in Microsoft Excel.
    - If the data file has a *.csv* extension, use the [Text Import Wizard](https://support.microsoft.com/office/text-import-wizard-c5b02af6-fda1-4440-899f-f78bafe41857) to import the data into Excel by preserving the data in the original format.
1. Locate the row that's listed in the error message, enter the correct value in the **Employee ID** field.

    **Note**: The header row isn't included in the row count. For example, if the error indicates line 20, go to row 21.
1. Save the file and then upload it again to Viva Glint.
...
This error occurs when the uploaded file contains email addresses that aren't associated with any user in Viva Glint.
...
This warning occurs when a user's first and/or last name doesn't match the values in Viva Glint. This warning doesn't cause the employee ID update to fail.
...
This error occurs when multiple users have the same employee ID in the uploaded file.
...
This issue occurs when an employee ID in the uploaded file is already assigned to a different user in Viva Glint.
...
1. Open the employee attribute data file. Use the appropriate method depending on the file type:
    ...
    - If the data file has a *.xls* or a *.xlsx* extension, open it in Microsoft Excel.
    - If the data file has a *.csv* extension, use the [Text Import Wizard](https://support.microsoft.com/office/text-import-wizard-c5b02af6-fda1-4440-899f-f78bafe41857) to import the data into Excel by preserving the data in the original format.
1. Locate the row that's listed in the error message and enter a unique employee ID that's not associated with any other users in Viva Glint.
