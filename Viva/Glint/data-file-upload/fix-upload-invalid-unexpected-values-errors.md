---
title: Resolve file upload errors related to invalid or unexpected values
description: Fix errors that occur when you upload employee attribute data to Microsoft Viva Glint. These errors are related to invalid or unexpected values.
...
# Resolve file upload errors related to invalid or unexpected values
...
1. Locate the row that’s listed in the error message.

    **Note**: The header row isn’t included in the row count. For example, if the error says line 20, go to row 21.
1. Review the value in the column that’s listed in the error message.
...
- The **Status** attribute isn’t saved correctly in Viva Glint and is listed in the error message.
...
- If the **Status** attribute is listed in the error message, follow these steps in Viva Glint:
