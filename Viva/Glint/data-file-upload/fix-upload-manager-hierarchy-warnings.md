...
This issue occurs if employees other than the CEO or top-level person aren't assigned a **Manager ID** value. Only the CEO or top-level person should have a blank Manager ID.
...
1. Fill in their Manager ID values.

   **Note**: If there are employees whose manager role needs to be filled, use the Manager ID of their skip-level manager.
...
This issue occurs if the uploaded file contains employees who report to a Manager ID that doesn't have a corresponding employee record.
