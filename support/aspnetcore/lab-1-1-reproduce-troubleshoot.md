...
- Part 1: You will reproduce the crash issue, check Nginx and system logs to search for the crash symptoms and indicators, and then troubleshoot by generating a crash dump file. Finally, you will gather the system-generated core dump file from the crash report that's generated by the Ubuntu manager, apport.

- [Part 2](lab-1-2-analyze-core-dumps-lldb-debugger.md): You will install and configure lldb to work together with a .NET Core debugger extension named SOS. You will also analyze the dump file in lldb.

## Reproducing a crash problem

When you browse to the site URL, `http://buggyamb/`, and select the **Problem Pages** link, you will see links to some problem scenarios. There are three different crash scenarios. However, for this lab, you will troubleshoot only the third crash scenario.
...
Before you reproduce the crash problem, note the process ID of the buggy application. You will use this information to verify that your application restarts. Run the `systemctl status buggyamb.service` command to get the current PID. In the following results, the PID of the process that's running the service is 2255.

:::image type="content" source="./media/lab-1-1-reproduce-troubleshoot/service.png" alt-text="BuggyAmb service" border="true":::

Select the **Crash 3** link. The page loads and displays a message that resembles the following message.

:::image type="content" source="./media/lab-1-1-reproduce-troubleshoot/crash3.png" alt-text="BuggyAmb crash3" border="true":::

This message asks the user to consider the following question: Will this page cause the process to crash? Run the same `systemctl status buggyamb.service` command, and you should see the same PID. This indicates that a crash did not occur.
...
Even if you select any other link on the webpage, you will experience the same error for a short while. After 10–15 seconds, everything will start to respond as expected again.
...
- Select **Crash 3**. The page loads correctly but returns a confusing message that suggests that the process will crash.
- Select **Slow**. You receive an "HTTP 502 - Bad Gateway" error message instead of the product table.
- After the problem starts, none of the pages render for the next 10–15 seconds.
- After 10–15 seconds, the application starts responding correctly.

The "HTTP 502 - Bad Gateway" error message by itself does not tell you much. However, it should provide a first clue: This is a proxy error, and it can occur if a proxy can't communicate with the application that's running behind the proxy. In the proposed setup, Nginx is working as a reverse proxy to the ASP.NET Core application. Therefore, this error from Nginx indicates that it was not able to reach the back-end application when it forwarded incoming requests.

## Verifying that Nginx works correctly

Before you continue, you might want to check whether Nginx is working correctly. This step is not mandatory because you know that the application is crashing. However, you can still verify the status of Nginx by checking the Nginx logs. You practiced similar troubleshooting steps earlier in the ["Installing and configuring Nginx"](2-2-install-nginx-configure-it-reverse-proxy.md) section.
...
Access logs are just like IIS log files. Open and examine them, just as you did in the previous section. The logs do not show any new information other than the "HTTP 502" response status code that you already encountered during the navigation attempts on the site's pages.
...
This log clearly indicates that what you see is not a Nginx problem.

## Checking system logs - the journalctl command
...
To see the details, scroll down and to the right by using the arrow keys. You will find a `System.Net.WebException` exception.
...
If you closely examine the logs, you will see the code file name and the line number on which the problem occurred. For this lab, we'll assume that this information is not available. This is because, in real world scenarios, you might not be able to retrieve this kind of information. Therefore, the objective is to continue by analyzing a crash dump to learn the cause of the crash.

## Getting a core dump file after a crash
...
However, this is not the core dump file. This is a report file that contains several pieces of information together with the dump file. You have to unpack this file to get the core dump file.
...
Extracting the auto-generated core dump file can be a cumbersome process. In the next lab, you'll see that it's easier to capture core dump files by using `createdump`. Continue to [part 2](lab-1-2-analyze-core-dumps-lldb-debugger.md) of this lab to see how to open this dump file in the lldb debugger to do a quick analysis.

## Next steps

[Lab 1.2 Opening and analyzing system-generated core dump files in lldb debugger](lab-1-2-analyze-core-dumps-lldb-debugger.md)
