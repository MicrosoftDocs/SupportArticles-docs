...
In [part 1](lab-1-1-reproduce-troubleshoot.md), you were instructed how to unpack the core dump file from the apport report. Now it's time to download the symbol files. Symbol are mappings between the source code and the binaries. Symbols have several purposes. Specific to this training, they help you during debugging to view the correct method names and local variables when displaying stacks.
...
What is SOS? According to the [official documentation](https://github.com/dotnet/diagnostics/blob/main/documentation/sos.md), SOS is a debugger extension that lets developers inspect the managed state of a .NET Core and desktop runtime process. SOS can be loaded by WinDbg/cdb debuggers on Windows and lldb on Linux and MacOS.
...
Remember the **resource unavailable** message that was generated by the command. You can already see that the type of exception and the function names are not resolved. and indicated by their value of **unknown**. The address of the exception is also displayed, and you can use this address as a parameter in the `pe` command to see more details. Now, run `pe 00007F8244048538` (replacing this address with the address that's displayed in the dump file). Notice, however, that this won't help your investigation much because the function names are still listed as **unknown**.
