...
There are two processes displayed in the listing above, but one of them is shown as an "elevated process" and its path cannot be determined by `dotnet-dump` ps command. To find its path, we can look at the process' command line information by using `cat /proc/<PID>/cmdline` command by replacing the `<PID>` with the actual process ID of the target process, as seen in the sample output below.

> [!NOTE]
> You may remember that special */proc/* directory from previous sections. As explained earlier, a process in Linux can be seen as another folder under that special */proc/* directory (with the name of process ID of the process) and you can find all the details about a process by examining the */proc/\<PID>/* directory structure.
...
As seen in the in the screenshot above, output of the first command (`cat /proc/6164/cmdline`) tells us that it is our first demo ASP.NET Core application, and the output of the second command (`cat /proc/15586/cmdline`) tells us that it is our buggy application.
...
The idea behind this tool is that you do not need a full process dump to be able to complete an investigation in many scenarios in which you mainly want to look at the managed heap. So, why not just capture the heap information and generate a report about it? Most importantly, those GC dumps that are generated by this tool are portable and can be analyzed on a Windows machine. Since this type of dump files just contains the GC heap information only, you cannot open them in lldb to investigate the threads or threads' call stacks, but, you can open them in PerfView or in Visual Studio.
...
If you have to look at only the managed heap, then this could be the fastest option for you. Just remember to exercise caution when you use `dotnet-gcdump` in production because as discussed earlier it will trigger a full GC in your process, which may cause long pauses. So, use this tool in production carefully when it is needed when you are troubleshooting a memory problem.
