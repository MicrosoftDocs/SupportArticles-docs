...
A fix for this issue has been introduced in Azure AD Connect version 2.2.1.0. If the sync service (ADSync) cannot be started you need to apply the [Mitigation](#mitigation) steps above before upgrading Azure AD Connect.
Please install the lastest Azure AD Connect build available at [Azure AD Connect: Version release history](/azure/active-directory/hybrid/reference-connect-version-history) to prevent SQLLocalDB's Model database corruption issues.
