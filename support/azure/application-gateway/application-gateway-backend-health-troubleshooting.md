...
Users can also create [custom probes](https://learn.microsoft.com/en-us/azure/application-gateway/application-gateway-probe-overview#custom-health-probe) to mention the host name, the path to be probed, and the status codes to be accepted as Healthy.
...
1. After you figure out the time taken for the application to respond, select the **Health Probes** tab, then select the probe associated with your HTTP settings.
...
1. If you can't connect on the port from your local machine as well, then:
...
Learn more about [Application Gateway probe matching](/azure/application-gateway/application-gateway-probe-overview#probe-matching).

> [!NOTE]
> For all TLS related error messages, to learn more about SNI behavior and differences between the v1 and v2 SKU, check the [TLS overview](/azure/application-gateway/ssl-overview) page.
...
* By logging into the backend server (Windows):
   ...
   1. Select Win+R or right-click the Start button and select Run.
...
**Solution:** An Intermediate certificate is used to sign the Leaf certificate and is thus needed to complete the chain. Check with your Certificate Authority (CA) for the necessary Intermediate certificate(s) and install them on your backend server. This chain must start with the Leaf Certificate, then the Intermediate certificate(s), and finally, the Root CA certificate. We recommend installing the complete chain on the backend server, including the Root CA certificate. For reference, look at the certificate chain example under [Leaf must be topmost in chain](/azure/application-gateway/application-gateway-backend-health-troubleshooting#leaf-must-be-topmost-in-chain).

> [!NOTE] 
> A self-signed certificate which is NOT a Certificate Authority also results in the same error. This is because application gateway considers such self-signed certificate as "Leaf" certificate and looks for its signing Intermediate certificate. You can follow this article to correctly [generate a self-signed certificate](/azure/application-gateway/self-signed-certificates).
...
* Using a browser: Access the backend server directly (not through Application Gateway) and click on the certificate padlock in the address bar to view the certificate details. 
   ...
   1.	Open that .CRT file.
   1.	Go to the Details tab and click on "Copy to File",
   1.	On Certificate Export Wizard page, click Next,
...
* By logging into the backend server (Windows)
   ...
   1.	In the Certificate properties, select the Details tab and click "Copy to File",
   1.	On Certificate Export Wizard page, click Next,
   1.	Select "Base-64 encoded X.509 (.CER) and click Next,
   1.	Give a new file name and click Next,
   1.	Click Finish to get a .CER file. 
...
:::image type="content" source="./media/application-gateway-backend-health-troubleshooting/appgwunknown.png" alt-text="Screenshot of Application Gateway backend health - Unknown.":::
...
1. The UDR on the Application Gateway subnet is set to the default route (0.0.0.0/0) and the next hop isn't specified as "Internet."
...
1. Default route advertised by the ExpressRoute/VPN connection to the virtual network over BGP (Border Gateway Protocol):
...
- [Application Gateway backend health diagnostics and logging](https://learn.microsoft.com/en-us/azure/application-gateway/application-gateway-backend-health).
- Azure Network Watcher's Connection troubleshoot - Visit the [Connection Troubleshoot](https://learn.microsoft.com/en-us/azure/network-watcher/connection-troubleshoot-manage) documentation article to learn how to use this tool.
