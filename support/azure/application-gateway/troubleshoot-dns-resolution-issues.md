...
Azure Private Endpoint DNS guidance requires that you use the suggested private link-specific DNS zone.

Application Gateway performs DNS resolution for customer-provided FQDNs, such as back ends. It also performs DNS resolution for Key Vault endpoints that are used for listener certificates and other management and control-plane FQDNs that are required for Azure infrastructure operations.
...
To verify and correct DNS resolution for the Key Vault private endpoint, configure DNS for Key Vault private endpoint scenarios by using a private link-specific zone, such as `privatelink.vaultcore.azure.net`.
