...
#customer intent: As a cloud administrator, I want to troubleshoot TCP connection timeout problems in Azure Application Gateway so that client traffic can successfully establish a TCP connection to the gateway frontend.
---
...
A TCP connection timeout occurs when a client can't establish a TCP connection to the Application Gateway frontend. These problems happen before HTTP processing, so the request never reaches the backend application.
...
You see one or more of the following browser or client-side errors:
...
- Client network problems like firewall or proxy configurations that block or inspect outbound traffic.
...
Make sure that outbound traffic to the Application Gateway frontend IP and port is allowed and isn't changed before reaching Azure.
...
- Confirm that NSG rules allow inbound and outbound traffic on the destination port.
...
DNS problems don't result in a TCP connection timeout. Instead, they prevent the TCP connection from being created because the client can't resolve the destination hostname to an IP address.
...
- The listener must be associated with a routing rule that forwards traffic to a backend pool.
