...
description: Troubleshoot issues that can occur when scaling Azure Kubernetes Service (AKS) clusters to 500 nodes or more
...
Pre-requisites: 
1.	To scale beyond 400 nodes, you need to use Azure CNI networking plugin 
2.	Plan your Virtual network and subnets to account for the no. of nodes and pods you're deploying, learn more about [planning IP addresses for your cluster][planning IP addresses for your cluster]. To reduce the overhead of subnet planning or cluster re-creation due to Ip exhaustion, use [Dynamic Ip allocation][Dynamic Ip allocation]
...
3.	Drain pods from the old node pool residing in the old subnet to be replaced.
