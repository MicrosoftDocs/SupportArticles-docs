...
| Enable the [cluster autoscaler](/azure/aks/cluster-autoscaler?tabs=azure-cli) | Cluster Autoscaler looks at pod requests during pod schedule time in order to determine if additional nodes are needed to meet demand. If request values are not set or are too low, autoscaler is not aware that there are not enough resources and pods are simply allowed to run.
