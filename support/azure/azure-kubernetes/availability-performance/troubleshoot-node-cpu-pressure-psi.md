...
This article helps you monitor CPU pressure by using PSI metrics and provides best practices to resolve resource contention problems.
...
To identify and resolve CPU pressure problems, follow these steps:
...
1. Enable Azure Monitoring Managed Prometheus for your AKS cluster by following the instructions in [Enable Prometheus and Grafana](/azure/azure-monitor/containers/kubernetes-monitoring-enable#enable-prometheus-and-grafana).
    ...
    To enable customized scrape metrics for Prometheus, see [Scrape configs](/azure/azure-monitor/containers/prometheus-metrics-scrape-configuration#scrape-configs). Set `minumum ingestion profile` to `false` and `node-exporter` to `true`.

1. Go to the Azure Monitor workspace associated with the AKS cluster from the [Azure portal](https://portal.azure.com).

    :::image type="content" source="media/troubleshoot-node-cpu-pressure-psi/configure-azure-monitor-for-containers.png" alt-text="Screenshot of navigation to the Azure Monitor workspace for AKS CPU pressure metrics." lightbox="media/troubleshoot-node-cpu-pressure-psi/configure-azure-monitor-for-containers.png":::

1. Under **Monitoring**, select **Metrics**.

1. Select **Prometheus metrics** as the data source.

    > [!NOTE]
    > To use the metrics, you need to enable them in Azure Monitoring Managed Prometheus. Node Exporter or cAdvisor exposes these metrics.

1. Query specific PSI metrics in Prometheus explorer:

   - For node-level CPU pressure, use the `node_pressure_cpu_waiting_seconds_total` Prometheus Query Language (PromQL).

      :::image type="content" source="media/troubleshoot-node-cpu-pressure-psi/node-level-cpu-pressure.png" alt-text="Screenshot of a Prometheus query for node-level CPU pressure in an AKS cluster." lightbox="media/troubleshoot-node-cpu-pressure-psi/node-level-cpu-pressure.png":::

   - For pod-level CPU pressure, use the `container_cpu_cfs_throttled_seconds_total` PromQL.

1. Calculate the PSI-some percentage (percentage of time at least one task is stalled on CPU):

   `rate(node_pressure_cpu_waiting_seconds_total[5m]) * 100`

> [!NOTE]
> Some of the container level metrics, like `container_pressure_cpu_waiting_seconds_total` and `container_pressure_cpu_stalled_seconds_total`, aren't available in AKS as they're part of the Kubelet PSI feature gate that is in alpha state. AKS begins supporting the use of the feature when it reaches beta stage.

### [Command Line](#tab/command-line)

Access PSI metrics safely by using kubectl without requiring Secure Shell (SSH) access:
...
1. For more detailed PSI metrics, use the `kubectl debug` feature to create a temporary debug pod:
...
## Key PSI metrics for CPU pressure monitoring
...
- In oversized and multicore nodes, load average often underreports CPU saturation.
