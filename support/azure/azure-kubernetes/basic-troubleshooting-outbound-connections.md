...
- The source and the destination for a request.
- The hops in between the source and the destination.
- The request-response flow.
- The hops that are topped with extra security layers, such as the following layers:
...
Traffic that originates from within the AKS cluster, whether it's from a pod or from a worker node, is considered the outbound traffic from the cluster. Suppose there's an issue in the outbound flow for an AKS cluster. Before you continue to troubleshooting, first understand the nature of the request-response flow.
...
1. Traffic to a device or endpoint in the same virtual network or a different virtual network (using virtual network peering)
...
In this document, we cover basic troubleshooting steps for issues that concern outbound connectivity:

- Within the cluster
- Within the virtual networks
- To the outside world (public traffic)

When you troubleshoot outbound traffic in AKS, it's also important to understand what your egress device is from which the traffic is going through. Here, the egress device could be one of the following components:
...
The flow could also differ based on where the destination is. For example, internal traffic (that is, within the cluster) doesn't go through the egress device. The internal traffic would only use the cluster networking.
...
In some cases, the egress traffic has to be filtered, and may require Azure Firewall, as shown in the following diagram:
...
In place of a firewall, a user might want to place a proxy server. Or, the user might want to have a NAT gateway in place for egress traffic. The basic flow remains same as the diagram.

It's important to understand the nature of egress flow for your cluster so that you can continue troubleshooting, as explained here.

## Troubleshooting checklist

For basic troubleshooting for egress traffic from an AKS cluster, we can take the following steps:
...
1. Run the [kubectl exec](https://kubernetes.io/docs/reference/generated/kubectl/kubectl-commands#exec) command to connect to the pod using PowerShell.
...
You should also check whether the endpoint is reachable from the node. Then, verify the DNS settings in the node. Take the steps in the following procedure:
...
1. Check the *resolv.conf* file to find out whether the expected name servers are added.
...
In one unusual scenario that involves DNS resolution, the DNS queries get a correct response from the node but fail from the pod. For this scenario, you may consider checking [Troubleshoot DNS resolution failures from inside the pod but not from the worker node](troubleshoot-dns-failure-from-pod-but-not-from-worker-node.md).
...
To learn whether you can reach the endpoint through the network from your cluster, follow these steps:

1. Check the route to the endpoint to find out whether there's a time-out at a specific operation.
...
1. Check whether the wanted port is open on the remote host.
