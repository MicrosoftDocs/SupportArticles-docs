...
When you use Microsoft Azure Container Registry together with Azure Kubernetes Service (AKS), you need to establish an authentication mechanism. Set up the AKS to Container Registry integration by using a few simple Azure CLI or Azure PowerShell commands. This integration assigns either the [Container Registry Repository Reader role](/azure/role-based-access-control/built-in-roles#container-registry-repository-reader) (for attribute-based access control (ABAC)-enabled registries) or the [AcrPull role](/azure/role-based-access-control/built-in-roles#acrpull) (for non-ABAC-enabled registries) for the kubelet identity that's associated with the AKS cluster to pull images from a container registry. For more information about the necessary ACR role to assign based on whether your ACR registry is ABAC-enabled, see [Microsoft Entra attribute-based access control (ABAC) for repository permissions](/azure/container-registry/container-registry-rbac-abac-repository-permissions?tabs=azure-portal).
...
This article assumes that you have an existing AKS cluster and an existing container registry. See the following quickstarts:
...
You also need Azure CLI version 2.0.59 or a later version installed and configured. Run [az version](/cli/azure/reference-index#az-version) to determine the version. If you need to install or upgrade, see [Install Azure CLI](/cli/azure/install-azure-cli).
...
Every image pull that kubelet performs goes through three distinct stages. Identifying which stage fails is the fastest way to find the cause:

1. **Authentication and authorization.** The node identity (kubelet managed identity, service principal, or an image pull secret) gets a token that's authorized to pull from the registry. A failure at this stage appears as `401 Unauthorized`, `403 Forbidden`, or `failed to fetch anonymous token`.

1. **Registry (login) endpoint.** kubelet resolves and connects to the registry endpoint to authenticate and read the image manifest. For Azure Container Registry, this endpoint is `<registry>.azurecr.io`; for the Microsoft Artifact Registry, it's `mcr.microsoft.com`. DNS, private endpoint, or firewall problems on this hop appear as `i/o timeout`, `TLS handshake timeout`, or `connection reset`. For a complete list of endpoints supported by ACR, see [Azure Container Registry endpoint](/azure/container-registry/container-registry-endpoint-reference).

1. **Data (storage) endpoint.** kubelet downloads the image layers (blobs) from the registry's *data* endpoint, which is a **separate host** from the authentication endpoint. For Azure Container Registry, the data endpoint is `*.blob.core.windows.net`; for the Microsoft Artifact Registry, it's `*.data.mcr.microsoft.com`. If the authentication endpoint works but the data endpoint is blocked or resolves incorrectly, the manifest read succeeds but the layer download fails.

> [!TIP]
> An unauthenticated request to the registry endpoint (for example, `curl -v https://<registry>.azurecr.io/v2/`) that returns `401 Unauthorized` is *expected* and confirms that the authentication endpoint is reachable. If pulls still fail, focus on the authorization token (stage 1) or the data endpoint (stage 3) rather than the authentication endpoint.
...
| --- | --- | --- |
| `failed to fetch oauth token: ... 401 Unauthorized`, `no basic auth credentials` | Authentication and authorization | [Cause 1](#cause-1-401-unauthorized-error) |
| `no match for platform in manifest`, `exec format error` | Image or node architecture mismatch | [Cause 1b](#cause-1b-401-unauthorized-error-due-to-incompatible-architecture), [Cause 5](#cause-5-no-match-for-platform-in-manifest) |
| `not found` for a repository, tag, or manifest | Image name, tag, or availability | [Cause 2](#cause-2-image-not-found-error) |
| `403 Forbidden` | Network access to the registry (private DNS or IP allowlist) | [Cause 3](#cause-3-403-forbidden-error) |
...
- Solutions [2][cause1-solution2], [3][cause1-solution3], and [5][cause1-solution5] apply only to [AKS clusters that use a service principal](/azure/aks/kubernetes-service-principal).

- Solutions [1][cause1-solution1], [2][cause1-solution2], [3][cause1-solution3], and [4][cause1-solution4] apply to the Azure method of [creating the role assignment at Container Registry level for AKS's identity](/azure/aks/cluster-container-registry-integration).

- Solutions [5][cause1-solution5] and [6][cause1-solution6] apply to the Kubernetes method of [pulling a Kubernetes secret](https://kubernetes.io/docs/tasks/configure-pod-container/pull-image-private-registry/).
...
> [!NOTE]
> Make sure that you create the correct role assignment (`Container Registry Repository Reader` for ABAC-enabled registries, and `AcrPull` for non-ABAC-enabled registries).
> See [Microsoft Entra attribute-based access control (ABAC) for repository permissions](/azure/container-registry/container-registry-rbac-abac-repository-permissions?tabs=azure-portal) for more information on the correct ACR role needed based on whether the registry is ABAC-enabled or not.
...
When you use a managed identity for authentication with the ACR, the managed identity is the kubelet identity. By default, the AKS Virtual Machine Scale Sets assign the kubelet identity. If you remove the kubelet identity from the AKS Virtual Machine Scale Sets, the AKS nodes can't pull images from the ACR.
...
To find the kubelet identity for your AKS cluster, run the following command:
...
If you pull an image by using an [image pull secret](https://kubernetes.io/docs/tasks/configure-pod-container/pull-image-private-registry), and you created that Kubernetes secret by using the values of a service principal, make sure that the associated service principal is correct and the secret is still valid. Follow these steps:
...
> [!NOTE]
> If a **Regenerate** password operation occurs, an operation named `Regenerate Container Registry Login Credentials` displays in the **Activity log** page of the container registry. The **Activity log** has a [90-day retention period](/azure/azure-monitor/essentials/activity-log#retention-period).

### Cause 1b: `401 Unauthorized` error due to incompatible architecture

You might encounter a `401 Unauthorized` error even when the AKS cluster identity is authorized (as described in the [Cause 1a: `401 Unauthorized` error due to incorrect authorization](#cause-1a-401-unauthorized-error-due-to-incorrect-authorization) section). This problem occurs if the container image in the ACR doesn't match the architecture (like ARM64 versus AMD64) of the node that's running the container. For example, deploying an ARM64 image on an AMD64 node or vice versa can result in this error.
...
When diagnosing this problem by using the Azure CLI, you might see an unexpected `exec format` error if your system node pool runs a different architecture than the image in the ACR:
...
- Ensure the container images you push to ACR match the architecture of your AKS nodes (for example, ARM64 or AMD64).
...
If the AKS cluster connects publicly to the container registry (**not** through a private link or an endpoint) and you limit the public network access of the container registry to selected networks, add the AKS load balancer's public IP address to the allowed IP address range of the container registry by using the following steps:

1. Verify that the public network access is limited to selected networks.

    1. In the Azure portal, go to the container registry. 
    1. In **Settings**, select **Networking**. 
    1. On the **Public access** tab, set **Public network access** to **Selected networks** or **Disabled**.

1. Get the AKS load balancer's public IP address by using one of the following methods:

    - Use the Azure portal:
        
        1. In the Azure portal, go to the AKS cluster. 
...
1. Allow access from the AKS load balancer's public IP address by using one of the following methods:
...
The host operating system (node OS) is incompatible with the image that you use for the pod or container. For example, if you schedule a pod to run a Linux container on a Windows node, or a Windows container on a Linux node, the following error occurs:
...
This error occurs for an image that's pulled from any source if the image is incompatible with the host OS. The error isn't limited to images that you pull from the container registry.

### Solution: Configure the nodeSelector field correctly in your pod or deployment

Specify the correct `nodeSelector` field in the configuration settings of your pod or deployment. The correct value for this field's `kubernetes.io/os` setting ensures that the pod is scheduled on the correct type of node. The following table shows how to set the `kubernetes.io/os` setting in YAML:
...
If the troubleshooting guidance in this article doesn't help you resolve the issue, consider the following suggestions:

- Check the network security groups and route tables that are associated with subnets.
