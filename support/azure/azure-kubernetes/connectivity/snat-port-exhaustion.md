...
1. Get the IP addresses of AKS nodes that experience active SNAT port exhaustion from the Azure portal.
    ...
    To do this, navigate to your AKS cluster in the Azure portal, select **Diagnose and Solve problems** > **Connectivity Issues** > **SNAT connection and Port Allocation**. The **SNAT connection and Port Allocation** tab displays the private IP address of the AKS node that experiences SNAT port exhaustion.
     ...
     :::image type="content" source="media/snat-port-exhaustion/diagnose-and-solve-problems.png" alt-text="Screenshot of the 'Diagnose and Solve problems' pane.":::

     :::image type="content" source="media/snat-port-exhaustion/connectivity-issues.png" alt-text="Screenshot of the 'Connectivity Issues' pane.":::
...
2. Connect with SSH to the node that experiences SNAT port exhaustion and trace TCP established connections by using tcptracer:
...
4. Map the IP address with most connections in the previous output to a pod. If it doesn't work, you can continue.

5. Note the `SADDR`/`NETNS` value with most connections in the previous output, and then run the following command to map that to a PID by using [lsns](https://man7.org/linux/man-pages/man8/lsns.8.html). Lsns is a Linux tool used to list namespaces and map a namespace to a PID in the Linux process tree.
...
6. Map the previous PID to a containerd process by using [pstree](https://man7.org/linux/man-pages/man1/pstree.1.html). Pstree is a Linux tool used to list processes in tree format for readability. 
