...
- Check resource health events
  ...
  AKS provides Resource health events for critical component downtime. Before you proceede, make sure that no critical events are reported in [Resource Health](/azure/service-health/resource-health-overview).
...
> 
> After you identify the offending agents and apply the recommendations, you can use [Priority and Fairness](https://kubernetes.io/docs/concepts/cluster-administration/flow-control/) or refer to the "Cause 3" section of [Troubleshoot API server and etcd problems in Azure Kubernetes Services](/troubleshoot/azure/azure-kubernetes/create-upgrade-delete/troubleshoot-apiserver-etcd?branch=pr-en-us-9260&tabs=resource-specific#cause-3-an-offending-client-makes-excessive-list-or-put-calls) to throttle or isolate problematic clients.
...
You might receive an alert that states that etcd memory usage exceeds 20 GiB. This alert indicates that your cluster is experiencing intensive API server load that has cascaded to overwhelm etcd memory capacity. This condition can cause performance degradation or potential outages if it's not resolved promptly.

To check the current etcd memory usage and understand the specific factors that contribute to the high memory consumption, navigate to **Diagnose and Solve Problems** in the Azure portal. Run the **Etcd Performance Analyzer** by searching for "_etcd performance_" in the Search box. The analyzer shows you the memory usage breakdown and helps identify whether the problem is caused by high request rates, large object counts, or large object sizes.
...
The root cause of high etcd memory usage is typically intensive API server load. Because this problem overlaps the other causes that are discussed in this article, use the following solution to identify the specific problem that's affecting your cluster.
...
- **For high request rates**: Use the **API Server Resource Intensive Listing Analyzer** that's discussed in [Step 2](#step-2-identify-and-chart-the-average-latency-of-api-server-requests-per-user-agent) to identify agents that make excessive LIST calls.
...
- Move environment variables from pod specifications to ConfigMaps to reduce pod specification sizes.
...
> [!IMPORTANT]
> If the API server becomes unresponsive because of severe etcd memory pressure, and you can't perform the diagnostic steps that are provided in this article, contact Azure support immediately to request assistance to clean up problematic objects or throttle excessive requests.
