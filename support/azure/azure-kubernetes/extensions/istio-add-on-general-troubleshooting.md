...
| `My customization from ConfigMap is reverted`|If you're using the AKS Istio add-on (managed Istio), the most common reason is that you modified a managed ConfigMap that is continuously reconciled by the add-on. Unsupported changes are automatically reverted back to the desired state|Allowed customizations  [HPA](/azure/aks/istio-gateway-api#horizontalpodautoscaler-hpa-customization-allow-list-fields)  [Ingress gateway service annotations](/azure/aks/istio-deploy-ingress#ingress-gateway-service-customizations)  [Allowed mesh config fields](/azure/aks/istio-meshconfig#meshconfig) |
| `My namespace is already enabled with label injected  but is the traffic is not working!` | Not use  kubectl label namespace default istio-injection=enabled or istio-injection=disabled | The default istio-injection=enabled label will not work and will cause the sidecar injection to skip the namespace for the add-on going to unsupported scenarios. [Use the current revision deployment](/azure/aks/istio-deploy-addon#enable-sidecar-injection) |
