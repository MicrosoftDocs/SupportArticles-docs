...
### Troubleshooting step 1: Check the Secrets Store Provider pod logs

1. Run the following [kubectl get](https://kubernetes.io/docs/reference/generated/kubectl/kubectl-commands#get) command to find the Secrets Store Provider pod that runs on the same node that your application pod runs on:

    ```bash
    kubectl get pod -l app=secrets-store-provider-azure -n kube-system -o wide
    ```

2. Run the following [kubectl logs](https://kubernetes.io/docs/reference/generated/kubectl/kubectl-commands#logs) command to view logs from the Secrets Store Provider application pod:

    ```bash
    kubectl logs -n kube-system <provider-pod-name> --since=1h | grep ^E
    ```

### Troubleshooting step 2: Check the Secrets Store CSI Driver pod logs

1. Run the following [kubectl get](https://kubernetes.io/docs/reference/generated/kubectl/kubectl-commands#get) command to find the Secrets Store CSI Driver pod that runs on the same node that your application pod runs on:

    ```bash
    kubectl get pod -l app=secrets-store-csi-driver -n kube-system -o wide
    ```

2. Run the following [kubectl logs](https://kubernetes.io/docs/reference/generated/kubectl/kubectl-commands#logs) command to view logs from the Secrets Store CSI Driver application pod:

    ```bash
    kubectl logs -n kube-system <driver-pod-name> -c secrets-store --since=1h | grep ^E
    ```
