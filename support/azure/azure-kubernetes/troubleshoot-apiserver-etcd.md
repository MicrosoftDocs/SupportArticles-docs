...
When considering the resilience of the API server we have tested reliability and performance to a scale of 5,000 nodes and 65,000 pods, with the ability to automatically scale out and deliver [Kubernetes SLOs][K8s SLOs]. As such, if you observe high latencies or timeouts, these are very likely due to a resource leakage on etcd or an offending client with excessive heavy API calls (e.g too many LIST calls with heavy objects).
...
- Ensure that you are using the Standard tier of AKS. If you are on the Free tier, the API server and Etcd will come with limited resources and will not provide high availability, or autoscale, which is often the root cause of API server and Etcd issues. 
...
The following sections will help you identify if your cluster is affected by any of the three most common causes for failures:
- A network rule blocking traffic to API Server from agent nodes
- An offending client is making excessive LIST or PUT calls
- An offending client leaks Etcd objects and results in slow down of Etcd

### Verify network connectivity between the API server and agentpool nodes

This command can be used to validate whether a misconfigured network policy is blocking communication between the API server and agentpool system nodes. If the following command returns connection succeeded then connectivity is unimpeded.
...
It can be useful to identify which clients are generating the most requests, and potentially the most API Server load. Below you will find a query that lists the top 10 user agents by number of apiserver requests sent. 
...
This query identifies the average latency of apiserver requests per user agent plotted on a time chart. This query is a follow-on to the previous query that may lend additional insights into the true load over time generated by each user agent.

Troubleshooting tip: By analyzing this data, you can identify patterns and anomalies that may indicate issues with your Kubernetes cluster or applications. For example, you might notice that a particular user is experiencing high latency, which could indicate the type of API call that is causing excessive load on either API server or etcd.
...
Troubleshooting tip:  This can be useful for identifying types of API calls that may be causing the problems. In most cases, an offending client may be making too many LIST calls on a large set of objects or very large objects in size. Unfortunately, there aren't hard scalability limits that can guide us on when such issues occur as the scalability of the control plane depend on variety of factors explained in the [Kubernetes Scalability documents](https://github.com/kubernetes/community/blob/master/sig-scalability/configs-and-limits/thresholds.md). 
...
A common issue is to continuously create objects without deleting unused ones in the Etcd database. The etcd database is prone to performance issues when dealing with too many objects of any type (>10K). A rapid increase of changes on such objects could also result in exceeding the etcd database size (4GB by default).

To check for Etcd database usage, go to Diagnose and Solve blade on the Azure Portal. Run the Etcd Availability diagnosis by searching for Etcd in the search box. The diagnosis will show you usage breakdown as well as the total database size. 
...
## References
[Query and Logging apiserver][monitor-aks-reference.md]
