...
#### In Application Insights, I see only a fraction of the events that are being generated by my app
...
## Request count collected by tje Application Insights SDK doesn't match the IIS log count for my application

Internet Information Services (IIS) logs counts of all requests reaching IIS and can inherently differ from the total requests reaching an application. Because of this behavior, it isn't guaranteed that the request count collected by the SDKs will match the total IIS log count.
...
* In Solution Explorer, right-click the project and select **Application Insights** > **Configure Application Insights**. In the dialog box, you can either select to send telemetry to an existing resource, or create a new one.
...
1. Restart the process so that these new settings are picked up by the SDK.
