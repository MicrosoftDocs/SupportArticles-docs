...
|Unsupported URL|This URL is not supported|<p>Availability tests only allows communicating over publicly available IP addresses and hostnames. This error might occur when you try to communicate with an internal only IP address that isn't routable via the public internet.</p> <p>To resolve this error, ensure only public IP addresses are defined in your webtest and that any DNS lookups your webtest depends on only return valid publicly routable IP addresses. For what IP addresses your webtest tries to use during execution, see [DNS resolution troubleshooting step](diagnose-ping-test-failure.md#view-the-application-insights-troubleshooting-report)</p>|
