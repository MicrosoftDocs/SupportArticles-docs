---
title: Troubleshoot 403 errors in Azure Blob Storage
description: Resolve HTTP 403 errors in Azure Blob Storage by diagnosing authentication, authorization, network, and configuration issues. Learn how to identify root causes and fix them quickly.
ms.date: 07/10/2026
...
This article provides guidance for troubleshooting HTTP 403 errors in Azure Blob Storage. It includes a comprehensive checklist of potential causes and solutions, covering authentication, authorization, network restrictions, encryption policies, and more.

When you access Azure Blob Storage, you might encounter HTTP 403 errors that prevent clients from reading, writing, or managing data. These errors can result from a variety of configuration or authorization issues, including role assignments, token settings, network restrictions, encryption policies, and more. This article provides a comprehensive checklist and troubleshooting guidance to help you identify the root cause of 403 errors and resolve them efficiently. If you already know the error code or cause category, use the links in the [diagnostic checklist](#comprehensive-403-error-diagnostic-checklist) to jump directly to the relevant section.
...
## Common 403 errors related to authentication failures
...
| **Error message**<br>Server failed to authenticate the request. Make sure the value of Authorization header is formed correctly including the signature.<br><br>**Error code**<br>AuthenticationFailed<br><br>**Error detail**<br>SAS field 'sv' is not well formed. | **Invalid SAS format**: Use official Azure SDKs or tools like Storage Explorer to generate SAS tokens.<br><br>**Manual construction errors**: If you construct SAS tokens manually, verify all required fields and their formats.<br><br>**Version mismatch**: Ensure the `sv` (version) parameter matches your Storage Client Library version.|
| **Error message**<br>Server failed to authenticate the request. Make sure the value of Authorization header is formed correctly including the signature.<br><br>**Error code**<br>AuthenticationFailed<br><br>**Error detail**<br>The specified signed resource is not allowed for the this resource level | **Service vs Account SAS**: You're using a service SAS (for specific blobs or containers) for account-level operations.<br><br>**Scope mismatch**: Generate the appropriate SAS type for your operation. Use an account SAS for account-level operations (list containers), and use a service SAS for specific blob or container operations. |
| **Error message**<br>Key based authentication is not permitted on this storage account.<br><br>**Error code**<br>KeyBasedAuthenticationNotPermitted | **Policy restriction**: The storage account is configured to disallow shared key authentication.<br><br>**Switch to Microsoft Entra ID**: Use Microsoft Entra ID authentication instead of account keys.<br><br>**Enable shared key**: If needed, re-enable shared key access in the storage account configuration. |

## Common 403 errors related to authorization failures
...
>
> - **Shared Key authorization**: If you're using an account key, confirm that Shared Key authorization is allowed. (**[Learn more](/azure/storage/common/shared-key-authorization-prevent)**).
>
...
>
> - **Disabled accounts**: Confirm the storage account or subscription isn't disabled. (**[Read more](#disabled-accounts)**).
>
...
Common causes of 403 errors when you use SAS tokens include:
...
- **Token generation method:** You didn't generate the SAS token by using an official SDK or tool (such as Storage Explorer). To avoid errors in the signature string, consider using official SDKs to generate SAS tokens. If you construct SAS tokens manually, use the latest storage service version and libraries. See [Delegating Access with a Shared Access Signature](/rest/api/storageservices/delegate-access-with-shared-access-signature).

- **Expired SAS token:** The client is using a SAS token that is expired.

- **Clock skew:** Small time differences between the host generating the SAS and the storage service cause the SAS to be invalid or expire earlier than expected.
...
- **Regenerated access keys:** If you regenerate storage access keys, you might invalidate existing SAS tokens, especially those with long expiry times.
...
- **Interference by a stored access policy:** A container with stored access policies can override the permission given by a SAS. For example, a stored access policy on a container might revoke permission earlier than the SAS expiry date and time.
...
- **Revoked key:** The request is made after the user delegation key is revoked or expired.

See [Create a user delegation SAS](/rest/api/storageservices/create-user-delegation-sas).

### Stored access policies

When you use stored access policies, review the following items:

- **Policy settings:** Ensure the permissions, start time, and expiration time defined in the stored access policy are correct for your scenario.

- **SAS token:** Ensure the SAS token associated with the policy matches the intended permissions and time window.

After you apply or update a stored access policy, allow up to 30 seconds for the policy to take effect. During this interval, any SAS associated with the policy might fail with a 403 (Forbidden) error until the policy becomes active.

To learn more, see [Define a stored access policy](/rest/api/storageservices/define-stored-access-policy).

### Access control lists (ACLs)

When you use ACLs to authorize access to files and directories, review the following items.
...
- **Sticky bit isn't set:** ACLs have a setting called the sticky bit, which can also cause 403 errors. If the sticky bit is enabled on a directory, only the child item's owner, the directory's owner, or the Superuser ($superuser) can delete or rename child items. To learn more, see [403 Access Denied authorization error when the sticky bit is enabled in ADLS Gen2](adls-gen2-sticky-bit-403-access-denied.md).
...
- **Source and destination access:** When copying blobs between storage accounts, the client must have network access to both accounts. If you're using private link for only one account, ensure network access to the other account as well.

For general troubleshooting, see [Troubleshoot Azure Private Endpoint connectivity problems](../../../private-link/troubleshoot-private-endpoint-connectivity-problems.md).

### Encryption scopes

When you use encryption scopes, review the following items.

- **Encryption scope override setting:** If a container enforces a default encryption scope and prohibits overrides, uploading a blob with a different encryption scope results in a 403 error. You can set the override prohibition only when you create the container. To learn more, see [Create a container with a default encryption scope](/azure/storage/blobs/encryption-scope-manage#create-a-container-with-a-default-encryption-scope). To check if overrides are prohibited, examine the container properties. See [Manage blob containers using PowerShell](/azure/storage/blobs/blob-containers-powershell) or [Manage blob containers using Azure CLI](/azure/storage/blobs/blob-containers-cli).

- **Disabled key:** If an encryption scope uses a customer-managed key and you disable the key, all access attempts to data in that container fail with a 403 error. See [Revoke access to a storage account that uses customer-managed keys](/azure/storage/common/customer-managed-keys-overview#revoke-access-to-a-storage-account-that-uses-customer-managed-keys).
...
A 403 error can occur if your subscription includes an Azure Databricks namespace. Roles scoped to the subscription don't grant access to blob data. Assign roles at the resource group, storage account, or container level instead. To reassign a role, go to the storage account in the Azure portal, select **Access control (IAM)**, and add the role assignment at the container or storage account scope.
