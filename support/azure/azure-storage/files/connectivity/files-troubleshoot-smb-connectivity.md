...
Ensure that SMB mount commands don't override the default NTLM v2 authentication via the `sec` option. The `sec` option should never use `ntlm` or `ntlmi` when connecting to SMB Azure file shares. If you use `/etc/samba/smb.conf` to set global options, ensure you don't disable NTLM v2.

##### Cause 4: Storage account key access is disabled or enforced via a policy
