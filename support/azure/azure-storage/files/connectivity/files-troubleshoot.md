...
- [Microsoft Q&A question page for Azure Files](/answers/tags/133/azure).
...
The most common problem encountered by Azure Files customers is that mounting or accessing the Azure file share fails because of an incorrect networking configuration. This problem can happen with any of the three file sharing protocols that Azure Files supports: SMB, NFS, and FileREST.
...
> [!NOTE]  
> You must use the fully qualified domain name (FQDN) for your storage account when mounting or accessing the share. The following commands let you see the current IP addresses of the network endpoints of your storage account, but you shouldn't hardcode these IP addresses into any scripts, firewall configurations, or other locations. IP addresses aren't guaranteed to remain the same, and they may change at any time.
...
The output returned by `Resolve-DnsName` might be different depending on your environment and desired networking configuration. For example, if you're trying to access a public endpoint of a storage account that doesn't have any private endpoints configured, you see the following output. In this output, `x.x.x.x` is the IP address of the cluster `file.phx10prdstf01a.store.core.windows.net` of the Azure storage platform that serves your storage account.
...
If you're trying to access the public endpoint of a storage account that has one or more private endpoints configured, you see the following output. The output includes an extra CNAME record for `mystorageaccount.privatelink.file.core.windows.net`, positioned between the usual FQDN of the storage account and the name of the storage cluster. This record enables name resolution to the public endpoint's IP address when the user is accessing from the internet, and resolution to the private endpoint's IP address when the user is accessing from inside of an Azure virtual network (or peered network).
...
If you're resolving to a private endpoint, you normally expect an A record for `mystorageaccount.privatelink.file.core.windows.net` that maps to your private endpoint's IP address:
...
The output returned by `nslookup` might be different depending on your environment and desired networking configuration. For example, if you're trying to access the public endpoint of the storage account that doesn't have a private endpoint configured, you see a result that looks like the following, where `x.x.x.x` is the IP address of the cluster `file.phx10prdstf01a.store.core.windows.net` of the Azure storage platform that serves your storage account:
...
If you're trying to access the public endpoint of a storage account that has one or more private endpoints configured, you see a result that looks something like the following. Note that an additional CNAME record for `mystorageaccount.privatelink.file.core.windows.net` is inserted in between the normal FQDN of the storage account and the name of storage cluster. This record enables name resolution to the public endpoint's IP address when the user is accessing from the internet, and resolution to the private endpoint's IP address when the user is accessing from inside of an Azure virtual network (or peered network).
...
The following command lets you test your client's ability to make a TCP connection to the resolved IP address and port number.
...
If the connection is successfully established, you see the following result:
