...
| 0x800705aa | -2147023446 | ERROR_NO_SYSTEM_RESOURCES | The file failed to recall due to insufficient system resources. <br/> Note: If the Azure File Sync agent version is 19 or later and the Storage Sync Agent service (FileSyncSvc) is not running, renaming files or folders within the server endpoint location will fail with this error.| Verify the Storage Sync Agent service (FileSyncSvc) is running. If FileSyncSvc is running and the error persists, investigate which application or kernel-mode driver is exhausting system resources. |
