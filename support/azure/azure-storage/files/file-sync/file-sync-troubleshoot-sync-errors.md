...
<a id="per-item-error-bad-net-name"></a>
...
See [Tiered files are not accessible on the server](file-sync-troubleshoot-cloud-tiering.md#tiered-files-are-not-accessible-on-the-server).

<a id="per-item-ecs-e-sync-constraint-conflict"></a>
...
<a id="per-item-ecs-e-sync-constraint-conflict-on-failed-dependee"></a>
...
<a id="per-item-ecs-e-sync-item-process-fail-recoverable"></a>
...
<a id="per-item-ecs-e-sync-constraint-conflict-session-failed"></a>
...
<a id="per-item-error-invalid-name"></a>
...
Rename the file or directory. For more information, see [Handling unsupported characters](?tabs=portal1%252cazure-portal#handling-unsupported-characters).

<a id="per-item-error-no-unicode-translation"></a>
...
<a id="per-item-ecs-e-xsmb-rest-incompatibility"></a>
...
<a id="per-item-ecs-e-sync-item-restricted-at-root"></a>
...
No action is required. For more information, see [File system compatibility](/azure/storage/file-sync/file-sync-planning#file-system-compatibility).

<a id="per-item-ecs-e-sync-file-in-use"></a>
...
<a id="per-item-ecs-e-sync-cloud-file-in-use"></a>
...
Check for open file handles and close them if they're no longer needed by using the [Get-AzStorageFileHandle](/powershell/module/az.storage/get-azstoragefilehandle) and [Close-AzStorageFileHandle](/powershell/module/az.storage/close-azstoragefilehandle) PowerShell cmdlets.

<a id="per-item-ecs-e-concurrency-check-failed"></a>
...
<a id="per-item-ecs-e-azure-file-path-too-long"></a>
...
Run the *FileSyncErrorsReport.ps1* PowerShell script in the Azure File Sync agent installation directory to identify failing file paths. Then, use the [ScanUnsupportedChars](https://github.com/Azure-Samples/azure-files-samples/tree/master/ScanUnsupportedChars) script.

<a id="per-item-error-file-not-found"></a>
...
<a id="per-item-error-path-not-found"></a>
...
<a id="per-item-ecs-e-sync-item-skip"></a>
...
No action is required if the error occurs during upload. If the error occurs during download, rename the file or directory. For more information, see [Handling unsupported characters](?tabs=portal1%252cazure-portal#handling-unsupported-characters).

<a id="per-item-error-already-exists"></a>
...
<a id="per-item-ecs-e-azure-storage-share-size-limit-reached"></a>
...
See [You reached the Azure file share storage limit](?tabs=portal1%252cazure-portal#-2134351810).

<a id="per-item-ecs-e-cannot-create-azure-staged-file"></a>
...
<a id="per-item-ecs-e-access-denied-efs"></a>
...
Decrypt the file and use a supported encryption solution. For more information, see [Encryption](/azure/storage/file-sync/file-sync-planning#encryption).

<a id="per-item-ecs-e-access-denied-dfsrro"></a>
...
Azure File Sync doesn't support server endpoints on DFS-R read-only replication folders. For more information, see [Distributed File System (DFS)](/azure/storage/file-sync/file-sync-planning#distributed-file-system-dfs).

<a id="per-item-error-access-denied"></a>
...
<a id="per-item-ecs-e-azure-authorization-failed"></a>
...
Add the server IP address or virtual network. For more information, see [Configure firewall and virtual network settings](/azure/storage/file-sync/file-sync-deployment-guide?tabs=azure-portal#optional-configure-firewall-and-virtual-network-settings).

<a id="per-item-e-unexpected"></a>
...
<a id="per-item-ecs-e-etag-verifcation-failed"></a>
...
<a id="per-item-error-sharing-violation"></a>
...
<a id="per-item-ecs-e-sync-oplock-broken"></a>
...
See [Tiered files are not accessible on the server](file-sync-troubleshoot-cloud-tiering.md#tiered-files-are-not-accessible-on-the-server) to remove orphaned tiered files. If the error continues after you remove orphaned tiered files, run [chkdsk](/windows-server/administration/windows-commands/chkdsk) on the volume.

<a id="per-item-error-file-invalid"></a>
...
If the file is tiered, see [Tiered files are not accessible on the server](file-sync-troubleshoot-cloud-tiering.md#tiered-files-are-not-accessible-on-the-server) to remove orphaned tiered files.

<a id="per-item-error-file-corrupt"></a>
...
Run [chkdsk](/windows-server/administration/windows-commands/chkdsk) on the volume.

<a id="per-item-ecs-e-sync-conflict-name-exists"></a>
...
Reduce the number of conflict files. The file syncs after the number of conflict files is less than 100. For more information, see [Azure File Sync FAQ](/azure/storage/files/storage-files-faq?toc=/azure/storage/filesync/toc.json#afs-conflict-resolution).

<a id="per-item-ecs-e-directory-rename-failed"></a>
...
<a id="per-item-error-bad-file-type"></a>
...
<a id="per-item-ecs-e-data-transfer-blocked"></a>
...
<a id="per-item-ecs-e-sync-invalid-staged-file"></a>
...
<a id="per-item-error-invalid-handle"></a>
...
<a id="per-item-error-not-supported"></a>
...
<a id="per-item-error-delete-pending"></a>
...
This error should resolve automatically. If it doesn't, check for open file handles and close them if they're no longer needed by using the [Get-AzStorageFileHandle](/powershell/module/az.storage/get-azstoragefilehandle) and [Close-AzStorageFileHandle](/powershell/module/az.storage/close-azstoragefilehandle) PowerShell cmdlets.

<a id="per-item-sync-e-item-must-exist"></a>
...
<a id="per-item-ecs-e-sync-initial-scan-completed"></a>
...
<a id="per-item-ecs-e-azure-file-share-file-not-found"></a>
...
<a id="per-item-ecs-e-sync-unprocessable-item-reparsepoint"></a>
...
<a id="per-item-ecs-e-item-path-component-has-trailing-dot"></a>
...
<a id="per-item-ecs-e-sync-item-skip-constraint-conflict-not-allowed"></a>
...
<a id="per-item-ecs-e-sync-item-reconciliation-skip"></a>
...
#### ECS_E_REQUEST_CANCELLED_EXTERNALLY and related errors: The operation was cancelled.
...
#### ECS_E_EXTERNAL_STORAGE_ACCOUNT_AUTHORIZATION_FAILED and related errors: Sync can't access the Azure file share specified in the cloud endpoint.
...
#### ECS_E_AZURE_STORAGE_SHARE_SIZE_LIMIT_REACHED and related errors: You reached the Azure file share storage limit.
...
#### ECS_E_SYNC_METADATA_WRITE_LOCK_TIMEOUT and related errors: Sync failed due to a problem with the sync database.
...
<a id="-1906441711"></a><a id="-2134375654"></a><a id="doesnt-have-enough-free-space"></a>

#### JET_errLogDiskFull and related errors: The volume where the server endpoint is located is low on disk space.
...
<a id="-2134364145"></a><a id="replica-not-ready"></a>
...
#### ECS_E_SYNC_METADATA_KNOWLEDGE_SOFT_LIMIT_REACHED and related errors: Sync failed due to problems with many individual files.
...
#### ECS_E_WORK_FRAMEWORK_TIMEOUT and related errors: Sync session timeout error.
...
#### ECS_E_CANNOT_CREATE_ACTIVE_SESSION_PLACEHOLDER_BLOB and related errors: Sync session error.
...
#### ECS_TOO_MANY_ETAGVERIFICATION_FAILURES and related errors: Sync session error.
...
#### ERROR_INVALID_PARAMETER and related errors: An internal error occurred.
...
#### COR_E_INVALIDOPERATION and related errors: An internal error occurred.
...
#### ERROR_LOGON_FAILURE and related errors: Operation failed due to an authentication failure.
