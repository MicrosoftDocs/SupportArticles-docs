...
3.    Confirm that the DNS domain name is correctly [delegated to the Azure DNS name servers](/azure/dns/dns-domain-delegation). There are a [many 3rd-party web sites that offer DNS delegation validation](https://www.bing.com/search?q=dns+check+tool). This test is a *zone* delegation test, so you should only enter the DNS zone name and not the fully qualified record name.
