...
Azure provides a stable and fast way to connect your on-premises network to Azure. Customers of all sizes use methods like Site-to-Site VPN and ExpressRoute to run their businesses in Azure. But what happens when performance doesn't meet your expectations or previous experience? This article helps you standardize the way you test and baseline your specific environment.

You learn how to easily and consistently test network latency and bandwidth between two hosts. You also receive advice on ways to look at the Azure network to help isolate problem points. The PowerShell script and tools discussed require two hosts on the network (at either end of the link being tested). One host must be a Windows Server or Desktop, and the other host can be either Windows or Linux.

## Network components

Before digging into troubleshooting, let's discuss some common terms and components. This discussion ensures you're thinking about each component in the end-to-end chain that enables connectivity in Azure.

:::image type="content" source="./media/expressroute-troubleshooting-network-performance/network-components.png" alt-text="Screenshot of a diagram showing routing domains between on-premises and Azure using ExpressRoute or VPN.":::
...
- **Virtual Machine** - The server might have multiple NICs. Ensure any static routes, default routes, and operating system settings send and receive traffic the way you expect. Also, each VM SKU has a bandwidth restriction. If you're using a smaller VM SKU, your traffic is limited by the bandwidth available to the NIC. Use a DS5v2 for testing to ensure adequate bandwidth at the VM.
...
- **Subnet NSG** - Just like the NIC, you can apply NSGs at the subnet level as well. Ensure the NSG rule set is appropriate for the traffic you're trying to pass. For traffic inbound to the NIC, the subnet NSG applies first, then the NIC NSG. When traffic is going outbound from the VM, the NIC NSG applies first, then the Subnet NSG.
 
- **Subnet UDR** - User-Defined Routes can direct traffic to an intermediate hop, like a firewall or load balancer. Ensure you know if there's a UDR in place for your traffic. If so, understand where it goes and what that next hop does to your traffic. For example, a firewall could pass some traffic and deny other traffic between the same two hosts.
...
- **VNet Gateway (ExpressRoute)** - Once peering (ExpressRoute) or VPN is enabled, there aren't many settings that can affect how or if traffic routes. If you have a virtual network gateway connected to multiple ExpressRoute circuits or VPN tunnels, be aware of the connection weight settings. The connection weight affects connection preference and determines the path your traffic takes.
...
At this point, you're on the WAN portion of the link. This routing domain can be your service provider, your corporate WAN, or the Internet. Many hops, devices, and companies are involved with these links, which can make it difficult to troubleshoot. You need to first rule out both Azure and your corporate networks before you can investigate the hops in between.

In the preceding diagram, on the far left is your corporate network. Depending on the size of your company, this routing domain can be a few network devices between you and the WAN or multiple layers of devices in a campus or enterprise network.
...
You can analyze and isolate most network problems by using basic tools like ping and traceroute. It's rare that you need to go as deep as packet analysis by using tools like Wireshark.
...
You can use a PowerShell module (AzureCT) that wraps these tools and methods.

### AzureCT - the Azure Connectivity Toolkit

The AzureCT PowerShell module includes two components: [Availability Testing][Availability Doc] and [Performance Testing][Performance Doc]. This article focuses on Performance Testing, specifically the two Link Performance commands in this PowerShell module.

To use this toolkit for Performance Testing, follow these three basic steps:
...
4. **Review the Test Output**
    ...
    :::image type="content" source="./media/expressroute-troubleshooting-network-performance/powershell-output.png" alt-text="Screenshot of PowerShell output from the Link Performance test command.":::
...
If the performance test results aren't what you expected, follow a systematic approach to identify the problem. Given the number of components in the path, a step-by-step process is more effective than random testing.

>[!NOTE]
>This scenario is a performance problem, not a connectivity problem. To isolate connectivity problems to the Azure network, see [Verifying ExpressRoute connectivity](expressroute-troubleshooting-expressroute-overview.md).
>

1. **Challenge your assumptions**

    Make sure your expectations are reasonable. For example, with a 1-Gbps ExpressRoute circuit and 100 ms of latency, expecting the full 1 Gbps of traffic is unrealistic due to the performance characteristics of TCP over high latency links. For more information about performance assumptions, see the [References section](#references).

2. **Start at the edge of the network**

    Begin at the edges between routing domains and try to isolate the problem to a single major routing domain. Avoid blaming the "black box" in the path without thorough investigation, as this assumption can delay resolution.
...
4. **Divide and conquer**
    ...
    Segment the network and narrow down the problem. Identify where it works and where it doesn't. Keep moving your testing points to isolate the offending component.

5. **Consider all OSI layers**

    While it's common to focus on the network and layers 1-3 (Physical, Data, and Network layers), remember that problems can also occur at Layer 7 (Application layer). Keep an open mind and verify all assumptions.
...
:::image type="content" source="./media/expressroute-troubleshooting-network-performance/expressroute-troubleshooting.png" alt-text="Screenshot of a diagram showing multiple virtual networks connected to an ExpressRoute circuit.":::
...
The more you isolate the problem, the faster you can find a solution. Sometimes, you reach a point where you can't go further with troubleshooting. For example, you might see the link across your service provider taking hops through Europe when you expect it to remain in Asia. At this point, engage someone for help based on the routing domain you isolated the issue to. Narrowing it down to a specific component is even better.
...
For WAN issues, share your testing results with your service provider or ISP to help them with their work and avoid redundant tasks. They might want to verify your results based on the principle of *trust but verify*.
...
>[!TIP]
> Geographic distance between endpoints is the largest factor in latency. While equipment latency (physical and virtual components, number of hops, and other factors) also plays a role, the distance of the fiber run, not the straight-line distance, is the primary contributor. This distance is difficult to measure accurately, so use a city distance calculator for a rough estimate.

For example, you set up an ExpressRoute in Seattle, Washington, USA. The following table shows the latency and bandwidth you observe when testing to various Azure locations, along with estimated distances.
...
- All tests use the AzureCT Get-LinkPerformance command with a 5-minute load test for each of the six test runs. For example:
    ...
    ```
- The data flow for each test is from the on-premises server (iPerf client in Seattle) to the Azure VM (iPerf server in the listed Azure region).
...
:::image type="content" source="./media/expressroute-troubleshooting-network-performance/test-diagram.png" alt-text="Screenshot of a diagram showing the AzureCT testing environment and network data flow.":::

### Latency and bandwidth results

>[!IMPORTANT]
> Use these numbers for general reference only. Many factors affect latency, and while these values are generally consistent over time, conditions within Azure or the service provider's network can change, affecting latency and bandwidth. Generally, these changes don't result in significant differences.
...
\* The latency to Brazil is an example where the fiber run distance significantly differs from the straight-line distance. The expected latency is around 160 ms, but it's actually 189 ms due to the longer fiber route.

>[!NOTE]
> AzureCT tests these numbers by using iPerf in Windows via PowerShell. iPerf doesn't honor default Windows TCP options for Scaling Factor and uses a lower Shift Count for the TCP Window size. By tuning iPerf commands by using the `-w` switch and a larger TCP Window size, you can achieve better throughput. Running iPerf in multithreaded mode from multiple machines can also help you reach maximum link performance. To get the best iPerf results on Windows, use `Set-NetTCPSetting -AutoTuningLevelLocal Experimental`. Check your organizational policies before making any changes.
