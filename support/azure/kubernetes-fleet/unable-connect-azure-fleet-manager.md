...
This article helps you resolve an issue preventing you from connecting to an Azure Kubernetes Fleet Manager hub cluster using `kubectl`.
...
When attempting to call any API on the Fleet Manager hub cluster using `kubectl`, you're prompted to enter a device code to authenticate.
...
To resolve this issue, use the following command to modify kubeconfig to use Azure CLI authentication instead of device code authentication:

```bash
kubelogin convert-kubeconfig -l azurecli
```

Following this change, when you use `kubectl`, your existing Azure sign-in session is used or a non-device flow is initiated if you aren't already authenticated.
