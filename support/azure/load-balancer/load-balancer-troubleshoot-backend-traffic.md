...
Use this article to troubleshoot Azure Load Balancer backend traffic issues and restore reliable connectivity and distribution for your virtual machines.

## Virtual machines behind a load balancer are receiving uneven distribution of traffic

If you suspect backend pool members are receiving traffic, it could be due to the following causes. Azure Load Balancer distributes traffic based on connections. Be sure to check traffic distribution per connection and not per packet. Verify using the **Flow Distribution** tab in your preconfigured [Load Balancer Insights dashboard](/azure/load-balancer/load-balancer-insights#flow-distribution).
