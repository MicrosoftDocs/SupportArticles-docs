...
This article helps you troubleshoot Azure Load Balancer health event logs so you can quickly restore connectivity and keep applications available.
...
1. Confirm at least one backend instance is responding to the health probe configured to the associated load balancing rule.  The rule includes the frontend IP, protocol, and port provided in the event description.
   ...
   2. If no, refer to [Troubleshoot Azure Load Balancer health probe status](/azure/load-balancer/load-balancer-troubleshoot-health-probe-status) | for more detailed troubleshooting steps.
...
### Troubleshooting steps
- Refer to [Troubleshoot Azure Load Balancer health probe status](/azure/load-balancer/load-balancer-troubleshoot-health-probe-status) for common reasons why your backend instances aren't responding to the configured health probes.
...
```
### Troubleshooting steps
- For more information about Azure's common outbound connectivity options, see [Source Network Address Translation (SNAT) for outbound connections - Azure Load Balancer](/azure/load-balancer/load-balancer-outbound-connections).
...
1. To resolve SNAT exhaustion issues, we recommend using NAT Gateway instead. To learn more about how NAT Gateway mitigates the risk of SNAT port exhaustion, see more on [Source Network Address Translation (SNAT) with Azure NAT Gateway](/azure/nat-gateway/nat-gateway-snat).
2. If you're currently using load balancer outbound rules and would like to migrate to using NAT Gateway instead, see [Tutorial: Migrate outbound access to NAT gateway](/azure/nat-gateway/tutorial-migrate-outbound-nat).
...
- [Azure Load Balancer health event logs](/azure/load-balancer/load-balancer-health-event-logs)
- [Learn to monitor and alert with Azure Load Balancer health event logs](/azure/load-balancer/load-balancer-monitor-alert-health-event-logs)