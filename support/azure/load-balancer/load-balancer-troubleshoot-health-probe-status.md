...
ms.date: 04/06/2026
ms.custom: No connectivity to backend pool
...
This article helps you troubleshoot Azure Load Balancer health probe status issues so backend VMs can respond correctly and stay in load-balancing rotation.

## Symptom: VMs behind the Load Balancer aren't responding to health probes
For the backend servers to participate in the load balancer set, they must pass the probe check. For more information about health probes, see [Understanding Load Balancer Probes](/azure/load-balancer/load-balancer-custom-probe-overview). 
