...
* Adjust the [Transmission Control Protocol (TCP) idle timeout timer](/azure/nat-gateway/nat-gateway-resource#tcp-idle-timeout) settings as needed. An idle timeout timer set higher than the default (4 minutes) holds on to flows longer, and can create [extra pressure on SNAT port inventory](/azure/nat-gateway/nat-gateway-resource#timers).
...
* Free up SNAT port inventory by reducing the [TCP idle timeout timer](/azure/nat-gateway/nat-gateway-resource#idle-timeout-timers) to a lower value. The TCP idle timeout timer can't be set lower than 4 minutes.
...
* Check if you have [Private Link](/azure/private-link/manage-private-endpoint?tabs=manage-private-link-powershell#manage-private-endpoint-connections-on-azure-paas-resources) or [service endpoints](/azure/virtual-network/virtual-network-service-endpoints-overview#logging-and-troubleshooting) enabled for connecting to other Azure services.
...
When possible, Private Link should be used to connect directly from your virtual networks to Azure platform services in order to [reduce the demand](/azure/nat-gateway/troubleshoot-nat) on SNAT ports. Reducing the demand on SNAT ports can help reduce the risk of SNAT port exhaustion.
...
* [NAT gateway resource](/azure/nat-gateway/nat-gateway-resource)
