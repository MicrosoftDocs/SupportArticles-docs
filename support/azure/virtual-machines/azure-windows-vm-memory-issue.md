...
Any production system needs consistent monitoring, so Microsoft proactively gets alerts and use them for historic patterns to understand the resource requirements for the app environment better. Luckily, Azure Virtual Machines (both Windows and Linux) are deeply integrated with Azure Monitoring capability, which provides robust automated metrics collection and several avenues to build reports or use third-party tools to build reports your team needs.
