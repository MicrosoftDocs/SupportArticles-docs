...
Then, create the SWAP file under the resource disk path by using bash script or cloud-init.

## Create a SWAP file under the resource disk path by using bash script
...
> [!NOTE]
> In order to customize the swap file location, set the `LOCATION` variable to a desired path.

## Create a SWAP file under the resource disk path by using cloud-init

1. Create the `CLOUD_CFG` variable in */systemd/system.conf* to set both the swap and resource disk.

    ```bash
    sudo echo 'DefaultEnvironment="CLOUD_CFG=/etc/cloud/cloud.cfg.d/00-azure-swap.cfg"' >> /etc/systemd/system.conf
    ```
2. Create a yaml file that sets the SWAP and resource disk creation and custom mount point:
    ...
    > [!NOTE]
    > In order to customize the swap file location, set the `mounts: ephemeral0.1` setting to a desired path.
