...
export MY_VM_NAME="testvmRHEL83"
export OS="Red Hat"
```
...
<!--add a run command to validate the OS and choose which commadn to run -->
```bash
az vm run-command invoke --resource-group $MY_RESOURCE_GROUP_NAME --name $MY_VM_NAME --command-id RunShellScript --scripts "if [ \"\$OS\" == \"Ubuntu\" ] || [ \"\$OS\" == \"Debian\" ]; then sudo apt install sysstat; elif [ \"\$OS\" == \"Red Hat\" ]; then sudo dnf install sysstat; elif [ \"\$OS\" == \"SUSE\" ]; then sudo zypper install sysstat; else echo \"Unknown distribution\"; fi"
```
