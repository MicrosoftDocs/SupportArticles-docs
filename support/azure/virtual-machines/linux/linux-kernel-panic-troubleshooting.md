...
| **Kernel panic - not syncing: VFS: Unable to mount root fs on unknown-block(0,0)**| It's assumed that the kernel will use an initramfs to mount the rootfs. This error occurs when the kenel has no initramfs. |
