...
2. The ALAR scripts cannot run with only the VM Contributor role permission, as they also require read, write, and delete permissions at the resource group level. For more information, [Azure built-in roles](/azure/role-based-access-control/built-in-roles#privileged).
