...
> [!IMPORTANT]
> This action will change the VM's security posture by altering the auditd configuration so that the VM shut down issue can be resolved. Once the VM is running and accessible, revert the auditd configuration to the original state. For this purpose a backup of the auditd.conf file is created in /etc/audit by the ALAR action.
