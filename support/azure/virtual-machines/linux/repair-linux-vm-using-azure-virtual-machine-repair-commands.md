...
> * A running script can't be canceled.
> * The maximum time a script can run is 90 minutes, after which it times out.
> * Don't modify the tags that are created on the repair VM. The tags are necessary for the restore command to function correctly.
...
1. Launch Azure Cloud Shell
   ...
   The Azure Cloud Shell is a free interactive shell that you can use to run the steps in this article. It includes common Azure tools preinstalled and configured to use with your account. If you run a repair script that takes longer than 20 minutes, you might prefer to run the commands locally. To run the commands locally, Azure CLI version 2.0.67 or later version is required. Run `az --version` to find the version. If you need to install or upgrade your Azure CLI, see [Install Azure CLI](/cli/azure/install-azure-cli).
   ...
   If you need to log in to Cloud Shell with a different account than you're currently logged in to the Azure portal with you can use `az login` [az login reference](/cli/azure/reference-index#az-login&preserve-view=true). To switch between subscriptions associated with your account you can use `az account set --subscription` [az account set reference](/cli/azure/account#az-account-set&preserve-view=true).

2. If this is the first time you used the `az vm repair` commands, add the vm-repair CLI extension.

   ```azurecli-interactive
   az extension add -n vm-repair
   ```

   If you previously used the `az vm repair` commands, apply any updates to the vm-repair extension.

   ```azurecli-interactive
   az extension update -n vm-repair
   ```

3. Run `az vm repair create`. This command creates a copy of the OS disk for the non-functional VM, create a repair VM in a new Resource Group, and attach the copy of the OS disk. The repair VM is the same size and region as the non-functional VM specified. The Resource Group and VM name used in all steps are for the non-functional VM. If your VM is using Azure Disk Encryption, use `--unlock-encrypted-vm` to unlock the encrypted disk so that it's accessible when attached to the repair VM. For more information, see [confirm that ADE is enabled on the disk](../windows/unlock-encrypted-disk-offline.md#confirm-that-ade-is-enabled-on-the-disk).

> [!IMPORTANT]
> The run and restore commands require all inputs to be entered using the same case as used in the create command, make note or refer to the tags on the repair VM to see what was used.
...
4. Run `az vm repair run`. This command runs the specified repair script on the attached disk via the repair VM. If the troubleshooting guide you're using specified a run-id, use it here. Otherwise, you can use `az vm repair list-scripts` to see available repair scripts. The Resource Group and VM name used here are for the non-functional VM used in step 3. Additional information about the repair scripts can be found in the [repair script library](https://github.com/Azure/repair-script-library).
...
5. Run `az vm repair restore`. This command swaps the repaired OS disk with the original OS disk of the VM. The Resource Group and VM name used here are for the non-functional VM used in step 3.
...
* If you're having issues connecting to your VM, see [Troubleshoot RDP connections to an Azure Virtual Machine](../windows/troubleshoot-rdp-connection.md).
