...
Grand Unified Bootloader (GRUB) is loaded at an early stage in a Linux virtual machine (VM) process. It operates before the Linux kernel is loaded and before any system services (such as networking or SSH) are started, so GRUB isn't accessible via SSH. You must use the Serial Console to access GRUB. In GRUB, you can modify your boot configuration to boot into single-user mode, among other things.
