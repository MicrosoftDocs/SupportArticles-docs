...
3. Modify and use the correct value for the release version:
   ...
   - If your system is using EUS repositories, make sure that the appropriate version lock is applied. In the following example, the version lock is added for RHEL 9.6 because `RHUI EUS` repositories are used:
     ...
     > [!NOTE]
     > Support for RHEL 7 EUS ended on August 30, 2021. We recommend that you no longer use EUS repositories in RHEL7.
     >
