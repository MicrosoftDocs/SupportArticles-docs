...
Azure Serial Console requests to certain Azure v5 series virtual machines consistently result in the error “Sorry, the serial console was unable to connect to the VM because the service did not respond in a timely manner.” | On v5 series VMs: Resizing the virtual machine to a series not affected (e.g. Dv4) will enable a successful Azure Serial Console connection. VM series other than v5: In rare circumstances, communication failure may prevent the Azure node from properly applying boot diagnostics configuration. In the VM portal experience under “Redeploy and reapply”, choose to Reapply virtual machine state. Please see further details about the issue and the status of its fix on our GitHub repo [KNOWN ISSUE: Azure Serial Console connectivity issues with certain Azure v5 series Virtual Machines](https://github.com/microsoft/azserialconsole/issues/96)
