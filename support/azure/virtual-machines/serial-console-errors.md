...
> [!NOTE]
> The service URLs of Serial Console are changed from `*.console.azure.com` to `*.sereialconsole.azure.com`.
> If you receive the "Web socket is closed or could not be opened" error, add `*.sereialconsole.azure` to the firewall allow list.
