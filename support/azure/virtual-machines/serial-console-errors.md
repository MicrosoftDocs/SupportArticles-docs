...
Azure Serial Console requests to certain Azure v5 series virtual machines consistently result in the error “Sorry, the serial console was unable to connect to the VM because the service did not respond in a timely manner.” | For v5 series VMs: Resizing the virtual machine to a series (e.g. Dv4) that is not affected by the issue. This workaround will enable a successful Azure Serial Console connection. For more information about the issue and the status of its fix, see [KNOWN ISSUE: Azure Serial Console connectivity issues with certain Azure v5 series Virtual Machines](https://github.com/microsoft/azserialconsole/issues/96).</br></br>VM series other than v5: In rare circumstances, communication failure may prevent the Azure node from properly applying boot diagnostics configuration. To resolve the issue, [reapply virtual machine state](vm-stuck-in-failed-state.md#resolution) in the Azure portal.
