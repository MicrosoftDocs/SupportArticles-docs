...
6. Open **ProcMonTrace.PML** by using Process Monitor on the working VM. Then filter by **Result is ACCESS DENIED**, as shown in the following screenshot：
    ...
    :::image type="content" source="media/troubleshoot-remote-desktop-services-issues/process-monitor-access-denined.png" alt-text="Screenshot of the Filter button in the ribbon of Process Monitor and the Process Monitor Filter dialog when you filter by Result is ACCESS DENIED." border="false":::
