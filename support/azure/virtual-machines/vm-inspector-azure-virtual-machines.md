...
The VM Inspector tool helps you troubleshoot OS related issues that can affect a Window or Linux Virtual Machines (VM). It is designed as a self-help diagnostic tool that allows customers, support, or third-party personnel with privileged access to collect logs and configuration files on Azure VMs OS disk directly. VM Inspector is a fast and secure way for an authorized user to retrieve well-known log files from a VM OS disk to aid in the diagnosis of Guest OS issues.
...
A3. If you don't see error messages when you run VM inspector, it means that you don’t have the minimum access level of running this tool (Owner). To get access, refer to the section **Get access to VM Inspector**.

**Q4. Why do I need to provide consent to run VM Inspector successfully?**

A4. This means that the your disk doesn’t have the Disk Backup Reader Access in order to permit the tool to run successfully. But by selecting the agreement above to consent the Disk Backup Reader Access Role to be assigned to the disk, then you will be able to run VM Inspector successfully.
