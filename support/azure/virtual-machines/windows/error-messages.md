...
|  AllocationFailed  |  Allocation failed. Try reducing the VM size or number of VMs, retry later, or try deploying to a different Availability Set or different Azure location.  |  
|  AllocationFailed  |  The VM allocation failed due to an internal error. Retry later or try deploying to a different location.  |
|  ArtifactNotFound  |  The VM extension with publisher '{0}' and type '{1}' couldn't be found in location '{2}'.  |
|  ArtifactNotFound  |  Extension with publisher '{0}', type '{1}', and type handler version '{2}' couldn't be found in the extension repository.  |
...
|  AttachDiskWhileBeingDetached  |  Can't attach data disk '{0}' to VM '{1}' because the disk is currently being detached. Please wait until the disk is completely detached and then try again.  |
|  BadRequest  |  Aligned' Availability Sets aren't yet supported in this region.  |
|  BadRequest  |  Addition of a VM with managed disks to non-managed Availability Set or addition of a VM with blob based disks to managed Availability Set isn't supported. Please create an Availability Set with 'managed' property set in order to add a VM with managed disks to it.  |
|  BadRequest  |  Managed Disks aren't supported in this region.  |
|  BadRequest  |  Multiple VM Extensions per handler not supported for OS type '{0}'. VM Extension '{1}' with handler '{2}' already added or specified in input.  |
|  BadRequest  |  Operation '{0}' isn't supported on Resource '{1}' with managed disks.  |
|  CertificateImproperlyFormatted  |  The secret's JSON representation retrieved from {0} has a data field, which isn't a properly formatted PFX file, or the password provided doesn't decode the PFX file correctly.  |
|  CertificateImproperlyFormatted  |  The data retrieved from {0} isn't deserializable into JSON.  |
|  Conflict  |  Disk resizing is allowed only when creating a VM or when the VM is deallocated.  |
|  ConflictingUserInput  |  Disk '{0}' can't be attached as the disk is already owned by VM '{1}'.  |
...
|  DiskEncryptionKeySecretMissingTags  |  {0} secret doesn't have the {1} tags. Update the secret version, add the required tags and retry.  |
|  DiskEncryptionKeySecretUnwrapFailed  |  Unwrap of secret {0} value using key {1} failed.  |
|  DiskImageNotReady  |  Disk image {0} is in {1} state. Retry when image is ready.  |
...
|  InvalidParameter  |  The Admin password specified isn't allowed.  |
...
|  InvalidParameter  |  The Admin Username specified isn't allowed.  |
|  InvalidParameter  |  Can't attach an existing OS disk if the VM is created from a platform or user image.  |
|  InvalidParameter  |  Container name {0} is invalid. Container names must be 3-63 characters in length and might contain only lower-case alphanumeric characters and hyphen. Hyphen must be preceded and followed by an alphanumeric character.  |
...
|  InvalidParameter  |  The URI {0} doesn't look to be correct blob URI.  |
|  InvalidParameter  |  A disk named '{0}' already uses the same LUN: {1}.  |
|  InvalidParameter  |  A disk named '{0}' already exists.  |
|  InvalidParameter  |  Can't specify user image overrides for a disk already defined in the specified image reference.  |
...
|  InvalidParameter  |  Linux host name can't exceed {0} characters in length or contain the following characters: {1}.  |
...
|  InvalidParameter  |  {0}' isn't a valid captured VHD blob name prefix. A valid prefix matches regex '{1}'.  |
|  InvalidParameter  |  Certificates can't be added to your VM if the VM agent isn't provisioned.  |
|  InvalidParameter  |  A disk at LUN {0} already exists.  |
|  InvalidParameter  |  Unable to create the VM because the requested size {0} isn't available in the cluster where the availability set is currently allocated. The available sizes are: {1}. Read more on VM resizing strategy at <https://aka.ms/azure-resizevm>.  |
|  InvalidParameter  |  The requested VM size {0} isn't available in the current region. The sizes available in the current region are: {1}. Find out more on the available VM sizes in each region at <https://aka.ms/azure-regions>.  |
|  InvalidParameter  |  The requested VM size {0} isn't available in the current region. Find out more on the available VM sizes in each region at <https://aka.ms/azure-regions>.  |
|  InvalidParameter  |  Windows admin user name can't be more than {0} characters long, end with a period(.), or contain the following characters: {1}.  |
|  InvalidParameter  |  Windows computer name can't be more than {0} characters long, be entirely numeric, or contain the following characters: {1}.  |
|  MissingMoveDependentResources  |  The move resources request doesn't contain all the dependent resources. Check error details for missing resource IDs.  |
|  MoveResourcesHaveInvalidState  |  The Move Resources request contains VMs, which are associated with invalid storage accounts. Check details for these resource IDs and referenced storage account names.  |
|  MoveResourcesHavePendingOperations  |  The move resources request contains resources for which an operation is pending. Check details for these resource IDs. Retry your operation once the pending operations complete.  |
|  MoveResourcesNotFound  |  The move resources request contains resources that can't be found. Check details for these resource IDs.  |
...
|  NotFound  |  The Availability Set {0} can't be found.  |
|  NotFound  |  Source Virtual Machine '{0}' specified in the request doesn't exist in this Azure location.  |
|  NotFound  |  Tenant with ID {0} not found.  |
|  NotFound  |  The Image {0} can't be found.  |
|  NotSupported  |  The license type is {0}, but the image blob {1} isn't from on-premises.  |
|  OperationNotAllowed  |  Availability Set {0} can't be deleted. Before deleting an Availability Set please ensure that it doesn't contain any VM.  |
|  OperationNotAllowed  |  Changing availability set SKU from 'Aligned' to 'Classic' isn't allowed.  |
|  OperationNotAllowed  |  Can't modify extensions in the VM when the VM isn't running.  |
...
|  OperationNotAllowed  |  The resource {0} can't be created from Image {1} until Image has been successfully created.  |
|  OperationNotAllowed  |  Updates to encryptionSettings isn't allowed when VM is allocated. Retry after VM is deallocated  |
|  OperationNotAllowed  |  Addition of a managed disk to a VM with blob based disks isn't supported.  |
|  OperationNotAllowed  |  The maximum number of data disks allowed to be attached to a VM of this size is {0}.  |
|  OperationNotAllowed  |  Addition of a blob based disk to VM with managed disks isn't supported.  |
|  OperationNotAllowed  |  Operation '{0}' isn't allowed on Image '{1}' since the Image is marked for deletion. You can only retry the Delete operation (or wait for an ongoing one to complete).  |
|  OperationNotAllowed  |  Operation '{0}' isn't allowed on VM '{1}' since the VM is generalized.  |
|  OperationNotAllowed  |  Operation '{0}' isn't allowed as Restore point collection '{1}' is marked for deletion.  |
|  OperationNotAllowed  |  Operation '{0}' isn't allowed on VM extension '{1}' since it's marked for deletion. You can only retry the Delete operation (or wait for an ongoing one to complete).  |
|  OperationNotAllowed  |  Operation '{0}' isn't allowed since the Virtual Machines '{1}' are being provisioned using the Image '{2}'.  |
|  OperationNotAllowed  |  Operation '{0}' isn't allowed since the Virtual Machine ScaleSet '{1}' is currently using the Image '{2}'.  |
|  OperationNotAllowed  |  Operation '{0}' isn't allowed on VM '{1}' since the VM is marked for deletion. You can only retry the Delete operation (or wait for an ongoing one to complete).  |
|  OperationNotAllowed  |  Operation '{0}' isn't allowed on VM '{1}' since the VM is either deallocated or marked to be deallocated.  |
|  OperationNotAllowed  |  Operation '{0}' isn't allowed on VM '{1}' since the VM is running. Please power off explicitly in case you shut down the VM from inside the guest operating system.  |
|  OperationNotAllowed  |  Operation '{0}' isn't allowed on VM '{1}' since the VM isn't deallocated.  |
|  OperationNotAllowed  |  Operation '{0}' isn't allowed on VM '{1}' since VM has extension '{2}' in failed state.  |
|  OperationNotAllowed  |  Operation '{0}' isn't allowed on VM '{1}' since another operation is in progress.  |
...
|  OperationNotAllowed  |  Disk with size {0}GB, which is smaller than the size {1}GB of corresponding disk in Image, isn't allowed.  |
...
|  OperationNotAllowed  |  VM '{0}' belongs to 'Classic' availability set '{1}'. Update the availability set to use 'Aligned' SKU and then retry the Conversion.  |
|  OperationNotAllowed  |  VM created from Image can't have blob based disks. All disks have to be managed disks.  |
|  OperationNotAllowed  |  Capture operation can't be completed because the VM isn't generalized.  |
...
|  OperationNotAllowed  |  Unable to resize the VM because the requested size {0} isn't available in the cluster where the availability set is currently allocated. The available sizes are: {1}. Read more on VM resizing strategy at [Troubleshoot allocation failures](./allocation-failure.md).  |
|  OperationNotAllowed  |  Unable to resize the VM because the requested size {0} isn't available in the cluster where the VM is currently allocated. To resize your VM to {1} please deallocate (this is Stop operation in the Azure portal) and try the resize operation again. Read more on VM resizing strategy at [Troubleshoot allocation failures](./allocation-failure.md).  |
...
|  OSProvisioningTimedOut  |  OS Provisioning for VM '{0}' didn't finish in the allotted time. The VM may still finish provisioning successfully. Check provisioning state later.  |
|  OSProvisioningTimedOut  |  OS Provisioning for VM '{0}' didn't finish in the allotted time. The VM may still finish provisioning successfully. Check provisioning state later. Also, make sure the image has been properly prepared (generalized).   <ul><li>Instructions for [Windows](/azure/virtual-machines/windows/upload-generalized-managed)</li><li> Instructions for [Linux](/azure/virtual-machines/linux/capture-image)</li></ul>  |
|  OSProvisioningTimedOut  |  OS Provisioning for VM '{0}' didn't finish in the allotted time. However, the VM guest agent was detected running. This suggests the guest OS has not been properly prepared to be used as a VM image (with CreateOption=FromImage). To resolve this issue, either use the VHD as is with CreateOption=Attach or prepare it properly for use as an image:   <ul><li>Instructions for [Windows](/azure/virtual-machines/windows/upload-generalized-managed) </li><li> Instructions for [Linux](/azure/virtual-machines/linux/capture-image)</li></ul>  |
|  OverConstrainedAllocationRequest  |  The required VM size isn't currently available in the selected location.  |
|  ResourceUpdateBlockedOnPlatformUpdate  |  Resource can't be updated at this time due to ongoing platform update. Try again later.  |
|  StorageAccountLimitation  |  Storage account '{0}' doesn't support page blobs which are required to create disks.  |
...
|  StorageAccountNotFound  |  Storage account {0} not found. Ensure storage account isn't deleted and belongs to the same Azure location as the VM.  |
|  StorageAccountNotRecognized  |  Please use a storage account managed by Storage Resource Provider. Use of {0} isn't supported.  |
...
|  StorageAccountTypeNotSupported  |  Disk {0} uses {1} which is a Blob storage account. Retry with General purpose storage account.  |
...
|  SubscriptionNotAuthorizedForImage  |  The subscription isn't authorized.  |
|  TargetDiskBlobAlreadyExists  |  Blob {0} already exists. Please provide a different blob URI to create a new blank data disk '{1}'.  |
|  TargetDiskBlobAlreadyExists  |  Capture operation can't continue because target image blob {0} already exists and the flag to overwrite VHD blobs isn't set. Either delete the blob or set the flag to overwrite VHD blobs and retry.  |
|  TargetDiskBlobAlreadyExists  |  Capture operation can't continue because target image blob {0} has an active lease on it.   |
|  TargetDiskBlobAlreadyExists  |  Blob {0} already exists. Please provide a different blob URI as target for disk '{1}'.  |
|  TooManyVMRedeploymentRequests  |  Too many redeployment requests have been received for VM '{0}' or the VMs in the same availabilityset with this VM. Retry later.  |
...
|  VMMarketplaceInvalidInput  |  Creating a virtual machine from a non Marketplace image doesn't need Plan information, please remove the Plan information in the request. OS disk name is {0}.  |
|  VMMarketplaceInvalidInput  |  The purchase information doesn't match. Unable to deploy from the Marketplace image. OS disk name is {0}.  |
...
|  VMNotFound  |  The VM '{0}' can't be found.  |
|  VMRedeploymentFailed  |  VM '{0}' redeployment failed due to an internal error. Retry later.  |
...
|  VMStartTimedOut  |  VM '{0}' didn't start in the allotted time. The VM may still start successfully. Check the power state later.  |
