...
- Network adapters
- Virtual network that the adapters are attached to
- All other adapters that are attached to that virtual network (and their VMs and disks)
...
Yes. The Azure resource move operation doesn't require that you stop or deallocate the VM. The VM continues to run and serve traffic normally. Both the source and destination resource groups are locked for write and delete operations during the move.

### How long does a move operation take?

Most moves finish within 30 minutes. Azure Resource Manager enforces a maximum of four hours. If the move doesn't finish within four hours, it times out and rolls back. For more information, see [Move operation times out after four hours](move-resources-4-hour-timeout.md).
...
Not by using the standard resource move API. Cross-region moves require [Azure Resource Mover](/azure/resource-mover/overview). The Azure Backup and Recovery Services team manages this process.
...
| `MoveResourcesHaveInvalidState` | [Move fails because of invalid or deleted storage account](move-resources-invalid-storage-account.md) |
