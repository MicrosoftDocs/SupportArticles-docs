...
For more information about how to use the run command feature to run scripts on the VM, see [Run PowerShell scripts in your Windows VM with run command](/azure/virtual-machines/windows/run-command).
...
* Outside the virtual network or deployment
  ...
  * For an Azure Resource Manager VM, first add a DNS name to the public IP address. For detailed steps, see [Create a fully qualified domain name in the Azure portal for a Windows VM](/azure/virtual-machines/create-fqdn). Then, run the following command:
