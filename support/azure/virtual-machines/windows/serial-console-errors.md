...
The storage account '' used for boot diagnostics on this VM couldn't be found. Verify that boot diagnostics is enabled for this VM, this storage account hasn't been deleted, and you have access to this storage account. | Double check that you haven't deleted the boot diagnostics storage account for your VM or virtual machine scale set
...
Serial console doesn't work with a storage account using Azure Data Lake Storage Gen2 with hierarchical namespaces. | This is an issue with hierarchical namespaces. Ensure that your VM's boot diagnostics storage account isn't created using Azure Data Lake Storage Gen2. This option can only be set upon storage account creation. You may have to create a separate boot diagnostics storage account without Azure Data Lake Storage Gen2 enabled to mitigate this issue.
...
The serial console shows the error message: **Another connection is currently in progress to this VM. Please wait and retry the request.** | This error indicates that another user might have the serial console open in the VM. If this isn't the case, the easiest way is to disable and re-enable boot diagnostics to restore Azure Serial Console access.
