...
This article helps you troubleshoot crashes on Azure Windows Virtual Machines (VMs) caused by an outdated Mellanox mlx5 network adapter driver. The crashes typically manifest as a blue screen with the bug check **DRIVER_IRQL_NOT_LESS_OR_EQUAL (0x000000D1)** and reference the `mlx5.sys` driver module.

## Symptoms

A Windows Azure VM can crash, restart unexpectedly, or become unstable.
...
> [!NOTE]
> This problem isn't caused by the Azure platform or host hardware failure. It's a guest OS driver compatibility issue and requires a driver update inside the VM.

## Quick check

This article likely applies if both of the following conditions are true:
