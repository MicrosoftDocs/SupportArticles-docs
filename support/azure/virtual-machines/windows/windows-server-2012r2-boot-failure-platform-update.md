...
description: Fix a Windows Server 2012 R2 VM that won't boot after an Azure platform update with INACCESSIBLE_BOOT_DEVICE. Follow this workaround to restore your VM.
ms.date: 04/09/2026
...
A Windows Server 2012 R2 VM that was previously running without problems fails to start after an Azure host platform update. When you use [Boot diagnostics](/azure/virtual-machines/boot-diagnostics) to view the screenshot of the VM, you see one of the following screens:
...
Windows Server 2012 R2 is no longer in Extended Support and its boot-time storage stack can't adapt to these newer ACPI device identifiers. During early startup, the OS fails to bind the storage driver correctly, which causes the INACCESSIBLE_BOOT_DEVICE error.

This problem affects only legacy guest OSs (Windows Server 2012 and Windows Server 2012 R2). Newer supported guest OS versions (Windows Server 2016 and later) aren't affected.
...
1. Take a snapshot of the affected VM's OS disk before proceeding.
1. Attach the OS disk as a data disk to the repair VM.
1. Connect to the repair VM by using Remote Desktop.
1. Note the drive letter assigned to the attached OS disk (for example, **F:**).
...
1. On the repair VM, create a script file at **C:\acpi_fix.cmd** with the following content. Replace **F:** with the drive letter of the attached OS disk by running the following command:
...
Perform the following steps to rebuild the affected VM so that it can boot from the OS disk that you repaired by using the repair VM:

1. Detach the OS disk from the repair VM.
1. Attach the disk back to the original VM as the OS disk.
1. Start the VM.

## Permanent resolution

Windows Server 2012 R2 exited Extended Support on October 10, 2023. Extended Security Updates (ESU), which might be active for your server, provide security patches only until October 13, 2026. ESU doesn't include bug fixes, compatibility updates, or platform behavior changes. Microsoft doesn't plan to release a permanent OS-level fix for this compatibility issue.
