...
---
# Troubleshoot Mellanox mlx5 driver crashes on Azure Windows virtual machines
...
A Windows Azure VM might crash, restart unexpectedly, or become unstable. Common signs include:
...
This problem occurs when the guest VM uses an outdated Mellanox mlx5 network adapter driver. The `mlx5.sys` driver is used on VM versions with **Accelerated Networking** that rely on Mellanox/NVIDIA ConnectX adapters.

If the installed driver version is no longer compatible with host firmware or platform updates, the driver can trigger a kernel memory access violation. In Windows, this error usually appears as bug check **0x000000D1 (DRIVER_IRQL_NOT_LESS_OR_EQUAL)**.

> [!NOTE]
> This problem isn't caused by the Azure platform or host hardware failure. It's a guest OS driver compatibility problem and requires a driver update inside the VM.
...
- Mellanox mlx5 driver versions that aren't updated in more than 12 months.
...
If the command returns events and the timestamps match the crash incidents, you confirm the pattern.
...
After you update the driver and restart the VM, run the validation script again to confirm that:
