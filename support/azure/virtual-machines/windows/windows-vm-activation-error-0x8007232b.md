...
description: "Resolve error 0x8007232B when activating your Azure Windows VM. Troubleshoot DNS name issues with step-by-step solutions."
ms.date: 04/14/2026
...
This error occurs when the KMS client can't find the Key Management Services (KMS) Service Location (SRV) records in DNS. The Windows activation client attempts to locate a KMS server by querying DNS for `_vlmcs._tcp` SRV records. If no KMS SRV records are published in DNS, activation fails.
...
If you use a self-hosted KMS server, make sure you publish KMS SRV records in DNS.

1. On the KMS host server, open an elevated Command Prompt and verify the KMS service is running by running the following command:
