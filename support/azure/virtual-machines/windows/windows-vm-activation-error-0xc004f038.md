...
description: Fix error 0xC004F038 when you activate an Azure Windows VM by correcting KMS settings and activation count thresholds. 
ms.date: 04/14/2026
...
This article explains how to resolve error 0xC004F038\ that occurs when you try to activate a Windows virtual machine (VM) in Microsoft Azure.
...
This error occurs if the KMS host count isn't high enough to activate the client. The KMS activation threshold requires a minimum number of unique computers that request activation.
...
You most commonly see this error in environments that use a self-hosted KMS server instead of the Azure KMS service. Azure VMs that use the default Azure KMS endpoints (`azkms.core.windows.net`) typically don't encounter this error because the Azure KMS service handles activation counts automatically.

## Scenarios in which this error occurs on Azure VMs
...
- You manually changed the VM KMS configuration to point to a KMS server that doesn't meet the activation threshold.
...
1. Add more computers to the KMS pool until the count reaches the required threshold (five for server, 25 for client).
