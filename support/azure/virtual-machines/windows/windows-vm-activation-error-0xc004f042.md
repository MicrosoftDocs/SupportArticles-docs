---
title: Error 0xC004F042 The specified Key Management Service (KMS) can't be used
description: Learn how to resolve the 0xC004F042 error that occurs when you try to activate an Azure Windows virtual machine (VM).
ms.date: 04/14/2026
...
---
# Error 0xC004F042 "The specified Key Management Service (KMS) can't be used"

**Applies to:** :heavy_check_mark: Windows VMs

This article discusses how to resolve the 0xC004F042 error that occurs when you try to activate a Windows virtual machine (VM) in Microsoft Azure.
...
- The VM is configured to contact a self-hosted KMS server that doesn't support the Windows edition installed on the VM.
- The KMS host key doesn't match the product being activated (for example, a Windows Server 2016 KMS key trying to activate a Windows Server 2022 client).
- The VM was migrated from an on-premises environment where a different KMS infrastructure was in use.
...
3. Verify the VM has the correct [KMS client setup key](/windows-server/get-started/kms-client-activation-keys) for your Windows edition. First, check the current configuration:

   ```cmd
   cscript c:\windows\system32\slmgr.vbs /dlv
   ```

4. If the product key doesn't match the installed edition, install the correct KMS client setup key:
...
2. If you use separate KMS hosts for different applications or OS versions, ensure the client is pointed at the correct one:
