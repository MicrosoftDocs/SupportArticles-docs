...
description: Troubleshoot Azure Instance Metadata Service connection issues on Windows VMs and fix firewall, proxy, and routing blockers. Resolve IMDS access now.
...
This article helps you troubleshoot and resolve connectivity problems between an Azure Windows VM and the Instance Metadata Service (IMDS) endpoint. IMDS connectivity is essential for VM activation, Extended Security Updates (ESU), and applications that depend on VM identity.

> [!NOTE]
> The [Azure Instance Metadata Service Verification Tool](windows-vm-imds-tool.md) Run Command can diagnose connectivity and certificate problems automatically. Run it first before manual troubleshooting.

> [!NOTE]
> If the IMDS endpoint is reachable but certificate chain validation is failing, see [IMDS certificate chain problems](windows-vm-imds-certissues.md) instead.
...
The Azure Virtual Machine can't establish a connection with the Azure Instance Metadata Service (IMDS) endpoint. 

* Applications or clients that depend on the IMDS service aren't working as expected.
* Windows Server 2022 or 2025 Azure Edition [activation watermark continues to be displayed](activation-watermark-appears.md).
* Extended Security Updates for Windows EOS versions might not install or function correctly.
...
> [!NOTE]
> Run the [IMDS Verification Tool](windows-vm-imds-tool.md) to diagnose connectivity and certificate issues.

> [!IMPORTANT]
> **Firewall vs. routing:** The Azure hypervisor at the host level handles IMDS traffic (`169.254.169.254`), not standard IP routing. A Windows Firewall outbound block rule on `169.254.169.254` can prevent IMDS access, but guest OS route table changes (such as `route add` or `route delete`) can't block IMDS. If IMDS is unreachable, check Windows Firewall rules and proxy settings rather than only the routing table.
...
1. Check whether the MAC and primary private IP addresses that Azure uses for the VM match the MAC address and the primary private IP address that the guest OS of the VM actually uses (the addresses that you found in the earlier step). To determine what Azure uses as the MAC address, use Azure CLI. To determine what Azure uses as the primary private IP address, examine the network configuration in the Azure portal.
...
- [IMDS Verification Tool](windows-vm-imds-tool.md)
