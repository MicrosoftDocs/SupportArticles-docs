---
title: Troubleshoot network security group misconfigurations blocking traffic in Azure virtual networks
description: "Diagnose and resolve NSG misconfigurations blocking traffic in Azure virtual networks. Learn to identify priority conflicts, service tag issues, and fix connectivity problems."
...
When you apply NSGs at both the subnet and network interface (NIC) levels, both NSGs must allow the traffic for it to flow successfully. If either NSG has a deny rule that matches the traffic, the traffic is blocked.
...
Traffic is blocked even though an allow rule exists for the port and protocol.
