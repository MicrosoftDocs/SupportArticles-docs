...
For all VPN Gateway logs, see [Azure VPN Gateway monitoring data reference](/azure/vpn-gateway/monitor-vpn-gateway-reference#resource-logs)
...
- If you observe a disconnection event on one gateway instance, followed by a connection event on a different gateway instance within a few seconds, it indicates a gateway failover. Such an event typically arises due to maintenance on a gateway instance. To learn more about this behavior, see [About Azure VPN gateway redundancy](/azure/vpn-gateway/vpn-gateway-highlyavailable#activestandby).
- The same behavior is observed if you intentionally run a **Gateway Reset** on the Azure side - which causes a reboot of the active gateway instance. To learn more about this behavior, see [Reset a VPN Gateway](/azure/vpn-gateway/reset-gateway).
...
[Default IPsec/IKE parameters](/azure/vpn-gateway/vpn-gateway-about-vpn-devices#ipsec) lists the IPsec parameters supported by the Azure Gateway with default settings.
...
To configure alerts on tunnel resource logs, see [Set up alerts on VPN Gateway resource logs](/azure/vpn-gateway/monitor-vpn-gateway-reference).