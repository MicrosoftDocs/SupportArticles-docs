...
For default policy configuration, check [configuration of IPsec/IKE connection policies](/azure/vpn-gateway/ipsec-ike-policy-howto) for site-to-site VPN & VNet-to-VNet to ensure the configuration on the tunnel of the on-premises VPN device has the matching configuration.
...
* Configure policy-based traffic selector on the connection resource in Azure to keep the same configuration as on-premises device traffic selector. For more information, see [Connect VPN gateways to multiple on-premises policy-based VPN devices](/azure/vpn-gateway/vpn-gateway-connect-multiple-policybased-rm-ps#create-the-virtual-network-vpn-gateway-and-local-network-gateway).
...
1. Ensure on-premises device is among one of the supported devices. See [About VPN devices for connections](/azure/vpn-gateway/vpn-gateway-about-vpn-devices#devicetable).
...
1. For default DH group configuration, check the [configuration of IPsec/IKE connection policies for S2S VPN & VNet-to-VNet](/azure/vpn-gateway/ipsec-ike-policy-howto) to ensure the configuration on the tunnel of the on-premises VPN device has the matching configuration.
...
Update the connection mode property on the connection resource in Azure to **Default** or **Responder only**. For more information, see [Connection mode](/azure/vpn-gateway/vpn-gateway-about-vpn-gateway-settings#connectionmode) settings.

## Next steps

For more information about VPN Gateway troubleshooting, see [Troubleshooting site-to-site connections](/azure/vpn-gateway/vpn-gateway-troubleshoot-site-to-site-cannot-connect).