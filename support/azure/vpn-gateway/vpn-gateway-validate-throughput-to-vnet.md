...
* On-premises VPN device (View a list of [validated VPN devices](/azure/vpn-gateway/vpn-gateway-about-vpn-devices#devicetable).)
* Public internet
* Azure VPN gateway
* Azure VM
...
1. Determine your Azure VPN gateway throughput limits. For help, see the "Gateway SKUs" section of [About VPN Gateway](/azure/vpn-gateway/vpn-gateway-about-vpngateways#gwsku).
...
If your calculated throughput doesn't meet your application's baseline throughput requirements, you must increase the bandwidth of the resource that you identified as the bottleneck. To upgrade an Azure VPN Gateway, see [Upgrade a gateway SKU](/azure/vpn-gateway/gateway-sku-upgrade). To resize a virtual machine, see [Resize a VM](/azure/virtual-machines/resize-vm). If you aren't experiencing the expected Internet bandwidth, you can also contact your ISP.
...
> [!Note]
> Iperf is not the only tool. [NTTTCP is an alternative solution for testing](/azure/virtual-network/virtual-network-bandwidth-testing).
...
> If there are poor results (in terms of overall throughput) coming from the iPERF/NTTTCP tests above, refer to [TCP/IP performance tuning for Azure VMs](/azure/virtual-network/virtual-network-tcpip-performance-tuning) to understand the key factors behind the possible root causes of the problem:
