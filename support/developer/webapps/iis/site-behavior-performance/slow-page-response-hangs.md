...
> [!NOTE]
> Some of these [email template]() instruct you to collect Perfmon traces as well. It's advised to skip that step for now and collect later if absolutely determined to be necessary.
...
> [!IMPORTANT]
> When setting up FREB for time taken, adjust the time taken trigger depending on how slow responses are. This will ensure only problematic requests are captured and will minimize false positives.
...
If slowness is accompanied by high CPU, avoid collecting logs when CPU usage is extremely high, like (>=95), as this might cause the server to hang or fail to collect logs due to insufficient CPU resources.
...
If the customer can't trigger the request (for example, the client is a mobile app and customer doesn't have access to it and the issue can't be just reproduced from browser), but by some means knows when the issue is occurring, follow these steps to confirm slowness:
