---
title: Error on .NET client that consumes Web service
description: Provides the resolution to solve the error message that occurs on a .NET client that consumes a Web service via an HTTP proxy server.
ms.date: 05/06/2020
ms.prod-support-area-path: 
ms.reviewer: ryankiv, sulabhs
---
# Error occurs on .NET client that consumes a Web service through an HTTP proxy server

This article provides the information to solve the **The underlying connection was closed: The remote name could not be resolved** error message that occurs on a .NET client.
