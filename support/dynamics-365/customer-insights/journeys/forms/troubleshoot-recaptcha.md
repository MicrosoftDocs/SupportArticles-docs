...
description: reCAPTCHA can block form submissions in Dynamics 365 Customer Insights - Journeys. Learn how to fix key mismatches and invalid domain errors now.
ms.date: 08/10/2026
...
1. Confirm the domain that hosts your form is added to your project in the [Google reCAPTCHA admin console](https://www.google.com/recaptcha/admin).
...
1. Add the domain exactly as it appears in your browser's address bar, without `https://` or `http://`. For example, enter `contoso.com`.
1. Save your changes.
...
| `Invalid domain for site key` | The hosting domain isn't registered with your site key. | Add the domain in the Google reCAPTCHA admin console. |
...
If you're an admin, check the saved values without opening the form settings. In a browser, go to `<environmentURL>/api/data/v9.1/msdynmkt_formsettings(<form settings ID>)` and confirm that:
