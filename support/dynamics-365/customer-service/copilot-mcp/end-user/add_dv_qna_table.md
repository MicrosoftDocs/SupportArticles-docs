...
**Yes. Adding a table changes your environment's configuration.** It creates a binding so the skill can answer from the table. It does not change the table's own records.
...
| `destructiveHint` | `false` | The tool creates a binding; it does not delete data. |
...
Creates a `dvtablesearchentity` binding row. It does not modify the bound table's own records.
