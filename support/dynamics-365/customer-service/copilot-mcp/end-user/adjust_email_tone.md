...
**No, adjusting the tone does not change data.**
...
The tool itself is marked with `readOnlyHint: true` and does not write to Dataverse on its own. The rewrite shown in chat is a preview.
