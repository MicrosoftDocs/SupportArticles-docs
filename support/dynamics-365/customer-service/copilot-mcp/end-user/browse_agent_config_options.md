...
**No, browsing configuration options does not change data.**
...
| Purpose | Guides a maker through agent configuration options via an interactive, multi-turn navigable tree |
...
|---|---|---|
| `path` | `path` (string, optional). Dot-delimited path to jump to any level of the configuration tree (e.g., `grid.incident`, `form`, `timeline.incident`, `tool-selection`, `mcp-registration`, `copilot-studio`, `cross-namespace-grant`). Omit or pass empty for the root menu. | No |
...
| Input | Description | Required |
|---|---|---|
| `search` | `search` (string, optional). Free-text search across category paths, entity names, override-type labels, and capability surfaces. Returns a flat list of matching nodes instead of tree navigation. | No |
