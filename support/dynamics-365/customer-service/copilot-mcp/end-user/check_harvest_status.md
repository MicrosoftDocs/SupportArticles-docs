...
**No, checking harvest status does not change Dynamics 365 data.**
...
- The user explicitly asks whether a harvest is ready or complete (e.g. 'is the article ready', 'check harvest status')
...
**Read-only.** Reads job status only; does not modify records.
