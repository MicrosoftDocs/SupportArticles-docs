...
Once you're happy with the template, click Save in the form to create it in Dynamics 365. The template is then available for future use when composing emails or applying templates to cases.
...
The creation form itself is read-only until you click Save. Once saved, the template is stored in Dynamics 365 and available for future use.
...
The tool is marked `readOnlyHint: true` and only opens the form widget. The actual Dataverse write occurs when the user clicks Save, which triggers `save_email_template`.
