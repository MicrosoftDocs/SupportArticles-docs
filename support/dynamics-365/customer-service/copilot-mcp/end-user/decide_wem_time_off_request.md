...
> [!NOTE]
> This tool is available by default on the following MCP servers: Dynamics 365 CX MCP Server - WEM. [Learn more about adding this tool to other MCP servers](/dynamics365/customer-service/administer/configure-customer-service-mcp-server).
...
- Use "show my pending time-off approvals" first (via the WEM request list) to see what's waiting on you.
...
| `requestName` | The exact name of the time-off request, e.g. "Time off on 1/1/2027". Used when `requestId` is omitted. | No* |
| `agentName` | The name of the agent who requested the time off, e.g. "agent1". Combine with `date` to pick one request. | No* |
...
Updates the `msdyn_wemrequest` time-off record's status to approved or rejected and records the reviewer. A rejection also saves the reviewer note. The tool refuses non-time-off records and will not overwrite a decision already made by another reviewer.
