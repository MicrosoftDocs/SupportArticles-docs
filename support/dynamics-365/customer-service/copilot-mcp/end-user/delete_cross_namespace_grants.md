...
## Helpful tips
- Use `level: org` to clear organization-wide grants.
...
## Tool behavior
Removes all cross-namespace grants at org or profile level, returning the agent to its native-namespace tools only. Requires confirmation. Profile level requires the service app.
...
|---|---|---|
| `level`, `org`, `profile` | `level` (`org` or `profile`, required). Chooses the scope to clear. Profile is service-app only. | Yes |
