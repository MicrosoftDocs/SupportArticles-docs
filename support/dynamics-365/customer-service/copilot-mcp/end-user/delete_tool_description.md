...
This tool deletes all org-level tool description overrides. The original tool descriptions are restored automatically on the next request — no further action is required.

## Try prompts like

- "Delete all tool description overrides"
- "Remove the tool description cues"
- "Clear the description overrides for this org"
...
- Pass `toolName` to remove the override for a single first-party tool. Omit `toolName` to remove all org-level overrides at once.
- Changes propagate within a few minutes after deleting.
- Use `list_tool_description` before deleting to review what will be removed.
...
Deletes org-level tool description override patches. Pass `confirm=true` to execute. Optionally pass `toolName` to remove a single tool's override; omit it to remove all org-level overrides. The affected original tool descriptions are automatically restored on the next request — no further action is required.
...
| `destructiveHint` | `true`  | Deletes all stored tool description overrides for the org.                |
...
- **`toolName`** (optional) — the exact 1P tool name whose override to remove (e.g. `draft_email`). Omit it to remove all org-level overrides.
...
| Tool                                                            | Relationship                                                    |
| --------------------------------------------------------------- | --------------------------------------------------------------- |
| [`list_tool_description`](list_tool_description.md)             | Lists overrides before deleting to confirm what will be removed |
...
Removes all tool description override patches at org level.
