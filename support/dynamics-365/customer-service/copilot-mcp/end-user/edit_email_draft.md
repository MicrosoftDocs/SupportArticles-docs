...
**This step opens an existing draft but does not change data by itself.**
...
The tool is marked `readOnlyHint: true`. The compose widget manages save and send operations — updates are applied to the same draft record in Dataverse when the user clicks Save or Send.
