...
**No, viewing a request does not change data.**
...
This tool is available on the Dynamics 365 CX MCP Server - WEM. See the availability note at the top of this page for details. Workforce Engagement Management must be enabled, and the signed-in user must have read access to WEM requests.
...
| `requestName` | The exact request name (`msdyn_name`, e.g. "Weekend cover swap"). Used to resolve the record when `recordId` is omitted. When the name matches more than one request, the matching requests are shown in a grid so you can click the one to open; agent-based lookup is not supported here (use `list_wem_requests` to find an agent's requests). | No* |
...
The form view does not change data.
