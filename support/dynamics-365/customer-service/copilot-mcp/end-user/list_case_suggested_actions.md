...
The grid is read-only. Clicking a row to take the suggested action (replying, closing, etc.) uses separate tools that may change data. The assistant asks for confirmation before making changes.
...
The listing itself does not change data. Clicking a row triggers a follow-up action (reply, draft email, close, etc.) that can change data via separate tools.
