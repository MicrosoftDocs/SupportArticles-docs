...
## What it does
The assistant shows cases that are assigned to you where the Case Management Agent (CMA) has escalated to a human. These are cases where the AI attempted to handle the case autonomously but reached a point where it needs human intervention. Each case shows why the AI escalated (the fallback reason) and when the escalation happened, ordered by most recent first.

## Try prompts like
- "Show me cases the AI has escalated"
- "Which cases is the AI stuck on?"
