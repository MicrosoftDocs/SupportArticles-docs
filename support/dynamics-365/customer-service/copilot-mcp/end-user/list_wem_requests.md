...
**No, listing requests does not change data.**
...
|---|---|---|
| `requestType` | Array of `time_off`, `shift_swap`, `shift_bid`. Omit to include all types (e.g. "show all requests"); pass e.g. `['time_off']` for time-off only, `['shift_swap']` for swaps. | No |
...
The listing does not change data; opening a request shows its record in a form view.
