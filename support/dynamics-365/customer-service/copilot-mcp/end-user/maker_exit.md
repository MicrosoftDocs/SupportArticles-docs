...
## What it does
When you are done configuring the agent in maker mode, this exits that mode and restores the standard tool set. On the next turn, the agent returns to its normal service-representative capabilities.

> [!NOTE]
> Due to the agent's tool cache, there may be a one-turn delay before the standard tools are fully restored.
