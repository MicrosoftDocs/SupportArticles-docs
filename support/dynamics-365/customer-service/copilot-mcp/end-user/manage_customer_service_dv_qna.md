...
Opening the management panel does not change data. Adding or removing a table from within the panel does change which tables the skill can answer from.
...
Listing the scope does not change data. Mutations happen only through the companion `add_dv_qna_table` and `remove_dv_qna_table` tools invoked from the widget.
