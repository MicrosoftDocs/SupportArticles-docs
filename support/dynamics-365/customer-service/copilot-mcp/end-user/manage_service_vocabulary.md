...
**Opening the editor does not change data. Saving or deleting an entry does.**
...
|------------|-------|---------|
| `readOnlyHint` | `true` | The launcher itself only reads; edits happen via the companion tools. |
...
Opening the editor does not change data. The add, edit, and delete actions from the editor write configuration via separate tools.
