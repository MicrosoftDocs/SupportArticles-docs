...
Because merging can't be undone, the assistant first shows a preview of exactly what will move and warns you if any case belongs to a different customer. Nothing changes until you confirm.
...
## Tool behavior
Merges duplicate cases into a surviving case by using the Dataverse Merge action. Runs in two phases: without `confirmed` (or `confirmed: false`) it returns a read-only preview of what will move and flags any cross-customer case; with `confirmed: true` it merges each case independently and returns a per-case outcome. Each merge is applied one case at a time; a failure on one case is reported without stopping the others.
...
| `idempotentHint` | `false` | Re-running does not repeat cleanly; an already-merged case is inactive and can't be merged again. |
...
|---|---|---|
| `survivingCaseId` | The case that stays active and receives the merged history. GUID or case number (for example, CAS-01010-A0A0A0). The user must choose this - the assistant does not infer it. | Yes |
