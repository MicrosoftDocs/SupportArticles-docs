...
**No, predicting fields does not change data.**
...
Learn more in [Configure generative AI features](/dynamics365/customer-service/administer/configure-generative-features).
...
| Purpose | Predicts values for caller-requested fields — direct case attributes or a related record's field via a dotted path — from case context, with reasoning, and suggests applying them via the appropriate update tool |
...
The prediction itself does not change data. The user may apply a predicted value via an update tool — `update_case` for a direct case field, or the related record's own update tool for a dotted related-record field — which changes data and confirms first.
