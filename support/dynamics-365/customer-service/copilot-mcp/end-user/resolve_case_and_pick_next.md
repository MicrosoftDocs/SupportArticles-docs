...
The assistant shows a text response with two parts: confirmation that the first case was resolved, and the details of the newly picked case (case number, title, priority, source queue). If no queue items remain, the assistant says so.
...
- At least one active queue with cases to be configured. The pick-next step silently succeeds (returns no new case) if no queue items are available
