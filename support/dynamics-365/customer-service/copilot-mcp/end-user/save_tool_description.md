...
Use this capability when you want to append a routing cue to a first-party tool's description so that a customer's equivalent third-party (3P) tool becomes the primary orchestrator choice, with the first-party tool acting as a fallback.

## What it does

This tool saves org-level tool description overrides. Each override appends a sanitized cue to the original first-party tool description. When a paired `primaryToolName` is specified, the override is applied only when that 3P tool is also registered in the same request (conditional demotion).
...
The assistant confirms how many overrides were saved and shows a preview of the appended cue for each patch (showing what text will be appended at runtime). Warnings are shown if unknown tool names or non-3P `primaryToolName` values are provided.

## Helpful tips

- Only first-party tool names can be targeted. Customer 3P tools (`mcs_*`/`ext_*`) are not supported as targets.
- `primaryToolName` should be a customer 3P tool name (`mcs_*` or `ext_*`) to enable conditional demotion.
...
It saves tool description overrides in the org-level configuration.
...
Saves org-level tool description override patches. Each patch appends a sanitized cue to the original first-party tool description. When a `primaryToolName` (`mcs_*`/`ext_*`) is specified, the override is applied at runtime only when that 3P tool is also registered in the same request (conditional demotion). Rejects customer 3P tool names as override targets.
...
| `destructiveHint` | `false` | Merges or upserts patches; does not delete existing overrides.            |
...
| Input             | Description                                                                               | Required |
| ----------------- | ----------------------------------------------------------------------------------------- | -------- |
| `primaryToolName` | Customer 3P tool name (`mcs_*`/`ext_*`) — override applies only when this tool is present | No       |
...
No interactive component is rendered. Returns a confirmation of how many patches were saved with a preview of each composed description and any warnings for unknown tool names or non-3P primary tool names.
...
- The user wants to steer the model toward a customer 3P tool
...
Creates or updates tool description override patches at org level.
