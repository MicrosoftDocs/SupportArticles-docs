...
description: Solves the errors that occur when updating old cases and entities in Microsoft Dynamics 365 Customer Service.
ms.date: 06/05/2024
...
You can't update cases and entities associated with an old SLA in a Unified Interface SLA, and you receive the one of the following error messages:
...
Make sure that new entity records use the new Unified Interface SLAs. For custom logic, for example, workflows or plugins, refer to Unified Interface SLAs. Set the default SLAs option to Unified Interface SLAs, so that legacy SLAs don't get applied to the new entity records.

For more information, see [Apply SLAs](/dynamics365/customer-service/administer/apply-slas?tabs=customerserviceadmincenter).
