...
After an optimization runs, Resource Scheduling Optimization can decide to create, update, or delete bookings based on objective and constraint settings. Therefore, you might find that the system deleted bookings to optimize results throughout the schedule. If you don't want Resource Scheduling Optimization to delete existing bookings, try the following options:
...
- If you set the booking status to **Ignore**, Resource Scheduling Optimization doesn't consider that booking record. Use this option for booking statuses that indicate work got canceled.
...
During the optimization process, the create, update, and delete operations are visible. If a booking needs to be updated, the system tries to make that change. If the update fails because the booking changed after the optimization run starts, Resource Scheduling Optimization creates a simulated booking instead.
