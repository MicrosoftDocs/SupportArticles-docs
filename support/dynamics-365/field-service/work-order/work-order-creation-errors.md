...
description: Troubleshoot work order save and creation errors in Dynamics 365 Field Service. Fix plug-in failures, required field errors, and incident type issues with these step-by-step solutions.
ms.date: 04/01/2026
...
This article helps you troubleshoot common errors that occur when you save or [create work orders](/dynamics365/field-service/create-work-order) in Microsoft Dynamics 365 Field Service. It covers plug-in and business rule failures, required field errors caused by custom scripts, and incident type auto-population issues.

## Symptoms

When you try to save or create a work order, you notice one of the following problems:
...
To identify and fix the failing plug-in or business rule:
...
1. Open the browser developer tools (<kbd>F12</kbd>) and check the **Console** tab for JavaScript errors during the save operation.
1. Go to [Power Apps](https://make.powerapps.com) and select your environment.
...
1. If the required field is a lookup, open the referenced record in the Dynamics 365 Field Service app and confirm that it still exists and is in an **Active** state.
1. Test in a clean environment without customizations to confirm whether the issue is caused by custom logic.

## Confirm the incident type configuration and user privileges
...
1. Open the incident type record and confirm that **Products**, **Services**, and **Service Tasks** appear under the related tabs.
1. Confirm that each product or service referenced in the incident type is **Active** in the product catalog:
    1. Go to **Settings** > **General** > **Products** to check product status.
1. On the work order form, confirm that the **Price List** field is populated with a valid price list that contains the products:
    1. Go to **Settings** area > **General** > **Price Lists** to check the price list.
1. Confirm the user has the required [security roles and privileges](/dynamics365/field-service/security-permissions):
    ...
    1. Confirm the user's role has **Create** privilege on:
...
1. If you use custom business rules that hide or lock the **Incident Type** field, confirm that they allow the system to trigger the auto-population logic.
