...
Advanced rules apply additional constraints on top of the base account structure. Account structures use the most restrictive combination of all applicable constraint nodes.

1. On the **Configure account structures** page, select **Advanced rules** for the relevant account structure.
1. Review each rule to determine whether any rule further constrains the valid values beyond what the base account structure allows.
1. Verify that no advanced rule was saved without a criteria filter, which would cause the rule to always apply an unexpected constraint.

### Verify that the dimension value is active and not suspended
...
1. Locate the value from the error message and confirm the following conditions:
   ...
   - If legal entity overrides are configured, verify that the value isn't suspended or inactive for the specific legal entity where the transaction is entered.

### Check for fixed dimensions

A [fixed dimension](/dynamics365/finance/general-ledger/dimensions-default-values) value on a main account replaces whatever value is entered on the transaction line, even if that means forcing a blank value. If the account structure requires a nonblank value and the fixed dimension is blank, the validation fails.

1. Go to **General ledger** \> **Chart of accounts** \> **Accounts** \> **Main accounts**.
1. Select the main account that's used in the transaction.
1. On the **Legal entity overrides** FastTab, check whether a fixed dimension value is set that conflicts with the value you entered or that forces a blank where one isn't allowed.
...
1. Check the journal header default dimensions that might be automatically applied.
1. Review the derived dimensions setup against each segment in the ledger account.
1. Check whether interunit dimensions are enabled that might cause the system to validate a different main account or dimension value than the one you believe you entered.
1. Update your default dimension setup accordingly.
