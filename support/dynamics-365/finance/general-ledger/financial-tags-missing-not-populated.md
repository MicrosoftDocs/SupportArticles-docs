...
description: If financial tags are missing in Dynamics 365 Finance journals, postings, or imports, learn how to troubleshoot caching, Feature management, and data entity mappings to fix the issue now.
ms.date: 04/14/2026
...
This article provides troubleshooting steps to resolve issues in which [financial tags](/dynamics365/finance/general-ledger/financial-tag) are missing, empty, or show incorrect values in Microsoft Dynamics 365 Finance. Financial tags might not appear on journal forms, might be empty after a [data management import](/dynamics365/fin-ops-core/dev-itpro/data-entities/data-entities-data-packages#import), or might be missing from voucher transactions after they're posted. These issues can be caused by browser or server caching, the [Feature management](/dynamics365/fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview) configuration, or data entity field mappings.
...
1. Temporarily rename one of the existing financial tags. This action triggers an update to the financial tag configuration, and clears the cached values.
1. Open a journal entry form, and verify that the financial tag columns are visible.
1. Revert the financial tag to its original name.
...
1. Verify that the financial tag columns in your source file are correctly mapped to the corresponding financial tag fields in the data entity. Financial tag fields in the data entity are named **Financial tag**, **Financial tag 2**, **Financial tag 3**, and so on. The names match the order in which the tags are configured.
1. Verify that the source data file contains the correct financial tag values in the mapped columns.
...
1. Change the **Value type** field to one of the following options:
   - **Fixed list**: for system table-backed lists that use validation
   - **Fixed custom list**: for custom lists that use validation
1. Select **Activate** to activate the tag by using the new value type.
...
To check whether a data entity supports financial tags, follow these steps:
