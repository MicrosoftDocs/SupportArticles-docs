---
title: Unexpected financial dimension values on transactions
description: Troubleshoot incorrect financial dimension values after posting documents or transactions in Microsoft Dynamics 365 Finance.
ms.date: 04/02/2026
...
Fixed dimensions are a setting on a main account that forces specific dimension values on any transaction that posts to that account, regardless of what was entered on the document or journal line. This override happens at the moment of posting and can be unexpected if you aren't aware the setting is enabled.

To resolve this issue, check whether fixed dimensions are set up on the main account involved in the transaction. For detailed information about how fixed dimensions work and the order in which dimension default values are applied during posting, see [Financial dimensions and posting](/dynamics365/finance/general-ledger/default-dimensions). For information about account structures that govern which dimensions apply to main accounts, see [Configure account structures](/dynamics365/finance/general-ledger/configure-account-structures).

## Check the source of dimension default values

Financial dimension values are merged from multiple sources during document and journal processing. If a higher-priority source provides a value for a dimension, it overwrites what was entered or expected from a lower-priority source. The following sources can contribute dimension default values:
...
- **Master data**: Dimension values associated with records like customer accounts, vendor accounts, or workers are applied when those records are selected on a transaction line.
...
Work through each source to identify which one supplies the unexpected value, then correct it. For a comprehensive explanation of how default dimensions are applied and the order of priority, see [Financial dimensions and posting](/dynamics365/finance/general-ledger/default-dimensions).

## Check whether the document is a correction, reversal, or relieving transaction

Certain document types, like **Correction**, **Reversal**, and **Relieving** transactions, are designed to undo a previously posted entry. To make sure the reversal precisely mirrors the original, the system intentionally reuses the dimension values from the original posted transaction rather than remerging default dimension values from the current setup.
...
1. Open the transaction and confirm whether it's a reversal, correction, or relieving document. This information is often visible in the document header or transaction type field.
1. If it is, the dimension values are pulled from the original posted transaction to maintain accounting integrity.
...
If you've ruled out fixed dimensions and the behavior doesn't occur in a standard demo environment, a customization is likely involved.
...
1. Ask whether any recent updates, deployments, or code changes were made before the issue started.
