...
description: Source record not found errors in Dynamics 365 Finance financial dimensions? Learn how to fix missing records, legal entity mismatches, XDS policies, and hidden characters blocking dimension values.
ms.date: 04/02/2026
...
A customization or integration tries to assign a financial dimension value by using a record that doesn't exist in the system, exists in a different legal entity than expected, or references the wrong record type.

This problem can occur in several scenarios:

- **The record doesn't exist.** The process tries to use a value (such as a vendor or customer account number) as a dimension, but the value hasn't been created in the corresponding master data table, or it was deleted.
- **The record exists in a different legal entity.** The process looks for the record in one company, but the record only exists in a different company. For example, a vendor account exists in company 001, but the system searches in company 002.
...
1. Verify that the master data record referenced in the error message (the value and table) exists in the correct legal entity.
...
The source record exists but belongs to a different legal entity (company) than the one where the process runs. Company-specific dimension values are only visible within their associated legal entity.
...
[Extensible Data Security (XDS)](/dynamics365/fin-ops-core/dev-itpro/sysadmin/extensible-data-security-policies) policies are enabled on the table or view that stores the financial dimension source records. XDS policies restrict which records a user can see, and these restrictions can prevent the financial dimension framework from finding the source record.

> [!IMPORTANT]
> XDS security policies aren't compatible with financial dimensions. Many system processes run under the current user's security context rather than a system administrator context. This condition means XDS restrictions can block dimension operations even when the underlying data exists.

In version 10.0.41 and later, the error message might indicate that XDS policies prevent resolution of the dimension value.

### Solution

1. Check whether XDS security policies are enabled on the table mentioned in the error message (such as `VendTable` or `CustTable`) or its corresponding dimension view (such as `DimAttributeVendTable` or `DimAttributeCustTable`).
1. If you find XDS policies on these tables or views, remove or adjust them so the financial dimension framework can access the required records. For more information, see [Bypassing XDS policy](/dynamics365/fin-ops-core/dev-itpro/sysadmin/extensible-data-security-policies#bypassing-xds-policy).
...
In version 10.0.42 and later, the error message might indicate that a partial match was found but the values don't match exactly due to string length or content differences.

### Solution

1. Go to the source record mentioned in the error message (for example, the customer or vendor record).
1. Select the key field (such as the account number) and temporarily change it to a different value, and then save. Change it back to the intended value and save again. This process removes any hidden characters. For guidance on renaming dimension values, see [Renaming financial dimensions](/dynamics365/finance/general-ledger/financial-dimensions#renaming-financial-dimensions).
1. After you correct the value, retry the operation that produced the error.
1. If your data comes from an external integration, review the integration to ensure it validates and cleans data before importing it into Dynamics 365 Finance.
