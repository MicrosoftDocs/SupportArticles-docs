...
Power Platform retired the legacy hierarchy control. This change removed the platform-level support for the linked icon in the **Hierarchy** column. The new visual hierarchy feature uses a different approach to show hierarchical data. Therefore, it doesn't include a column-level icon in views.
...
Use visual hierarchy to view and work with hierarchical data. The new feature offers customizable tiles, drill-down navigation, inline editing, and multitable support.

To open the visual hierarchy for a record:

1. Open the Dynamics 365 Sales app.
1. Go to the record that you want to view (for example, an account or contact record).
1. On the command bar, select **View hierarchy**. The hierarchy viewer opens and shows the record and its parent-child relationships in a visual tree layout.
   ...
   > [!NOTE]
   > If the **View hierarchy** button isn't visible, select **More commands** (⋮) on the command bar to find it. If the button is still not available, contact your administrator to enable visual hierarchies.
...
1. [Design and publish a hierarchy](/dynamics365/sales/create-activate-hierarchy-visualizations) for the tables that your organization uses.
