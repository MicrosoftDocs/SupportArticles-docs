...
Use this article to troubleshoot issues you might face when working with opportunities in Dynamics 365 Sales. It covers errors and unexpected behaviors when closing opportunities, editing closed opportunities, adding products, viewing the opportunity pipeline, and managing permissions and stakeholders. For more information on working with opportunities, see [Manage opportunities](/dynamics365/sales/opportunity-management-overview).

## Can't close an opportunity

The following errors can occur when you try to close an opportunity:

- [Active or draft quotes prevent closing the opportunity](#active-or-draft-quotes-prevent-closing-the-opportunity)
...
- [Currency mismatch error when closing an opportunity](#currency-mismatch-error-when-closing-an-opportunity)

The following sections describe each error and how to resolve it.

### Active or draft quotes prevent closing the opportunity

You get the following error message when you try to close an opportunity:

> The opportunity can't be closed

This error occurs because active or draft [quotes](/dynamics365/sales/create-edit-quote-sales) are associated with the opportunity.
...
Ask your system administrator to grant you the required permissions.

### The opportunity is already closed

The opportunity you're trying to close is already marked as Won or Lost.

To change a closed opportunity, reopen it, make your changes, and close it again.
...
- There are active customization [solution layers](/power-platform/alm/solution-layers-alm) on the opportunity close entity.
...
1. Check whether there are any active layers on the opportunity close entity. Remove these layers to validate the scenario.
...
1. Make sure the opportunity close customization is turned on in Sales Hub settings:
...
1. If you customized the opportunity close form with extra fields, make sure scripts or fields don't cause errors. Try removing customizations and close the opportunity again to isolate the issue.
...
To resolve this problem, remove one of the fields that has a cascading relationship with the same entity from the opportunity close form. For example, if a custom field and the **Regarding** field both reference the same entity, remove one of them. For more information, see [Create and edit 1:N (one-to-many) relationships](/dynamics365/customerengagement/on-premises/customize/create-and-edit-1n-relationships).
...
### Currency mismatch error when closing an opportunity
...
1. Make sure the actual revenue currency matches the transaction currency of the opportunity.

## Can't edit an opportunity

After you close an opportunity as Won or Lost, it becomes read-only and you can't change it. To change a closed opportunity, reopen it.
...
The following errors can occur when you try to add products to an opportunity:

- [No price list selected for the opportunity](#no-price-list-selected-for-the-opportunity)
- [Missing product name for a write-in product](#missing-product-name-for-a-write-in-product)
- [Selected product is in Draft status](#selected-product-is-in-draft-status)

### No price list selected for the opportunity

You get the following error message when you try to add a product to an opportunity:
...
### Missing product name for a write-in product

You get the following error message when you try to add a write-in product:
...
### Selected product is in Draft status

You get the following error message when you try to add an existing product:

> You can only add active products

This error occurs because the product you selected is in the **Draft** status.

To resolve this problem, make sure the product is in the Active state, and then add it.

## Insufficient permissions or access denied when closing an opportunity (for administrators)
...
1. On the connection record, open the connection role listed in the **As this role** field.
1. On the connection role record, make sure the **Connection Role Category** field is set to **Stakeholder**.

## Pipeline view doesn't display bubbles in the Deal tracker

The bubbles might not be visible for the following reasons:

- The [bubble chart](/dynamics365/sales/use-opportunity-pipeline-view#understand-the-opportunity-pipeline-charts) shows only the first 50 valid opportunity entries (those with defined x-axis, y-axis, radius data, and **Segment by** fields). As a result, some or all bubbles might not appear on the graph even though they're present in the grid view.
- If the [msdyn_Score](/dynamics365/sales/developer/entities/msdyn_predictivescore#BKMK_msdyn_Score) (opportunity score) and [msdyn_Grade](/dynamics365/sales/developer/entities/msdyn_predictivescore#BKMK_msdyn_Grade) (opportunity grade) fields are selected as the **Tooltips** fields for the Deal tracker, the "not null" filters of these fields are applied to the opportunities for the bubble chart, which might filter out all opportunities from the chart even though they're visible in the grid view.

To resolve this problem:

1. Make sure the fields selected for the **Deal tracker** setting in **App Settings** on the [opportunity pipeline settings page](/dynamics365/sales/opportunity-pipeline-view-for-admins#open-the-opportunity-pipeline-view-settings-page) have values for the x-axis, y-axis, radius data, and **Segment by** fields.
1. Avoid using the [msdyn_Score](/dynamics365/sales/developer/entities/msdyn_predictivescore#BKMK_msdyn_Score) (opportunity score) and [msdyn_Grade](/dynamics365/sales/developer/entities/msdyn_predictivescore#BKMK_msdyn_Grade) (opportunity grade) fields as the **Tooltips**, x-axis, y-axis, radius, or **Segment by** fields so the bubbles appear in the Deal tracker. Alternatively, [turn on predictive opportunity scoring](/dynamics365/sales/digital-selling-scoring#set-up-lead-and-opportunity-scoring) in the organization to populate data for the `msdyn_Score` and `msdyn_Grade` fields.

## Pipeline view doesn't show all the bubbles in the Deal tracker

This behavior is by design. The bubble chart shows a maximum of 50 opportunities to keep the view readable.
