...
description: Introduces a by-design behavior where all previous jobs that have a status of In progress are ended when using the job card device to report partial quantity on the last job in a production order.
