...
---
# Error AADSTS50107 when a guest user redeems invitation via SAML external trust

This article provides a solution to the Microsoft Entra authentication error AADSTS50107 that occurs when a guest user attempts to redeem an invitation from a tenant using Security Assertion Markup Language (SAML) external trust.
...
- A user is invited from an external organization to your tenant as a guest.
...
1. Verify or update the issuer URL in Microsoft Entra ID to align with the one in the SAML response.
    ...
    1. Check that the issuer URL configured in Microsoft Entra ID matches the `saml:Issuer` value in the SAML response sent by the external IdP.
