...
1. Decode the token by using [jwt.ms](https://jwt.ms), and take a note of the `kid`.
    ...
    :::image type="content" source="media/idx10501t-oken-signature-validation-error/kid-jwt-ms.png" alt-text="Screenshot of the kid by using jwt.ms.":::

1. Depend on your Microsoft Entra ID application version, use the access token as a Bearer token to call one of the following keys discovery endpoints. The API response will return three keys. The `kid` you obtained in step 2 should match one of the keys returned by the keys discovery endpoint.
    ...
    ```
    :::image type="content" source="media/idx10501t-oken-signature-validation-error/kid-endpoint.png" alt-text="Screenshot of the kid from key endpoints.":::
