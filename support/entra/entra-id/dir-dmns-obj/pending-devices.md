...
This article explains why a device might be in the pending state in Microsoft Entra ID and how to troubleshoot and resolve issues related to pending devices.

Pending devices are devices that sync to Microsoft Entra ID from your on-premises Active Directory but don't complete registration with the Microsoft Entra device registration service. When the registered state of a device is pending, the device can't complete any authorization or authentication requests, such as requesting a [Primary Refresh token](/azure/active-directory/devices/concept-primary-refresh-token) for single sign-on, or applying [device-based Conditional Access policies](/mem/intune/protect/create-conditional-access-intune).
...
When you configure a **Microsoft Entra hybrid join** task in the Microsoft Entra Connect Sync for your on-premises devices, the task syncs the device objects to Microsoft Entra ID and temporarily sets the registered state of the devices to "pending" before the device completes the device registration. This state exists because the device must be added to the Microsoft Entra directory before it can be registered. For more information about the device registration process, see [How it works: Device registration](/azure/active-directory/devices/device-registration-how-it-works#hybrid-azure-ad-joined-in-managed-environments).
...
Two scenarios can cause a device to get stuck in a pending state.
...
A new on-premises device can get stuck in a pending state if it can't complete the device registration process. Several factors can cause this problem, such as the device not connecting to the registration service.
...
### The state of a registered device changes to pending

This problem can occur in the following scenario:

1. You move the device object to another organizational unit (OU) that isn't in the sync scope in Microsoft Entra Connect Sync.
...
1. You move the device object back to the OU in the sync scope.
...
To fix the problem, unregister the device by running `dsregcmd /leave` at an elevated command prompt, and restart the device. The device reinitiates the device registration process through the scheduled task. For Windows 10-based devices, the scheduled task is under **Task Scheduler Library** > **Microsoft** > **Windows** > **Workplace Join** > **Automatic-Device-Join Task**.

## Get a list of pending devices

1. Install the [Microsoft Graph PowerShell SDK](/powershell/microsoftgraph/installation?view=graph-powershell-1.0&preserve-view=true) to run Microsoft Graph PowerShell commands.
