...
|Microsoft Azure AD Internal - Jit Provisioning|Microsoft Entra ID|Used when the service principal for a Microsoft service is automatically created or updated, usually in response to changes in a customer's subscriptions. These automatic service principal updates sometimes happen through a background asynchronous process, not necessarily immediately following a subscription event or change.| 
