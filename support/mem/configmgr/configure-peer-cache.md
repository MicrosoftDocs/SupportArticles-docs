...
1. In the Configuration Manager console, create a device collection. Add clients that you want to enable as peer cache sources to the collection.
...
1. Select **Client Cache Settings** from the navigation pane, set **Enable as peer cache source** to **Yes**, and then specify the ports.
...
1. Deploy this custom client setting to the device collection that's created in step 1.

You don't need to enable peer cache clients. When you enable clients to be peer cache sources, the management point includes them in the list of content location sources.
...
- In the WMI class instance `CCM_SuperPeerClientConfig.SiteSettingsKey=1` under `ROOT\ccm\Policy\Machine\ActualConfig`:
  ...
  The value of **CanBeSuperPeer** property is changed to **True**.
...
- A state message of topic type 7201 is raised. The following entries are logged in StateMessage.log:
...
When an application is deployed and installed on the peer cache source, the Content Access service raises a state message of topic type 7200. The following entry is logged in StateMessage.log:

```output
State message(State ID : 1) with TopicType 7200 and TopicId Cache add CAS00015.1 has been recorded for SYSTEM
```

The state message is sent to the management point via CCMMessaging.
...
ContentTransferManager.log also shows the content locations, which include the peer cache source and distribution points:
...
Peer cache clients prioritize peer cache sources for downloading content. It's shown in the following entry in DataTransferService.log:
