...
- Installs MSXML and the VC Redistributable components
...
If the DP is configured to be a Pull DP, t he DistMgr Upgrade Processing thread starts another DP upgrade worker thread to perform the pull DP installation. This DP upgrade worker thread instructs the SMS DP Provider to run `pulldp.msi` to install the pull DP.
