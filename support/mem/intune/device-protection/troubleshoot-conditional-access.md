...
- Desktop applications must use modern authentication methods, which rely on displaying an auth promot in a web browser or in an authentication broker. Scripts and automation that [send the password directly](/azure/active-directory/develop/v2-oauth-ropc) can prove device identity only if they use an authentication broker, currently only on Windows. 
