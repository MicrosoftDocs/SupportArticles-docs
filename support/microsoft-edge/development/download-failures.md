...
description: Resolve "Couldn't download" errors in Microsoft Edge with this step-by-step troubleshooting guide. Fix SmartScreen blocks, permission issues, and network problems quickly.
ms.date: 03/31/2026
...
This article helps you troubleshoot download failures in Microsoft Edge when you see error messages like "Couldn't download" or similar notifications. Downloads might fail because of SmartScreen or antivirus blocks, Group Policy restrictions, network problems, or file system permission issues. Use the following checklist to identify and fix the problem.

## Symptoms

You experience one or more of the following problems:

- Downloads fail with the error message "Couldn't download."
- A download starts but immediately fails or gets interrupted.
- The download progress bar reaches a certain percentage and then stops with an error.
- Specific file types fail to download while others succeed.
- Downloads fail only on certain websites but work on others.

## Solution

Work through the following checks in order. After each section, retry the download to check whether the problem is fixed before you move to the next section.

### Check the specific download error message

Microsoft Edge shows different error codes alongside the "Couldn't download" message. Identify the specific error to narrow down the cause:
...
| **Couldn't download - Network error**            | Network connectivity problem or timeout during the download.                          |
...
| **Couldn't download - Failed - Forbidden**       | The server returned a 403 error, which indicates an authentication or access problem. |

### Check the download folder and disk space

The download might fail if the target folder doesn't exist, is read-only, or the disk is full.

1. Open Microsoft Edge and go to `edge://settings/downloads`.
1. Check the **Location** setting and confirm that the download folder path is valid and accessible.
1. Open File Explorer and go to the download folder. Confirm that:
   - The folder exists.
   - You have write permissions (try creating a test file in the folder).
1. Check available disk space on the drive where the download folder is located. Free up space if necessary.
1. If the download folder is on a network drive, confirm that the network share is accessible and that you have write permissions.

### Check security and SmartScreen settings

[Microsoft Defender SmartScreen](/deployedge/microsoft-edge-security-smartscreen) or other security features might block the download if the file is considered unsafe.
...
1. To review SmartScreen settings:
   ...
   1. If SmartScreen blocks legitimate downloads, consider temporarily turning it off for testing only.
     ...
     > [!CAUTION]
     > Turning off SmartScreen reduces your protection against malicious downloads. Turn it back on after testing.

1. Check whether Windows Security is blocking the download:
   1. Open **Windows Security** > **Virus & threat protection** > **Protection history**.
   1. Look for recent blocked items related to your download.

### Check antivirus and security software

Third-party antivirus software or Microsoft Defender might intercept and block the download.
...
1. If the download succeeds with antivirus turned off, add an exception for:
   - The download URL or domain.
   - The download folder path.
   - The specific file type (if the antivirus blocks by file extension).
1. Turn real-time protection back on after testing.

> [!CAUTION]
> Only download files from sources you trust. Turning off antivirus protection, even temporarily, increases the risk of malware infection.

### Check Group Policy restrictions on downloads

Your organization might have policies that restrict or block downloads.
...
Network problems can interrupt or prevent downloads, especially for large files.

1. Try downloading the same file from a different website or URL to determine whether the problem is site-specific.
1. Try downloading from a different network (for example, a mobile hotspot) to rule out network-level blocking.
1. If your organization uses a proxy server:
   1. Go to `edge://settings/system` and check the proxy settings.
   1. Confirm that the download URL isn't blocked by the proxy.
   1. Contact your network administrator if the proxy is intercepting HTTPS traffic.
...
1. Try downloading the file in a different browser (for example, Chrome or Firefox). If the download also fails, the problem is likely network-related rather than Edge-specific.

### Clear browser cache and reset download settings

Corrupted cache or misconfigured download settings might cause persistent download failures.
...
1. Reset download settings:
   ...
   1. Turn off **Ask me what to do with each download** to see if the prompt behavior causes the failure.
...
Certain file types might be blocked by Edge, Windows, or Group Policy.

1. Note the file extension of the download that failed (for example, `.exe`, `.msi`, `.zip`, `.dll`).
1. Try downloading a different file type from the same site to see if the problem is file-type specific.
1. If your organization blocks specific file types, check the [ExemptFileTypeDownloadWarnings](/deployedge/microsoft-edge-browser-policies/exemptfiletypedownloadwarnings) policy in `edge://policy` to see if an exemption is needed.
1. Some high-risk file types (for example, `.exe`, `.bat`, `.cmd`, `.ps1`) trigger extra security warnings by design. Follow the steps in [Check security and SmartScreen settings](#check-security-and-smartscreen-settings) to proceed with the download if you trust the source.

### Test in a new browser profile or InPrivate window

A corrupted browser profile or conflicting extensions might interfere with downloads.

1. Try downloading the file in an **InPrivate** window (<kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>N</kbd>). If the download succeeds in InPrivate, an extension might be interfering.
...
1. **Exact error message**: Take a screenshot of the download error, including any error code.
1. **Download URL**: Note the URL of the file you're trying to download (if it isn't confidential).
1. **Active policies**: Go to `edge://policy` and export the policy list.
1. **Network trace**: Go to `edge://net-export/`, start logging, reproduce the download failure, then save the log file.
1. **Operating system version**: Go to **Settings** > **System** > **About** and note the OS version.
1. **Antivirus software**: Note the name and version of any antivirus or security software installed.
