...
description: Fix self-hosted Microsoft Edge extensions that don't update, and resolve update issues caused by configuration errors.
ms.date: 03/10/2026
...
If you don't set `"override_update_url": true` in the **ExtensionSettings** policy, Edge might ignore the custom `update_url` that's specified in the extension's `manifest.json` file. Therefore, Edge might not check your server for updates.
...
This method is the most stable and recommended approach for enterprise-managed deployments. It makes sure that Edge always checks your server for new versions.
...
1. In the address bar, go to `edge://extensions`.
1. Turn on **Developer mode**.
1. Select **Update**.

For production environments, the automatic update check occurs within a few hours. You don't have to take any action.

## More troubleshooting steps

If the extension still doesn't update after you apply the preceding solutions, use the following methods to diagnose the issue.
...
To verify that the file is accessible and contains the correct version and `.crx` path, open the update URL directly in the browser (for example, `https://contoso.com/updates/update.xml`).

### Verify the .crx download URL

To make sure that it points to the correct `.crx` file location, check the `codebase` attribute in `update.xml`:
...
During testing, Edge blocks manual installations when you deploy the extension through the **ExtensionSettings** or **ExtensionInstallForcelist** policy. To work around this limitation during test and debug scenarios, use one of the following methods:
