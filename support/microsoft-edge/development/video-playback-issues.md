...
description: Fix Microsoft Edge video playback problems like audio-only output, missing codecs, and autoplay blocks. Use this step-by-step checklist to get videos working again.
ms.date: 03/31/2026
...
This article helps you troubleshoot common video playback problems in Microsoft Edge. You might experience symptoms such as videos not playing, audio playing without video, videos not autoplaying, or error messages related to unsupported formats or decoders. Disabled hardware acceleration, invalid or missing codec extensions, misconfigured autoplay policies, security controls that block encrypted media modules, and restrictive WebRTC policies can all cause these problems. This article provides a step-by-step troubleshooting checklist that covers these common causes.
...
- Videos play in other browsers like Chrome but fail in Microsoft Edge.
- You receive the error "Playback of protected content is not enabled" when you play DRM-protected media.
...
Microsoft Edge doesn't natively support all video codecs. If the media format isn't supported, Edge might play audio without video or fail to play the file entirely. For example, XAVC isn't supported by the built-in playback stack on Windows 10 and Windows 11.

Follow these steps:

1. Open `edge://media-internals` in a new tab, and then reproduce the problem.
1. Look at the player properties and identify the codec from entries like `kVideoTracks` and the decoder information.
...
Common codecs and their support status in Edge include:
...
| XAVC / XAVC HS | Not supported by the built-in Windows playback stack | Can produce audio-only playback or full playback failure.              |

If the video uses an unsupported or narrowly supported format, convert it to a widely supported format. Convert the file to H.264 video with AAC audio in an MP4 container, and then test playback again in Edge.

> [!TIP]
> Open `edge://gpu` and review the **Video Acceleration Information** section to see which codecs are hardware-accelerated on the current device.
...
1. **Media Foundation H264 Decoder (DXVA VDA)**: Edge's default path that uses Windows platform hardware acceleration.
1. **D3D11 Video Decode Accelerator**: An alternative hardware path that some configurations use.
1. **Chromium SW H264 Decoder (FFmpeg)**: Software fallback when hardware acceleration is disabled or unsupported.
...
To confirm that the failing video uses H.264:
...
1. Check `edge://media-internals` again while you reproduce the issue. Confirm which decoder path is being used.

The following example shows the output in `edge://gpu` when hardware acceleration is enabled:
...
- Recheck `edge://media-internals` to confirm whether the decoder path changed.
...
You can confirm this problem by checking the following diagnostic information:
...
1. If an HEVC package is installed, uninstall it and remove any provisioned copy:
...
> [!NOTE]
> This issue can occur when the HEVC extension is installed from a third-party platform rather than the Microsoft Store, which results in an invalid license.

### Check encrypted media (DRM) playback

If you see the error "Playback of protected content is not enabled" on sites like Spotify, the user profile might block the Widevine Content Decryption Module from loading. This problem occurs when `widevinecdm.dll` is read from `%LOCALAPPDATA%` but isn't loaded into `msedge.exe`. `widevinecdm.dll` should be read and loaded from the Edge application folder instead.

Follow these steps:

1. Open Microsoft Edge and go to `edge://components`. Verify that **Widevine Content Decryption Module** is present.
1. If you use Process Monitor, confirm whether `widevinecdm.dll` is read from `%LOCALAPPDATA%` but not loaded into `msedge.exe`.
...
1. Delete the user-profile Widevine folder by using the following PowerShell command:
...
> [!NOTE]
> If AppLocker restricts DLL execution under user profiles, other dynamic Edge components under `%LOCALAPPDATA%\Microsoft\Edge\User Data\` can also be affected.
...
If videos don't autoplay as expected, this problem can have one of the following causes:

- The launch configuration blocks kiosk-mode autoplay.
- [AutoplayAllowList](/deployedge/microsoft-edge-browser-policies/autoplayallowlist) doesn't match the actual navigation type, so new-tab-page autoplay is blocked.
...
Use a launch command similar to the following example:
