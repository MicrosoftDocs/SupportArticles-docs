...
search.audienceType: 
  - admin
---
# Troubleshoot user access problems for different environments

## Summary

This article helps administrators diagnose and resolve common user access problems in Power Platform environments. You learn how to use the built-in diagnostics tool to identify problems with permissions, licenses, security roles, and group memberships that prevent users from accessing environments and resources.
...
If a user doesn't have permissions to access customer engagement apps (Dynamics 365 Sales, Dynamics 365 Customer Service, Dynamics 365 Field Service, Dynamics 365 Marketing, and Dynamics 365 Project Service Automation), a system administrator should complete the following steps:
...
## Troubleshoot unaccounted user problems

In some cases, the system doesn't automatically provision users into environments.
...
- Users with only Office licenses (with Dataverse plan enabled) aren't pre-provisioned into environments.

- Owners of Microsoft Entra groups that are associated with environments aren't pre-provisioned.

- Members of Microsoft Entra groups that are part of a Group Team created for the Microsoft Entra group aren't pre-provisioned.

- Users aren't pre-provisioned into Microsoft Dataverse for Teams environments. For more information, see [Users not added automatically in Dataverse](/power-platform/admin/create-users#categories-of-users-not-added-automatically-in-dataverse).

Although the system doesn't pre-provision these users, you can add them [on demand](#on-demand-user-management). To add or refresh users on demand, see the following section.

## On-demand user management

As mentioned earlier, some scenarios don't automatically provision users. Also, environments might experience delays in showing the users' latest status. In these situations, adding or refreshing specific users on demand can help.

Use one of the following methods to add or refresh users:
