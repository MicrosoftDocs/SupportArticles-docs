...
description: Resolve the "User is not authorized to read secrets" error when saving environment variables that reference Azure Key Vault secrets in Power Platform and Dataverse.
ms.date: 04/03/2026
...
This article helps you resolve the "User is not authorized to read secrets" error that occurs when you create, update, or import environment variables that reference [Azure Key Vault](/azure/key-vault/general/overview) secrets in [Microsoft Dataverse](/power-apps/maker/data-platform/) and Power Platform. This error means Power Platform can't access the specified Key Vault secret. Common causes include missing role assignments, incorrect access control settings, or network restrictions.

This article applies to Dataverse and Power Platform solutions. For Azure Key Vault errors related to _Wrap for Power Apps_, see [Azure Key Vault errors in Wrap for Power Apps](/troubleshoot/power-platform/power-apps/manage-apps/azure-key-vault-errors).

## Symptoms

When you create, update, or import a solution containing an environment variable that references an Azure Key Vault secret, you receive one of the following error messages:
...
You might also see the following error in diagnostic logs:
...
- Creating a new environment variable of type **Secret** with **Azure Key Vault** as the secret store.
- Updating the Key Vault secret reference value of an existing environment variable.
- Importing a solution that contains environment variables referencing Azure Key Vault secrets.

## Grant the Dataverse service principal the Key Vault Secrets User role

The Dataverse service principal needs the **Key Vault Secrets User** role on your Azure Key Vault. Without this role, Dataverse can't retrieve the secret value on behalf of users.

> [!IMPORTANT]
> If you previously configured your service principal with only the **Key Vault Reader** role, this role isn't sufficient. You must also add the **Key Vault Secrets User** role.
...
1. On the **Role** tab, search for and select **Key Vault Secrets User**.
1. On the **Members** tab, select **Select members**.
1. Search for and select the user who needs to create or update the environment variable.
1. Complete the wizard by selecting **Review + assign**.

## Switch Key Vault to Azure role-based access control (RBAC)

Power Platform uses [Azure RBAC](/azure/role-based-access-control/overview) to check permissions. If your Key Vault still uses the legacy **Vault access policy** permission model, permission checks can fail even if the correct policies are in place.
...
> [!IMPORTANT]
> After you switch to Azure RBAC, make sure the **Key Vault Secrets User** role is assigned to both the user and the Dataverse service principal.

## Register the Microsoft.PowerPlatform resource provider

You need to register the `Microsoft.PowerPlatform` resource provider for your Azure subscription. If you don't register it, Power Platform can't interact with Azure resources on your behalf.
...
1. Select **Resource providers** from the left navigation.
1. Search for `Microsoft.PowerPlatform`.
1. If the status shows **NotRegistered**, select it and then select **Register**.
...
## Verify the Key Vault is in the same tenant
...
1. Confirm that your Azure Key Vault and your Power Platform environment are in the same Microsoft Entra tenant.
...
If you use Azure Key Vault with [Private Link](/azure/private-link/private-link-overview) and [Virtual Network (VNet) support for Power Platform](/power-platform/admin/vnet-support-overview), the private endpoint must be in the same region as the Power Platform environment's cluster. For example, if your environment is in _West US_, the private endpoint must also be in _West US_.

> [!IMPORTANT]
> Virtual Network support for Power Platform requires a [Managed Environment](/power-platform/admin/managed-environment-overview). Make sure your environment is enabled as a Managed Environment before you set up Private Link.
...
1. Select the virtual network and subnet delegated to Power Platform.
...
If you enable a firewall on your Key Vault, it might block Power Platform requests. This block can happen because Power Platform IP addresses aren't on the **Azure Trusted Services** list, or the firewall only allows access from specific IP addresses or virtual networks.

To update the firewall rules:

1. In the [Azure portal](https://portal.azure.com), go to your Key Vault.
1. Select **Settings** > **Networking**.
1. Under **Firewalls and virtual networks**, add the Power Platform IP address ranges listed at [Power Platform URLs and IP address ranges](/power-platform/admin/online-requirements#ip-addresses-required).
