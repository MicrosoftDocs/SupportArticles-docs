...
- Browser local-network protections that treat proxied traffic as local-resource access in some configurations and block the request.
...
- If these headers are missing and the response resembles a proxy or gateway page, the response was likely generated by an intermediary network device.

When you engage Microsoft support, include any `x-ms-correlation-id` or `x-ms-request-id` values you captured. These identifiers help correlate the request with service telemetry.
...
If failures affect users only in specific regions, also consider DNS response-size filtering by intermediary devices. Some Power Platform host name chains can produce larger DNS answers (for example, due to CNAME expansion and geographic routing). Devices that still enforce older DNS size assumptions from [RFC 1035](https://www.rfc-editor.org/rfc/rfc1035.txt) can block responses that are valid under [RFC 6891](https://www.rfc-editor.org/rfc/rfc6891.txt), causing region-specific app load or connector failures.
...
Also confirm that allow-listing handles dynamic host names and subdomains used by Power Platform services. Restrictive rules that allow only a previously observed host can cause intermittent failures when service routing changes.

For example, Power Platform requests can use long, environment-specific host names under service domains rather than a single fixed host. If policy allows only a previously observed subdomain, the next routing change can fail even though no app logic changed.

If your organization uses traffic inspection, ask the team to verify that CORS headers returned by Power Platform are preserved end-to-end. Header rewrite or removal can cause browser-level failures (including HTTP status `0`) even when endpoint allow-listing is otherwise correct.

If issues are concentrated by region or office location, ask the team to validate support for larger DNS responses and EDNS behavior so required Power Platform host names resolve consistently.

If local-network protections are suspected, also review browser guidance for recent local-network access controls (for example, Chromium's local network access behavior): [Private Network Access: introducing preflights](https://developer.chrome.com/blog/private-network-access-preflight).
...
- Review HAR entries for signs of intermediary processing, such as protocol downgrade patterns (`HTTP/1.1` responses where you expect modern `HTTP/2`) or proxy-identifying headers like `Via`. Because connector traffic is expected to use modern HTTP behavior, repeated downgrade patterns are a strong indicator of intermediary inspection or proxy fallback and can materially affect reliability and performance.
- If a browser local-network access prompt appears (or appears to have been denied), collect that detail for the network team. Some proxy designs that use non-routable/private address ranges can trigger local-network protections and block requests.
