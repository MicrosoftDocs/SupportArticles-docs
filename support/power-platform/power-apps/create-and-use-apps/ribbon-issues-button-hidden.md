...
1. The following example shows the **New** button on the contact entity's grid page isn't visible and is represented by an item labeled **New (hidden)**.
    ...
    > [!NOTE]
    > If your button isn't listed, it could be due one of the following reasons:
    > - a [HideCustomAction](/powerapps/developer/model-driven-apps/define-custom-actions-modify-ribbon#hide-custom-actions) customization that may have been installed. In the left navigation panel, there is a `HideCustomAction` section at the top. Expand the `HideCustomAction` list and select each one and check the location properties - if it matches the name of your button, then it is the cause of why the button is hidden. Follow the Repair Options instructions below, but for the HideCustomAction instead of a command.
    > - the associated command has a `Mscrm.HideOnModern` display rule. At the time of writing this guide, the Command Checker tool does not list buttons that have been hidden by a `Mscrm.HideOnModern` display rule. We are currently working to augment this listing to include this information in a future update.
    > - the button has been manually hidden using the command bar designer for this specific app.  Go to the [command bar designer](/power-apps/maker/model-driven-apps/use-command-designer) and find the button, and check the properties pane.  At the bottom, there is a **Hidden** field - ensure that it is not checked.
    ...
    > [!NOTE]
    > If the button is still hidden when all rules evaluate to **True**, check hidden reason in the navigation panel.  If it says (hidden by selection), it is due to [context sensitive commands in grids](https://support.microsoft.com/topic/1100b6b5-ab64-6047-82c5-924a2b38296a). When records are selected on a grid, all buttons without a `SelectionCountRule` element will be considered not relevant to the selected record(s). And they're hidden even if their rule evaluation is **True**. Note that flyouts aren't affected since flyout children might still have record based commands.
    > If it says (hidden offline), the command is hidden because you are offline, or the app is offline by default.  This command is not supported offline. You can adjust your app's [offline settings](/power-apps/mobile/mobile-offline-overview) and disable offline-first.
