...
1. Find and click your button in the list of buttons displayed in the left-most pane of the Command Checker tool. Buttons that are not visible will be denoted by de-emphasized and italicized font along with the **(hidden)** term. The following example shows the **New** button on the contact entity's grid page is not visible and is represented by an item labeled **New (hidden)**.
    ...
    > [!NOTE]
    > If you see all rules evaluate to true, but the button is still hidden, it may be due to [context sensitive commands in grids](https://support.microsoft.com/en-us/topic/button-in-command-bar-does-not-appear-after-grid-item-selection-1100b6b5-ab64-6047-82c5-924a2b38296a). When records are selected on a grid, all buttons without a SelectionCountRule will be considered not relevant to the selected record(s) and be hidden even if their rule evaluation is true.  Note this does not effect flyouts, since flyout children might still have record based commands.
