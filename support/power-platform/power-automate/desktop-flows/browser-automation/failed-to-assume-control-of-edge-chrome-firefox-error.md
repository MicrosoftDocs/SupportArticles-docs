...
Use the **Launch Browser** action to start the process of the corresponding browser. Then, use a combination of an additional **Launch Browser** action that has the mode set to **Attach to running instance** and a **Wait** action that's set to a duration afterwards.
...
1. Insert an error handling policy:
    ...
    - Open the dropdown list, select **Continue flow run** > **Go to next action**, and then select **Save**.
