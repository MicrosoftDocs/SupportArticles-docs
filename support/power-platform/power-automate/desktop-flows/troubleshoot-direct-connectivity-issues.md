...
> [!NOTE]
> If you use an Azure Virtual Machine (VM) to run the Power Automate for desktop, make sure the Microsoft.ServiceBus endpoint is turned off at subnet level where the Azure VM is located. This is a known limitation. For more information, see [Azure Relay doesn't support network service endpoints](/azure/azure-relay/network-security).
