...
|SessionCreationError|400|Unattended|We couldn't create the session on the machine for an unknown reason. Please check the following:<br>- That you can remote desktop to the machine from another machine on your network. If you're on a Windows server, you can try to remote desktop from the machine logged in as another account to "localhost". If these fail, use general remote desktop troubleshooting.<br>- Ensure there is no third party software installed which may affect logon or interfere with creating a remote desktop connection.|
