...
For commercial Power Automate (all geos), Approval mails may come from any IP in the following SPF record. Note SPF records can be updated as more servers and regions are added, for the most up to date IP address ranges please look up the SPF record for `_spf1-meo.microsoft.com`
