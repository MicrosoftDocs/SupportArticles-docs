...
|SessionCreationInvalidCredentials|400|Unattended|This error occurs when the unattended session could not be created with the credentials provided. This may occur if you are not using the username in the right format. For AAD-joined machines ensure you are using user@domain.com and for domain-joined machines ensure you are using domain\user.|
