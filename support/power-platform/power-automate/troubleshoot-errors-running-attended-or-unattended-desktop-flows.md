...
|OnPremiseDataGatewayNotAvailable|502|Attended</br>Unattended|This error code only occures when connecting to the machine using an on-premises data gateway (deprecated). Please check that the targeted machine and the data gateway on that machine are up and running.|
