...
This article helps you troubleshoot and resolve authentication issues when connecting to Salesforce CRM from the [Sales agent in Outlook](/microsoft-sales-copilot/use-sales-copilot-outlook).

## Who is affected?

| Requirement type |Description  |
|---------|---------|
|**Client app**     |  Sales agent in Outlook and Teams app   |
...
When you try to sign in to Salesforce CRM from the Sales agent in Outlook, the following error message is displayed:
...
Sales agent connects to Salesforce CRM through the Microsoft Power Platform connected app. To sign in to Salesforce CRM from Sales agent, users must provide their consent to the Microsoft Power Platform app to contact Salesforce CRM on their behalf. When consent can't be provided, the sign-in process fails.

## Resolution

Sales agent uses the Microsoft Power Platform connected app to connect to Salesforce CRM. Ensure that the policies of the Microsoft Power Platform app are configured to allow users to connect to Salesforce CRM.
...
5. Under **OAuth Policies**, select one of the following values for **Permitted Users**:
    ...
    1. **All users may self-authorize**: This option allows any authenticated Salesforce user to connect to the Salesforce instance through the Microsoft Power Platform connector used by Sales agent, Microsoft Power Automate, and potentially other apps to gain access to Salesforce, thus resolving the issue.
