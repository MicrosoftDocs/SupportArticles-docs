...
description: Resolves an error message that states the user doesn't have required privileges to access the organization when the Administration mode is enabled in Sales app.
ms.date: 02/05/2025
author: sbmjais
ms.author: shjais
manager: shujoshi
...
This article provides a resolution for an error message that occurs in Sales app when the organization is in [Administration mode](/power-platform/admin/admin-mode).
...
|**CRM**     | Dynamics 365      |
|**Users**     | Users trying to use Sales app when **Administration mode** is enabled. |

## Symptoms

When you use Sales app, you receive the "Something went wrong" error message with the following error description:

> User does not have required privileges (or role membership) to access the org when it is in Admin Only mode.

The error might be displayed before or after you sign in to an environment from Sales app.
