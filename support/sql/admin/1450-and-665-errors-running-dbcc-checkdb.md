...
1. When you execute `DBCC CHECKDB`, you can consider setting up an Availability Group or Log Shipping/Standby server. Or use a second server where you can run the `DBCC CHECKDB` commands to  offload the work and avoid running into the issues caused by the heavy fragmentation.
