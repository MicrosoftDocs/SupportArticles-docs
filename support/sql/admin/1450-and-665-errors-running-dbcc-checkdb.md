...
Additionally, you may also notice blocking when you view various dynamic management views (DMV) such as `sys.dm_exec_requests` and `sys.dm_os_waiting_tasks`.
...
1. Break up large database into smaller files. For example, if you have one 8-TB data file you can break it up into eight 1-TB data files. This option might help because fewer modifications would happen on smaller files thus less likely to introduce attribute exhaustion. Also, in the process of moving data around, the files will be organized compactly and fragmentation reduced. The following are high-level steps outline the process:
   ...
   1. Shrink the original 8-TB file down, now that's about 12-15% full.
