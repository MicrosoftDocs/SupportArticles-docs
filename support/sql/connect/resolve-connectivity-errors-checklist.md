...
- If you have administrator access to the SQL Server computer, gather and review current computer settings and service accounts by using the following procedure:
  ...
  1. Run the command prompt to collect the data and save to a file. For example: `SQLCHECK > C:\Temp\server01.SQLCHECK.TXT`.
...
> [!NOTE]
> The next few sections help you to quickly check for connectivity issues. Review individual topics for detailed troubleshooting steps.

### Option 1

If you have access to the output of SQLCheck tool mentioned in the [Recommended prerequisites](#recommended-prerequisites) section and review information in various sections in the output file (Computer, Client Security, and SQL Server), use the information to address the issues contributing to your problem. See the examples below:
