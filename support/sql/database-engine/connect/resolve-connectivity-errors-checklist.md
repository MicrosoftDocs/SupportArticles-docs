...
description: Use this quick checklist and SQLCHECK-based prerequisites to diagnose and fix SQL Server connectivity errors, login failures, and network-related connection issues.
ms.date: 07/28/2026
...
This article provides a two-part starting point for troubleshooting SQL Server connectivity issues: a set of recommended prerequisites for collecting diagnostic data, and a quick checklist you can use before you begin in-depth analysis. Use it to gather error messages, event logs, connection strings, and SQL Server configuration details, and to verify that services, listeners, protocols, permissions, and network paths are healthy. After you complete these steps, continue to [common connectivity issues](resolve-connectivity-errors-overview.md#categories-of-sql-server-connectivity-errors) to select the error message that matches your scenario for detailed troubleshooting steps.
...
- Application and system event logs from SQL Server and client systems. These logs can help you check if there are any system wide issues occurring on your SQL Server.
- If the connections fail from an application, collect the connection strings from the application. These strings are typically found in *Web.config* files for ASP.NET applications. 
...
> [!NOTE]
> The following sections help you quickly check for connectivity problems. For detailed troubleshooting steps, review individual topics.

### Review the SQLCHECK tool output

If you have access to the output of the SQLCHECK tool mentioned in the [Recommended prerequisites](#recommended-prerequisites) section, review the information in various sections of the output file (Computer, Client Security, and SQL Server). Use the information to address the problems contributing to your issue. See the following examples:
...
|Client Security and Driver Information |Diffie-Hellman cipher suites are enabled. Possible risk of intermittent TLS failures if the algorithm version is different between clients and servers |If you're having intermittent connectivity problems, see [Applications experience forcibly closed TLS connection errors when connecting to SQL Servers in Windows](../../../windows-server/identity/apps-forcibly-closed-tls-connection-errors.md).|[An existing connection was forcibly closed by the remote host](../connect/tls-exist-connection-closed.md)|
|Client Security and Driver Information |SQL Aliases |If present, ensure aliases are configured properly and point to the correct server and IP addresses. |[A network-related or instance-specific error occurred while establishing a connection to SQL Server](../connect/network-related-or-instance-specific-error-occurred-while-establishing-connection.md) |
|SQL Server Information |Services of Interest |If your SQL service isn't started, start it. If you're having problems connecting to a named instance, ensure SQL Server Browser service is started or try restarting the browser service. |[A network-related or instance-specific error occurred while establishing a connection to SQL Server](../connect/network-related-or-instance-specific-error-occurred-while-establishing-connection.md) |
|SQL Server Information |Domain Service Account Properties |If you configure linked servers from your SQL Server and the **Trust for Del** value is set to false, you might run into authentication problems with your linked server queries. |[Troubleshooting "Login failed for user" errors](/sql/relational-databases/errors-events/mssqlserver-18456-database-engine-error?context=/troubleshoot/sql/context/context) |
...
### Run manual checks when SQLCHECK isn't available

If you can't run SQLCHECK on your SQL Server computer, check the following items before doing in-depth troubleshooting:
...
1. Verify basic connectivity over IP address and check for any abnormalities: `ping -a <SQL Server machine>, ping -a <SQL Server IP address>`. If you notice any problems, work with your network administrator. Alternatively, you can use `Test-NetConnection` in PowerShell:
...
1. Check whether you can connect to SQL Server by using a UDL file. If it works, then there might be an issue with the connection string. For instructions on the UDL test procedure, see [Test OLE DB connectivity to SQL Server by using a UDL file](test-oledb-connectivity-use-udl-file.md). Alternately, you can use the following script to create and launch a *UDL-Test.udl* file (stored in the *%TEMP%* folder):
...
1. Check whether you can connect to SQL Server from other client systems and different user logins. If you can, the problem might be specific to the client or login that is experiencing the problem. Check the Windows event logs on the problematic client for more pointers. Also, check whether network drivers are up to date.
...
1. If you're troubleshooting Kerberos related problems, you can use the scripts at [How to determine if the authentication type is Kerberos](determine-the-authentication-type.md).

## Common connectivity issues

After you review the prerequisites and checklist, see [common connectivity issues](resolve-connectivity-errors-overview.md#categories-of-sql-server-connectivity-errors) and select the error message that matches your scenario for detailed troubleshooting steps.

## Related content

- [Troubleshoot intermittent or periodic issues with connecting to SQL Server](intermittent-periodic-network-issue.md)
