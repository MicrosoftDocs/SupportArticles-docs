---
title: Resolve Blocking Problems Caused by Lock Escalation
description: Learn how to diagnose SQL Server blocking caused by lock escalation, prevent unnecessary escalation by tuning transactions and queries, and when to disable it.
ms.date: 08/04/2026
...
Lock escalation converts many fine-grained locks (such as row or page locks) into a single table lock. SQL Server triggers escalation dynamically based on lock count or lock memory, and its default behavior improves throughput in most workloads. However, some query or transaction designs cause escalation at times when the resulting table lock blocks other users. This article explains how to determine whether lock escalation is causing blocking in SQL Server, how to prevent it, and how (and when) to disable it.

## Determine whether lock escalation is causing blocking

Lock escalation doesn't cause most blocking problems. To determine whether lock escalation is occurring at or near the time when you experience blocking problems, start an Extended Events session that includes the `lock_escalation` event. If you don't see any `lock_escalation` events, lock escalation isn't occurring on your server, and the information in this article doesn't apply to your situation.

If lock escalation is occurring, verify that the escalated table lock is blocking other users.

For more information about how to identify the head blocker and the lock resource that's held by the head blocker and that's blocking other server process IDs (SPIDs), see [Understand and resolve SQL Server blocking problems](understand-resolve-blocking.md).

If the lock that's blocking other users is anything other than a TAB (table-level) lock that has a lock mode of S (shared) or X (exclusive), lock escalation isn't the problem. In particular, if the TAB lock is an intent lock (such as a lock mode of IS, IU, or IX), it isn't caused by lock escalation. If your blocking problems aren't caused by lock escalation, see the [Understand and resolve SQL Server blocking problems](understand-resolve-blocking.md) troubleshooting steps.

## Prevent lock escalation

The simplest and safest method to prevent lock escalation is to keep transactions short and reduce the lock footprint of expensive queries so that the lock escalation thresholds aren't exceeded. Use one or more of the following strategies.

### Break large operations into smaller transactions

Break up large batch operations into several smaller operations. For example, you run the following query to remove more than 100,000 old records from an audit table, and then you determine that the query caused a lock escalation that blocked other users:

```sql
DELETE FROM LogMessages WHERE LogDate < '20020102';
```

By removing these records a few hundred at a time, you can dramatically reduce the number of locks that accumulate per transaction. This approach prevents lock escalation. For example, you run the following query:
...
### Hold a compatible table-level lock on a separate connection
...
> [!NOTE]
> This technique is a workaround, not a general recommendation. It requires holding an open transaction and IX lock for the duration you want to suppress escalation. Prefer the query and transaction improvements described earlier.

Therefore, one method to prevent lock escalation on a particular table is to acquire and hold a lock on a different connection that isn't compatible with the escalated lock type. An IX (intent exclusive) lock at the table level doesn't lock any rows or pages, but it's still not compatible with an escalated S (shared) or X (exclusive) TAB lock. For example, assume that you must run a batch job that modifies many rows in the mytable table and that caused blocking because of lock escalation. If this job always finishes in less than one hour, you might create a Transact-SQL job that contains the following code, and schedule the new job to start several minutes before the batch job start time:
...
> [!NOTE]
> When a 1204 error occurs, it stops the processing of the current statement and causes a rollback of the active transaction. The rollback itself might block users or cause a long database recovery time if you restart the SQL Server service.
