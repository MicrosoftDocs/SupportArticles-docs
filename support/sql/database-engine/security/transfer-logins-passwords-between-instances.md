...
- **Always On availability groups and failover cluster instances**: Transfer logins to every replica or node so users can sign in after a failover. For more information, see [Manage logins and jobs after failover](/sql/database-engine/availability-groups/windows/logins-and-jobs-after-role-switching).
