...
After you move a database from the instance of SQL Server on server A to the instance of SQL Server on server B, users may be unable to log in to the database on server B. Additionally, users might receive the following error message:
...
The server principal '**MyLogin**' already exists.
  ...
  Similarly, a login that already is in the instance on server B may have a SID that's the same as a SID in the output script. In this case, you receive the following error message when you run the output script on the instance on server B:
