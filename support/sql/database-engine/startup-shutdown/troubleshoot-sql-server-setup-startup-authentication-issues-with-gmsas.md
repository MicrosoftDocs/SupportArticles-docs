...
description: Provides a template for troubleshooting SQL Server setup, startup, and authentication issues that involve group managed service accounts (gMSAs).
...
# Troubleshoot SQL Server setup, startup, and authentication issues with gMSAs

_Applies to:_ &nbsp; SQL Server 2016, SQL Server 2017, SQL Server 2019, SQL Server 2022, and SQL Server 2025 on Windows



## Overview

This article helps you diagnose SQL Server installation, upgrade, service startup, SQL Server Agent, failover cluster instance, Kerberos, linked server, and availability group authentication failures when SQL Server services use a group Managed Service Account (gMSA). Start with the quick validation checklist, choose the path that matches the observed failure, and then follow the numbered diagnostic and resolution steps. For information about selecting and configuring service accounts for SQL Server, including managed service accounts and gMSAs, see Configure Windows service accounts and permissions.

> **Note:** Collect evidence before you change service accounts,security policies, registry values, service principal names (SPNs), or   failover cluster settings. Record the failure time and server time zone.
...
**Command scope:** Run host validation commands locally on every affected standalone server, failover cluster node, or availability group replica. Replace example account, service, instance, server, listener, and port values with values from your environment.
...
## SQL Server Agent starts but jobs fail
## Symptoms
...
## FCI installation, startup, add-node, or failover fails
## Symptoms
...
## SQL Server Setup and servicing
...
- lient Kerberos tickets and connection authentication scheme.
...
Use SQL LogScout to automate collection of requested SQL Server diagnostic data when it is appropriate for the investigation.
