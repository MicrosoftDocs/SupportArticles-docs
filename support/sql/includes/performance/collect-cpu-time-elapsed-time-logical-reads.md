...
- If you can execute the query on demand in SQL Server Management Studio (SSMS), **sqlcmd**, or the MSSQL extension for Visual Studio Code, run it with [SET STATISTICS TIME](/sql/t-sql/statements/set-statistics-time-transact-sql) `ON` and [SET STATISTICS IO](/sql/t-sql/statements/set-statistics-io-transact-sql) `ON`.
