...
## Recommmednations:
Preventing lock escalation methods that are discussed earlier in this article are better options than disabling escaltion at table or server level. Additionally, the peventive methods methods generally result in better performance for the query than disabling lock escalation. Microsoft recommends enabling this trace flag only to mitigate severe blocking that is caused by lock escalation while other options, such as those discussed earlier in this article, are being investigated. To enable a trace flag so that it is turned on whenever SQL Server is started, add it as a server startup parameter.
