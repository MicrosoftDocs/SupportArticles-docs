...
- Reduce the query's lock footprint by making the query as efficient as possible. Large scans or many bookmark lookups can increase the chance of lock escalation. Additionally, these increase the chance of deadlocks, and adversely affect concurrency and performance. After you identify that the query that causes lock escalation, look for opportunities to create new indexes or to add columns to an existing index to remove index or table scans and maximize the efficiency of index seeks. Consider pasting the query into a Query Analyzer query window to run an automatic index analysis on it. To do this, open the **Query** menu, and select **Index Tuning Wizard** in SQL Server 2000 or **Perform Index Analysis** in SQL Server 7.0.
