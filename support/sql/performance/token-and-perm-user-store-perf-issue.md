...
1. Check the size of the TokenAndPermUserStore cache. To do this, run the following command:
    ...
    SELECT SUM(pages_kb) AS 
       ...
       WHERE name = 'TokenAndPermUserStore'
    ```
1. If the cache size is bigger than the observed threshold, run the following command:
`DBCC FREESYSTEMCACHE ('TokenAndPermUserStore')`
