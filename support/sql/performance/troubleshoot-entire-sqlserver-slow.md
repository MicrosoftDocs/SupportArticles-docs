...
Check the operating system where SQL Server is running. Is the system slow in its response - mouse moves slowly and windows do not respond for long periods of time.  

Common issues include:
   - **High CPU across all CPUs** - this could be caused by other applications, the OS or drivers running on the system. 
...
If many queries or several CPU-intensive queries are being executed on the system, they can use other queries to be starved of CPU capacity, or more frequently high CPU coming from queries can be an indication that queries need to be optimized. Here are high-level steps how to approach these issues
...
-  Scale up system to use more CPUs
...
- Check for sub-optimal SQL Server queries that are causing lots of I/O and saturating disk volumes with I/O requests. 
...
- Filter drivers: The SQL Server I/O response can be severely impacted if file-system filter drivers process heavy I/O traffic. 
   - Exclude data folders from anti-virus scanning and correct filter driver design by software vendors are recommended to prevent impact on I/O performance.

- Other application(s): Another application on the same machine with SQL Server can saturate the I/O path with excessive read or write requests. This situation may push the I/O subsystem beyond capacity limits and cause I/O slowness for SQL Server. Identify the application and tune it or move it elsewhere to eliminate its impact on the I/O stack. This issue can be caused by other applications running on other machines but sharing the same SAN with this SQL Server machine. For those, work with your SAN administrator to balance I/O traffic (see check for hardware issues above)
...
- Check the SQL Server errorlog for 701 errors
...
Lock acquisition is used to protect resources in a database system. If locks are acquired for a long time and other sessions end up waiting for those locks, then you are faced with a blocking scenario.
...
- Then find the query(s) that the head blocking chaing executes  (what is holding locks for a prolonged period)
  - If no queries are actively running on the head blocking session, then there could have been an orphaned transactions due to application issues
- Redesign or tune the head blocking query to run faster, or reduce the number of queries inside a transation
