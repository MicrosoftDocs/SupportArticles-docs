...
1. Rewrite the query with the goal to simplify it:
   ...
   - Remove `TOP`, `EXISTS`, `FAST` (T-SQL) in the queries that run for a very long time due to  [optimizer row goal](https://techcommunity.microsoft.com/t5/sql-server-blog/more-showplan-enhancements-8211-row-goal/ba-p/385839) which results in long-running nested loops. For more information, see [Row Goals Gone Rogue](/archive/blogs/bartd/row-goals-gone-rogue)
