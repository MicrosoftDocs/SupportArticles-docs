---
title: Troubleshoot a query that is slow on one server but fast on another
...
This article provides troubleshooting steps for a performance issue where a query runs slowly on one server while the same query runs fast on another server.
...
#### Next step: check wait types on Server1
...
The following sections show common causes for differences in query plans:

### Cause 1: Data size or data values differences
...
### Cause 2: Statistics differences
...
### Cause 3: Database compatibility level differences
...
### Cause 4: Server version/build differences
...
### Cause 5: Cardinality Estimator (CE) version differences
...
### Cause 7: Trace flags differences
...
### Cause 8: Hardware differences (CPU count, Memory size)
...
### Cause 9: Hardware differences according to the query optimizer
...
### Cause 12: Query Hint differences
...
### Cause 13: Parameter-sensitive plans (parameter sniffing issue)
