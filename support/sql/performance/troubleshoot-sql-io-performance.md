...
The follow is a description of the methodology Microsoft CSS uses to approach slow I/O issues with SQL Server. This is not an exhaustive or exclusive approach, but has proven useful in isolating the issue and resolving it.
...
2.1 If, the values of this counter are consistently above 10-15 ms. then you need to take a look at the issue further. Occasional spikes don't count in most cases but be sure to double-check the duration of a spike - if it lasted 1 minute or more, then it is more of a plateau than a spike.


2.2 If Performance monitor counters do not report latency, but SQL Server does, then the problem is between SQL Server and the Partition Manager (the I/O layer at which the OS collects Perfmon counters); that is file system filter drivers. Ensuring proper exclusions of filter driver and resolving filter driver issues is the next logical step. Filter drivers are used by programs like [Anti-virus software](https://docs.microsoft.com/en-us/windows-hardware/drivers/ifs/allocated-altitudes#320000---329998-fsfilter-anti-virus), [Backup solutions](https://docs.microsoft.com/en-us/windows-hardware/drivers/ifs/allocated-altitudes#280000---289998-fsfilter-continuous-backup), [Encryption](https://docs.microsoft.com/en-us/windows-hardware/drivers/ifs/allocated-altitudes#140000---149999-fsfilter-encryption), [Compression](https://docs.microsoft.com/en-us/windows-hardware/drivers/ifs/allocated-altitudes#160000---169999-fsfilter-compression), and so on. 
You can use this command to list filter drivers on the systems and what volumes they attach to. Then you can look up the driver names and software vendor in this article: [Allocated filter altitudes](https://docs.microsoft.com/en-us/windows-hardware/drivers/ifs/allocated-altitudes)

   ```Powershell
   fltmc instances
   ```
For more information see [How to choose antivirus software to run on computers that are running SQL Server](https://support.microsoft.com/en-us/topic/how-to-choose-antivirus-software-to-run-on-computers-that-are-running-sql-server-feda079b-3e24-186b-945a-3051f6f3a95b)

Avoid using file-level encryption (EFS) and file-system compression: both cause asynchronous I/O to become synchronous and therefore slower. For more information, see [Asynchronous disk I/O appears as synchronous on Windows](https://docs.microsoft.com/en-us/troubleshoot/windows/win32/asynchronous-disk-io-synchronous#compression)

### 3. Is the I/O subsystem overwhelmed beyond capacity?
...
If I/O subsystem is overwhelmed beyond capacity, then find out if SQL Server is the culprit by looking at **Buffer Manager: Page Reads/Sec** (most common culprit) and **Page Writes/Sec** (a lot less common) for the specific instance. If SQL Server is the main I/O driver and I/O volume is beyond what the system can handle, then you need to work with the Application Development teams (or application vendor) to
 - Tune queries - better indexes, update statistics, re-write queries, re-design the database, etc. 
...
1. **Hardware issues:** There is a SAN misconfiguration (switch, cables, HBA, storage), exceeded I/O capacity (throughout entire SAN network, not just back-end storage), drivers/firmware bug, and so on. This is where the hardware vendor need to be engaged.
...
Occur when some of the following I/O activities take place
