...
If you already installed the initially released update, you can [uninstall](/sql-server/install/uninstall-a-cumulative-update-from-sql-server) it, or install the currently available version of the update to restore Database Mail functionality.
...
Any email messages queued via Database Mail while the initially released version of this update was installed aren't sent. These messages aren't automatically resent when the initially release update is uninstalled, or when a cumulative update with the fix for this issue is installed.
