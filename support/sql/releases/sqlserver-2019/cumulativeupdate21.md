...
- You review the `sys.dm_exec_requests` DMV and notice the `DB STARTUP` command is blocked on the wait `HADR_RECOVERY_WAIT_FOR_UNDO`.
...
| <a id="2266806">[2266806](#2266806)</a> | Adds a new error 673 to help avoid the assertion failure (Location: IndexRowScanner.cpp:1449; Expression: m_versionStatus.IsVisible ()) that you may encounter when you enable change tracking on a database. </br></br>Error message: </br></br>Failure to access row object in snapshot isolation. | SQL Server Engine | Replication | All |
