...
| <a id=3560254>[3560254](#3560254) </a> | Fixes an issue in which some availability group (AG) databases are in a non-synchronized state after the Windows Server Failover Clustering (WSFC) quorum loss. After applying the fix, the databases should be able to switch to a resolving state when the persisted configuration data can't be read due to the WSFC quorum loss. | SQL Server Engine| High Availability and Disaster Recovery | All|
...
| <a id=3420397>[3420397](#3420397) </a> | Fixes an issue in which running queries with columnstore indexes is stuck due to non-yielding scheduler errors.| SQL Server Engine| SQL OS| Windows|
