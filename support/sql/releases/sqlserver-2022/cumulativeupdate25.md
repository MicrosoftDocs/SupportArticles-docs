...
| :------------ | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------- | :-------------------------------------- | :------- |
| 4954485       | Fixes an issue in Contained Availability Groups where DatabaseMail sends emails without attachments.                                                                                                            | SQL Server Client Tools | Database Mail                           | All      |
| 4999187       | Fixes an issue where permissions on stored procedures and other objects in `msdb` are lost during the SQL Server upgrade process, causing users to lose granted permissions.                                    | SQL Server Engine       | Management Services                     | Windows  |
| 5028794       | Fixes an access violation error that occurs when querying `sys.dm_xe_session_targets` on SQL Server instances running on Windows Server 2025.                                                                   | SQL Server Engine       | SQL OS                                  | Windows  |
| 5033534       | Fixes an issue in Contained Availability Group connections where T-SQL only allows adding a database to the currently connected Contained Availability Group, with additional restrictions for DBCreator roles. | SQL Server Engine       | High Availability and Disaster Recovery | All      |
| 5034739       | Fixes a memory leak issue where parallel execution of `SHORTEST_PATH` queries in Graph databases caused persistent `OBJECTSTORE_SOSTASK` growth.                                                                | SQL Server Engine       | Query Optimizer                         | All      |
| 5090699       | Fixes an issue where a self-assignment update on a small off-row XML column with compression enabled causes XML data corruption and a subsequent transport-level error when selecting the data.                 | SQL Server Engine       | Access Methods                          | All      |
| 5126699       | Fixes an issue where using the Oracle ODBC driver with the ODBC connector in SSIS packages could cause heap corruption and data loss.                                                                           | Integration Services    | Integration Services                    | All      |
...
| 5213660       | Fixes a backup cleanup issue on Linux that could cause SQL Server to crash when a network share is full or unreachable.                                                                                         | SQL Server Engine       | Linux                                   | Linux    |
