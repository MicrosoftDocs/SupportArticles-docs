...
This article describes Cumulative Update package 3 (CU3) for Microsoft SQL Server 2022. This update contains nine [fixes](#improvements-and-fixes-included-in-this-update) that were issued after the release of SQL Server 2022 Cumulative Update 2, and it updates components in the following builds:
...
|-------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|----------------------|----------|
| <a id="2279666">[2279666](#2279666)</a> | Fixes a failure in SQL Server Analysis Services (SSAS) that occurs after you rename partitions to the same name in a multidimensional model. | Analysis Services | Analysis Services | Windows |
...
| <a id="2168305">[2168305](#2168305)</a> | Fixes a memory leak issue that occurs when configuring SQL Server logging shipping that's in standby or read-only mode for an In-memory OLTP database. </br></br>**Note**: You need to turn on trace flag 9953 during startup to avoid the issue. | SQL Server Engine | In-Memory OLTP | All |
...
| <a id="2270253">[2270253](#2270253)</a> | Fixes a failure where the `DateTime` field can't be pushed down to certain PolyBase generic ODBC external data sources such as Denodo when you query an external table by using a filter clause on the `DateTime` field. | SQL Server Engine | PolyBase | All |
...
| <a id="2273502">[2273502](#2273502)</a> | Fixes an issue where the change tracking auto cleanup process generates dumps under rare conditions when the parent table name that the system tries to access isn't available at runtime, and then the process runs into an access violation. | SQL Server Engine | Replication | All |
| <a id="2214981">[2214981](#2214981)</a> | Fixes an issue where the `mssql-conf` command gives incorrect errors even if they succeed on Linux. | SQL Server Engine | SQL Agent | Linux |
| <a id="2283663">[2283663](#2283663)</a> | Adds a new feature that helps parallelize the version cleaner, which will help clean up the stale versions faster on databases that have persistent version stores. | SQL Server Engine | Storage Management | All |
