...
| <a id="2351584">[2351584](#2351584)</a> | Fixes an issue where high `PREEMPTIVE_OS_QUERYREGISTRY` waits occur when you are using [RBAC with Azure ARC enabled SQL Server](/azure/cloud-adoption-framework/scenarios/hybrid/arc-enabled-servers/eslz-identity-and-access-management.md#role-based-access-control-rbac). To enable this fix, you must turn on trace flag 12502, which is used to disable RBAC external authorization for SQL Server On-Premises. | SQL Server Engine | Security Infrastructure | Windows |
