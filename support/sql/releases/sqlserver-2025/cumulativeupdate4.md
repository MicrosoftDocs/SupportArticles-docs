...
| :------------ | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------- | :-------------------------------------- | :------- |
| 4954486       | Fixes an issue in contained availability groups where DatabaseMail sends emails without attachments when using `sp_RunMailQuery`.                                                                                                        | SQL Server Client Tools | Database Mail                           | All      |
| 5000227       | Fixes an issue where permissions on stored procedures and other objects in `msdb` are lost during the SQL Server upgrade process, causing unexpected permission removal for users.                                                       | SQL Server Engine       | Management Services                     | Windows  |
| 5001765       | Fixes an issue where the server could enter an unintended wait state on an internal thread that does not support suspension, potentially causing a hang. The lock is now acquired using a spin-wait on such threads instead of blocking. | SQL Server on Linux     | SQLPAL                                  | Linux    |
| 5003709       | Fixes an issue where `DTSWizard.exe` fails to open when SQL Server is installed without the SQL Server Integration Services (SSIS) features.                                                                                             | Integration Services    | Integration Services                    | Windows  |
| 5008479       | Fixes an issue where indexing certain Microsoft Word 6.0 documents with Full-Text Search causes the filter daemon host process (`fdhost.exe`) to crash.                                                                                  | SQL Server Engine       | Search                                  | All      |
...
| 5028791       | Fixes an access violation error that occurs when querying `sys.dm_xe_session_targets` for instances running on Windows Server 2025.                                                                                                      | SQL Server Engine       | SQL OS                                  | Windows  |
...
| 5063278       | Fixes non-yielding scheduler errors and dumps that might occur during a high amount of lazy writer I/O.                                                                                                                                  | SQL Server Engine       | Replication                             | All      |
| 5071334       | Fixes an issue in which inflectional Full-Text Search queries fail for version 2 languages that do not implement an `IStemmer`, causing errors with predicates such as `FREETEXT`, `FREETEXTTABLE`, and `FORMSOF(INFLECTIONAL, ...)`.    | SQL Server Engine       | Search                                  | All      |
