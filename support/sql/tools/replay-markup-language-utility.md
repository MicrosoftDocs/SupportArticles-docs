...
The SQL Server support team uses several internally written tools to help them work on a typical customer support case. This article describes one particular tool suite, Replay Markup Language (RML) Utilities for SQL Server. Database developers and system administrators can use these tools.
...
- Determine whether the execution plan for a batch is changed when you capture the trace for the batch. Additionally, you can use RML Utilities for SQL Server to determine how SQL Server executes these plans.
- Determine which queries are running slow.
...
> [!IMPORTANT]
> The applications that're provided as part of the RML tool suite require that several additional controls be available.
