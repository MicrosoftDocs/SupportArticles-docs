...
|30187|Protection agent on <_TargetServerName_> was unexpectedly shut down during synchronization. DPM may have replicated data partially for <_DatasourceType_> <_DatasourceName_>.| This error occurred because the DPMRA service crashed, or the protected server was restarted. To resolve this issue, increase the Auto-rerun attempts to 2 or more.<br><br> **Auto-rerun registry entries** <br><br>  `HKLM\Software\Microsoft\Microsoft Data Protection Manager\Configuration` <br> **Value name**: `AutoRerunNumberOfAttempts` <br> **Type**: `DWORD` <br> **Settings controlled**: The number of times a failed job will be retried before giving up if it consistently fails. <br> **Implications**: Default value is **1**. Increase this value may increase the load in the system. The reruns are done at the gap of `AutoRerunDelay`. <br><br>  **Note**: Altering the `AutoRerunNumberOfAttempts` value will affect all jobs for all data sources. Set this value to a high value will increase the load on the DPM server.|
...
|32519|The backup job was made to fail because another Task exists for the same data source.| This error can occur if the frequency of backups is scheduled too close together or a performance bottleneck causes backups to run excessively long. Adjust your backup schedule accordingly.|
...
|33123|The replica of <_DatasourceType_> <_DatasourceName_> on <_ServerName_> is not consistent with the protected data source. DPM error ID = <_DpmErrorCode_>.| To resolve this issue, increase the Auto-rerun attempts to 2 or more.<br><br> **Auto-rerun registry entries** <br><br>  `HKLM\Software\Microsoft\Microsoft Data Protection Manager\Configuration` <br> **Value name**: `AutoRerunNumberOfAttempts` <br> **Type**: `DWORD` <br> **Settings controlled**: The number of times a failed job will be retried before giving up if it consistently fails. <br> **Implications**: Default value is **1**. Increase this value may increase the load on your system. The reruns are done at the gap of `AutoRerunDelay`. <br><br>  **Note**: Altering the `AutoRerunNumberOfAttempts` value will affect all jobs for all data sources. Set this value to a high value will increase the load on the DPM server.|
...
|33344|DPM skipped cloud backup for <_DatasourceType_> <_DatasourceName_> on <_ServerName_> because the latest disk backup is already backed up to cloud.| This error can occur if a new disk based recovery point failed or did not complete before the next online recovery point was scheduled to run. Adjust your backup schedules accordingly to give enough time for the disk based recovery point to be created before the online backup is scheduled.|
