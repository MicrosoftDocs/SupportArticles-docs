...
|1001|InvalidJobDefinition|One known use case for this error is when attempting to do an item-level recovery for Hyper-V VMs from a disk-to-tape (D2T) configuration, which is unsupported. To work around this issue, restore the VM as a folder or configure a disk-to-disk-to-tape backup (D2D2T) for the VM.|
