...
|30187|Protection agent on <_TargetServerName_> was unexpectedly shut down during synchronization. DPM may have replicated data partially for <_DatasourceType_> <_DatasourceName_>.| This is caused due to DPMRA crash or the protected server was restarted. Increasing the Auto retry to 2 or more will resolve this issue.<br><br> **Auto-rerun registry entries** <br> HKLM\Software\Microsoft\Microsoft Data Protection Manager\Configuration <br> **Value**: AutoRerunNumberOfAttempts <br> **Type**: Dword <br> **Settings Controlled**: The number of times a failed job will be retried before giving up if it consistently fails. <br> **Implications: Default is 1. Increasing this value may increase the load on your system.** The reruns are done at the gap of **AutoRerunDelay**. <br> **Note**: Altering the AutoRerunNumberOfAttempts will affect all jobs for all datasources. Setting this value to a high value will increase the load on the DPM server.|
...
|33344|DPM skipped cloud backup for <_DatasourceType_> <_DatasourceName_> on <_ServerName_> because the latest disk backup is already backed up to cloud.| This can occur if a new disk-based recovery point failed or didn't complete before the next online recovery point was scheduled to run. Adjust your backup schedules accordingly to give enough time for disk-based recovery point to be made before online backup is scheduled.|
...
|100058|The current recovery operation failed because the number of data transfer failures was more than <_FailureCount_> Specified files could not be processed.|Select another recovery point and try the recovery operation again.|
