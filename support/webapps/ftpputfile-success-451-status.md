...
description: Describes an issue that the WinInet FtpPutFile returns TRUE even when the operation fails with 451 status. Provides resolutions.
ms.date: 03/23/2020
ms.prod-support-area-path:
---
# WinINet FtpPutFile returns success when 451 status is returned

This article provides information about resolving an issue that the Windows Internet (WinINet) `FtpPutFile` function still returns **TRUE** even when the operation fails with the 451 status.
