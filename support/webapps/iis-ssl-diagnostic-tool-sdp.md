...
description: IIS SSL Diagnostic tool for SDP is designed to troubleshoot SSL issues on IIS and it collects information used for troubleshooting common SSL issues.
ms.date: 04/10/2020
ms.prod-support-area-path: 
---
# Information about IIS SSL Diagnostic tool for SDP

This article describes the information that may be collected from a machine when running the Internet Information Services (IIS) Secure Sockets Layer (SSL) Diagnostic on a computer that is experiencing issues while browsing to web sites running over SSL.
