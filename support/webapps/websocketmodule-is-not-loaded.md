...
description: Provides the resolution to enable the WebSocket Protocol feature to avoid the WebSocketModule is not loaded error in IIS 8.0.
ms.date: 04/15/2020
ms.prod-support-area-path: 
ms.technology: iis
ms.reviewer: piyushjo
---
# WebSocketModule is not loaded when using WebSockets with WCF 4.5 or ASP.NET in IIS 8.0

This article provides information about resolving the **WebSocketModule is not loaded** error that occurs when the WebSocket Protocol feature isn't enabled in Internet Information Services (IIS) 8.0.
...
4. To enable the WebSocket Protocol feature using a startup task in Microsoft Azure:
