...
Windows 365 Business Cloud PCs don't support [Administrator protection](/windows/security/application-security/application-control/administrator-protection/). Components in the remote connection stack require updates to work with Administrator protection's least-privilege model. Support for this environment is planned for a future release. In the meantime, configure users as standard user to maintain least privilege in this environment. If you turn it on, users might see an unexpected authentication prompt when they sign in, extra approval prompts for tasks that require elevation, and elevation failures.
