---
title: Convert MBR partition to GPT
...
1. The existing MBR system partition is reused if it meets these requirements:
   ...
   2. It is at least 100 MB (or 260 MB for 4K sector size disks) in size.
   3. It's less than or equal to 1 GB in size. This is a safety precaution to ensure it isn't a data partition.
   4. The conversion isn't being performed from the full OS. In this case, the existing MBR system partition is in use and can't be repurposed.
...
The conversion tool will obtain volume unique ID data before and after the layout conversion, organizing this information into a lookup table. It will then iterate through all the entries in `HKLM\SYSTEM\MountedDevices`, and for each entry do the following steps:
...
:::image type="content" source="media/convert-mbr-partition-to-gpt/disk-partition-type.png" alt-text="Screenshot of the properties of a disk with the Volumes tab opened, which shows the Partition style of the disk." border="false":::
...
- When you manually run the `MBR2GPT.exe` command in a Command Prompt window, there's no output from the tool.
