...
| Message | Description | Mitigation |
|---------|-------------|------------|
|ERROR_NOT_FOUND|This error occurs during the driver installation as part of the update.|This issue occurs because details such as architecture for a driver being updated is missing in the registry. Manually add the missing inf file **Arch** value in the Driver operations registry:<br/>1. Open *regedit* and navigate to `HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\ComponentBasedServicing\DriverOperations\0\2(SequenceID)`<br/>2. Review the **Identity** value to determine the value that is missing.<br/>3. Manually add the missing value referring to the information in the **Identity** value. For example, Name: Arch; Type: REG_SZ (String Value); Data: amd64.<br/>4. Proceed with install the failing update.|

## 0x800f0922

| Message | Description | Mitigation |
|---------|-------------|------------|
|CBS_E_INSTALLERS_FAILED|July Cumulative update failed to be installed on Windows Server 2016|In the CBS.log, you may find updates sometimes may rollback when License and Product key tokens failed to get update. This issue can be resolved by adding write permission for the "User" and "Network Service" user account to the _C:\\Windows\\System32\\spp\\_ folder.|

## 0x80070bc9

| Message | Description | Mitigation |
|---------|-------------|------------|
|ERROR_FAIL_REBOOT_REQUIRED|The TrustedInstaller service startup type is set to **Manual** by Group Policy (GPO), which prevented it to start to complete pending operations.|The TrustedInstaller service changes the startup type from Manual to Automatic when it encounters an update that has to process a transaction after a restart. When the value is set back to Manual before the restart, the transaction cannot be applied. This transaction will be in pending and it will block all other update installations.<br/>To fix this issue, change the TrustedInstaller policy to **Automatic** and restart the computer. If it does not work, start the computer to WinRE to revert the pending actions. For example, `dism /Image:C:\ /Cleanup-Image /RevertPendingActions`. If it does not work either, start the computer to WinRE, rename *\WinSxS\Pending.xml* and remove the PendingXMLIdentifier from COMPONENTS Hive.|

## 0x800706be

| Message | Description | Mitigation |
|---------|-------------|------------|
|Failed to install cumulative updates|Windows Server 2016 Std failed to install cumulative packages by using the .msu package. No error is returned. When we tried to install them by using dism.exe, it returned error 0x800706be|The last cumulative update failed to install and was corrupted. To resolve this issue, navigate to the registry key for the corrupted update package. Change the “current state” value to 000020 hex (32 dec) - resolved, or 000040 hex (64 dec) - staged, or 000070 hex (112 dec) - installed.|
