...
| PHASE | Reports that a new phase in the migration is starting. This can be one of the following values:<ul><li>Initializing</li><li>Scanning</li><li>Collecting</li><li>Saving</li><li>Estimating</li><li>Applying</li></ul> |
| detectedUser | <ul><li>For the ScanState tool, these values are the users USMT detected on the source computer that can be migrated.</li><li>For the LoadState tool, these values are the users USMT detected in the store that can be migrated.</li></ul> |
| includedInMigration | Defines whether the user profile/component is included for migration. Valid values are Yes or No. |
| forUser | Specifies either of the following items:<ul><li>The user state being migrated.</li><li>*This Computer*, meaning files and settings that aren't associated with a user.</li></ul> |
...
An analysis of the XML elements reference article reveals that the &lt;pattern&gt; tag needs to be modified as follows:

```xml
<pattern type="File">c:\data\* [*]</pattern>
```

When the migration is performed again with the modified tag, the diagnostic log reveals as follows:
