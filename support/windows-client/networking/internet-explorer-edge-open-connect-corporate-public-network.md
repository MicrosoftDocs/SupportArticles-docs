...
_Original product version:_ &nbsp; Windows Server 2019, Windows Server 2016, Windows Server 2012, Windows 10, Windows 8.1  
...
- The network icon on the Task Bar shows an alert symbol (for example,![Network status alert](./media/internet-explorer-edge-open-connect-corporate-public-network/4496398_en_1.png)). If you hover over the icon, you see a message such as "No connectivity" or "Limited Internet access."
...
- [NCSI active probes and the network status alert](#one) 
- [Authentication and the automatic sign-in page](#two) 
- [NCSI passive monitoring and the network status alert](three#three) 
...
FQDN.
  2. If NCSI receives a valid response from a DNS server, NCSI sends a plain HTTP GET request to
```
http://www.msftconnecttest.com/connecttest.txt
```

.
  3. If NCSI successfully downloads the text file, it makes sure that the file contains
```
...
- [Manage connections from Windows operating system components to Microsoft services: 14. Network Connectivity Status Indicator](https://docs.microsoft.com/windows/privacy/manage-connections-from-windows-operating-system-components-to-microsoft-services#14-network-connection-status-indicator) 
- [Policy CSP - Connectivity: Connectivity/DisallowNetworkConnectivityActiveTests](https://docs.microsoft.com/windows/client-management/mdm/policy-csp-connectivity#connectivity-disallownetworkconnectivityactivetests) 
...
In some cases, such as when you connect to a network that uses a proxy server to connect to the internet or when network restrictions prevent NCSI from completing its active probe process, Windows opens the MSN Portal page in the default browser. If you analyze a network trace on the computer, it shows an HTTP connection to
```
http://www.msftconnecttest.com/redirect
```

that is followed by a connection to the MSN Portal. Windows opens this page for the benefit of the passive probe process. If the page loads, NCSI concludes that the computer has internet access. As the different probes fail and then succeed, the network status alert appears and then disappears.
Note
...
- *.msftncsi.com
- *.msftconnecttest.com
For more information, see [KB 2778122, Using authenticated proxy servers together with Windows 8](https://support.microsoft.com/help/2778122/using-authenticated-proxy-servers-together-with-windows-8) 
