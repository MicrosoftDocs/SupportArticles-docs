...
This issue is fixed on Windows builds 26100.7705, 26200.7705, and later builds. To put these fixes into effect, re-deploy the kiosk configuration.
...
On kiosks for which Microsoft Edge is an allowed app, packaged apps (apps installed from an MSIX or APPX package) don't launch.

To work around this problem, find the name of the .exe file that the packaged app launches. To find the .exe file name, in the app's installation folder, open the AppxManifest.xml file and look for the `Executable` attribute of the `Application` element.
