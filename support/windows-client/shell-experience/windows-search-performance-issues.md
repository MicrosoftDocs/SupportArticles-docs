...
|Indexing in progress. Search results might not be complete during this time.|The Indexer has found new files on the system and is adding them to the index. Depending on the number of files that have recently changed, this could take a few hours|Leave the computer turned on and connected to power (if applicable) for a few hours to let indexing finish.|
|Indexing speed is reduced because of user activity.|The Indexer is adding new items to be searched, but has slowed its progress because the user is interacting with the device.|The indexing process will complete, just slowly. Wait a few hours or leave the device unattended and connected to a power source.|
...
|Indexing is starting up.|The Indexer is starting.|Wait for the Indexer to start. This should take about one minute.|
...
|Index is performing maintenance. Please wait.|The Indexer is trying to recover and optimize the index database. This could be because lots of content was added recently, or because the Indexer encountered a problem while writing out data to the hard disk.|Wait a few minutes for the Indexer to finish. This can take up to 30 minutes on a slow computer. Make sure that the system hard disk isn't generating failures. Often, Indexer writing issues precede drive failure. Make sure that the user has backed up personal data.|
|Indexing is paused by an external application.|An application on the computer requested the Indexer to stop. This commonly occurs during Game mode or during an upgrade.|Make sure that the device isn't in Game mode. Use services.msc or Task Manager to restart the Windows Search service to resume indexing until the next time that an external app requests a pause.|
