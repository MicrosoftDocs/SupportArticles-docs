...
description: This article describes the issue where you're unable to scan documents from a USB scanner, which you connect to Windows 11 after it restarts.
ms.date: 05/03/2022
...
This article describes the issue where you're unable to scan documents from a USB scanner, which you connect to Windows 11 after it restarts.
...
1. Type the `sc.exe stop StiSvc` command, and then select the **Enter** key to stop the WIA service.
   ...
   :::image type="content" source="media/cant-scan-from-USB-scanner-after-win11-restarts/stop-wia-service.png" alt-text="Stopping WIA service command.":::
...
1. Type the `sc.exe start StiSvc` command, and then select the **Enter** key to start the WIA service. When the service completes the restart, the WIA driver will be able to retrieve the document from the USB scanner.
   ...
   :::image type="content" source="media/cant-scan-from-USB-scanner-after-win11-restarts/restart-wia-service.png" alt-text="Starting WIA service command.":::
