...
## More Information
 ...
 The following table lists the most common ports that you can use to connect to particular services, and describes how to use them.
...
<sup>2</sup> If LDS runs in the context of a custom service account, Active Directory PowerShell provider uses NTLM to authenticate requests instead of Kerberos. The Active Directory PowerShell provider uses the service principal name (SPN) that's registered to the custom service account (in this case, the LDAP service SPN). However, the ADWS endpoint that the provider connect to runs in the context of the computer account. If the provider tries to use Kerberos authentication in this situation, ADWS responds by sending error code `0x29` (`KRB_AP_ERR_MODIFIED`).
...
> [!NOTE]  
> Some of these cmdlets might not use ADWS. To be certain, you can check the "Notes" section of the cmdlet reference article. "Notes" lists whether the cmdlet requires ADWS.
...
| [New-ADAuthenticationPolicy](/powershell/module/activedirectory/new-adauthenticationpolicy) | Creates an AD DS authentication policy object. |
| [New-ADAuthenticationPolicySilo](/powershell/module/activedirectory/new-adauthenticationpolicysilo) | Creates an AD DS authentication policy silo object. |
| [New-ADCentralAccessPolicy](/powershell/module/activedirectory/new-adcentralaccesspolicy) | Creates a new central access policy in Active Directory that contains a set of central access rules. |
...
| [New-ADOrganizationalUnit](/powershell/module/activedirectory/new-adorganizationalunit) | Creates an Active Directory organizational unit (OU). |
...
| [New-ADReplicationSiteLink](/powershell/module/activedirectory/new-adreplicationsitelink) | Creates a new Active Directory site link to manage replication. |
...
| [Remove-ADDomainControllerPasswordReplicationPolicy](/powershell/module/activedirectory/remove-addomaincontrollerpasswordreplicationpolicy) | Removes users, computers, and groups from the allowed or denied list of a read-only DC password replication policy. |
...
| [Set-ADAuthenticationPolicy](/powershell/module/activedirectory/set-adauthenticationpolicy) | Modifies an AD DS authentication policy object. |
| [Set-ADAuthenticationPolicySilo](/powershell/module/activedirectory/set-adauthenticationpolicysilo) | Modifies an AD DS authentication policy silo object. |
...
| [Set-ADOrganizationalUnit](/powershell/module/activedirectory/set-adorganizationalunit) | Modifies an Active Directory OU. |
