...
The primary audience of this article is administrators managing Windows Server 2016, Windows Server 2019, Windows Server 2022, Windows Server 2025 machines in their deployment. This article highlights potential issues with the Secure Time Seeding (STS) feature in Windows Server OS and provides guidance and considerations relevant to disabling this feature in Windows Server OS deployments.
...
- STS-related timekeeping issues in some Windows Server OS stock keeping units (SKUs) reported to us
- Disabling the STS feature in Windows Server OS
- Critical aspects of timekeeping, time synchronization, and dissemination
- A sample set of suggested actions for you to assess your time synchronization needs and to disable the STS feature in your Windows Server OS deployments.

## Overview of Secure Time Seeding (STS)

Secure Time Seeding (STS) is a heuristic based timekeeping mechanism in Windows OS that determines the approximate current time using time metadata from outbound Secure Sockets Layer (SSL)/Transport Layer Security (TLS) connections on a machine and uses that time information to detect and correct any large errors in the system clock on that machine.

The approximate time determined by STS depends on the time metadata available to the feature. This time metadata originates from the SSL/TLS servers that a machine connects to. Refer to [Secure Time Seeding – improving time keeping in Windows](/archive/blogs/w32time/secure-time-seeding-improving-time-keeping-in-windows) (originally published in 2016) for more details on the STS feature. 
...
STS feature is enabled by default in all editions of Windows Server 2016, Windows Server 2019, Windows Server 2022, all editions of Windows client OS starting with Windows 10, version 1511 and all subsequent Windows client OS releases, Windows IoT releases, and SKUs of all other Windows OS released with/after Windows 10, version 1511.

STS feature is disabled by default in Windows Server 2025, based on customer feedback covered in the next section.
...
- The W32time service sets the system time to a random incorrect value, with a time error varying from days, weeks to years.
- This is followed in a high likelihood of the W32time service automatically correcting the previously introduced error in system time after a noticeable but brief period, which varies from deployment to deployment.
...
We are advising all enterprise customers who have deployed Windows Server OS (Windows Server 2016, Windows Server 2019, Windows Server 2022, and all intermediate releases therein) either as standalone machines or as part of ADDS to evaluate disabling the STS feature on those machines. This recommendation to disable STS applies even if you have never faced any prior issues with the STS feature. STS is disabled by default in Windows Server 2025, but make sure to verify the STS settings even on this OS meet the requirements of your deployment.
...
- Using Services Events (not available on all editions and releases):
  ...
  - Locate the latest event among the events 257, 260 and 263 and look for **UtilizeSslTimeData** text in the event message.
...
- Through W32time debug log (available on all editions and releases):
  - (optional) Archive any existing W32time debug log to avoid confusion.
    ...
    - Archive existing W32time debug log: `move c:\w32time.log c:\w32time_archive.log`
...
## Scope for the general recommendations in this artcile
...
- Capturing the system event logs (among other event logs) on Windows systems in a deployment can be helpful in triaging various issues in the deployment.
  ...
  - Some Windows OS SKUs by default write the system event logs into this file: **%SystemRoot%\\System32\\Winevt\\Logs\\System.evtx**, and these logs can be typically viewed in the Event Viewer application.
...
- Customers must first determine whether their Windows Server OS machine is deployed in a private network or a public network and then make choices about the specific Windows timekeeping settings they are going to use in that deployment. 
...
- Review the time distribution/synchronization mechanisms you are using for timekeeping on your Windows Server OS machines. 
