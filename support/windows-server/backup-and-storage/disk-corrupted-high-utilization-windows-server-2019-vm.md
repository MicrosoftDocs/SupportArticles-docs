...
You run a disk that's attached to a Windows Server 2019 VM at 95% utilization. Then, you experience one or more of the following symptoms:

- The disk appears to be missing.
  - You can't access it from within the VM.
...
To fix this issue and prevent it from recurring, follow these steps:
...
- Output from the following Windows PowerShell cmdlets:
  ...
  - `Get-Partition`
  - `Get-Volume`
- Azure activity logs (if applicable) around the time the issue occurred.
