...
If the destination domain controller and source domain controller are running either Windows Server 2003 or Windows Server 2008, you can use this procedure to remove lingering objects with Repadmin. If either domain controller is running Windows 2000 Server, follow the instructions in article 314282 in the Microsoft Knowledge Base ([https://go.microsoft.com/fwlink/?LinkId=41410](https://go.microsoft.com/fwlink/?linkid=41410)).
 **Requirements** 
- 
...
2. 
At the command prompt, type the following command, and then press ENTER:repadmin /removelingeringobjects <ServerName> <ServerGUID> <DirectoryPartition> /advisory_mode
...
|<DirectoryPartition>|The distinguished name of the directory partition that is identified in the event message, for example:<br/>   - For the Sales domain directory partition in the contoso.com forest: DC=sales,DC=contoso,DC=com<br/> <br/>   - For the configuration directory partition in the contoso.com forest: CN=configuration,DC=contoso,DC=com<br/> <br/>   - For the schema directory partition in the contoso.com forest: CN=schema,CN=configuration,DC=contoso,DC=com<br/> |
...
to delete the identified lingering objects from the directory partition.
4. 
