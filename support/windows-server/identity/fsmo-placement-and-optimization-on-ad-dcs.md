...
|RID|Domain|- Allocates active and standby RID pools to replica domain controllers in the same domain.<br/>- Must be online for the following situations: <ul><li>when newly promoted domain controllers have to obtain a local RID pool that's required to advertise</li><li>when existing domain controllers have to update their current or standby RID pool allocation.</li></ul>|
...
- Place the PDC on your best hardware in a reliable hub site that contains replica domain controllers in the same Active Directory site and domain.
  ...
  In large or busy environments, the PDC frequently has the highest CPU usage, because it handles pass-thru authentication and password updates. If high CPU usage becomes a problem, identify the source. The source includes applications or computers that may be performing too many operations (transitively) targeting the PDC. Techniques to reduce CPU include:
