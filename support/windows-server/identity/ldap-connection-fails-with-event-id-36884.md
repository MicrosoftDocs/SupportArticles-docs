...
---
# Event ID 36884 when you try to connect to a LDAP server

This article introduces how to troubleshoot event ID 36884 issue which occurs when you try to build a Lightweight Directory Access Protocol (LDAP) connection.

## Symptoms

Consider the following scenario:

- You installed a certificate on a LDAP connection point.
...
LDAP is looking for myldapserver.contoso.com to be associated with it. However, myldapserver.contoso.com is not covered by the SAN entries. Therefore, a server name match is not made and the connection is dropped.

## Resolutions

To fix this issue, use one of the following solutions:

- Use additional SAN(s) to cover the resolved HOST names on the certificate.
...
- Configure the following registry value on the client to use the CNAME for the server name comparison.
  ...
  After the registry value is configured, the client computer uses ldap.contoso.com to make the match. It is covered by the certificate SANs so the connection is allowed.
