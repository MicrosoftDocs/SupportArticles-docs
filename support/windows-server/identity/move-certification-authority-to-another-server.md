...
3. Save the registry settings for this CA. To do this, follow these steps:
    ...
    > [!NOTE]
    > By default, Active Directory Certificate Services (AD CS) is configured with certificate revocation list (CRL) Distribution Point extensions that include the CA computer host name in the path. This means any certificates issued by the CA before migration may contain certificate validation paths that contain the old host name. These paths may no longer be valid after the migration. To avoid revocation checking errors, the new CA must be configured to publish CRLs to the old (pre-migration) path as well as the new paths. If you have to delete the old CA permanently, you can add a [second computer name to the new CA](/previous-versions/windows/it-pro/windows-server-2012-r2-and-2012/cc835082(v=ws.11)). Before you can do that, the old computer name needs to be available in Active Directory. At this point, you can add the CRL Distribution Points to the new CA.
