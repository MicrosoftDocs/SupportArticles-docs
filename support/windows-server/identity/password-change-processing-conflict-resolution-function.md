...
description: Describes a registry value that administrators can use to control when the PDC is contacted, which can help reduce communication costs between sites and reduce the load on the PDC.
ms.date: 04/20/2023
author: Deland-Han
ms.author: delhan
...
This communication to the PDC isn't done for computer accounts. When authentication fails, computers will retry the authentication with the most recent password. Likewise, the computers will try the most recent password when decrypting a Kerberos service ticket they receive.
...
The following registry value can be modified to control Password Notification and Password Conflict Resolution:
...
By default, Windows domain controllers query the PDC FSMO role owner if a user is attempting to authenticate using a password that's incorrect according to their local database. If the password sent from the client by the user is correct on the PDC, the client access is allowed, and the domain controller replicates the password change.

Administators can use the `AvoidPdcOnWan` value to control when Active Directory domain controllers attempt to use the PDC FSMO role owner to resolve password conflicts. The PDC completes the logon, and the authentication is successful for the authenticating user.

If the `AvoidPdcOnWan` value is set to `TRUE`, and the PDC FSMO role owner is located at another site, the domain controller doesn't try to authenticate a client based on password information stored on the PDC FSMO. Note, however, that this results in the user being denied access. This may cause a productivity impact, as many users won't try to authenticate with the previous password. In some scenarios, they may not know the previous password.
...
If these two changes arrive at a DC, the normal AD conflict resolution is performed. The AD version of attributes will be the same, but the timestamp of the PDC will be a bit older, and the password of the initial DC will be used.
