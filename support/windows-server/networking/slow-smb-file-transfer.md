...
> 
> [SMB signing is now required](https://aka.ms/SmbSigningRequired), by default, starting with in Windows 11 version 24H2 and Windows Server 2025. We do not recommend turning off the SMB client and server signing requirement, as they provide significant protection against spoofing, tampering, and relay attacks.
...
- Try an unbuffered I/O copy for files larger than 1 GB by using the `robocopy /J` from the Command Prompt or PowerShell.
- Enable and use [SMB compression](/windows-server/storage/file-server/smb-compression).
  ...
  - Non-compressible data, like archive (`.zip`, `.7z`, and `.rar`), video (`.mp4`, `.mkv`), and audio (`.mp3`, `.flac`) files will not see significant performance improvements with SMB compression.
