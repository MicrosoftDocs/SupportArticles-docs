...
If the node cannot ping its local IP, the local stack is not working. Note any error messages that are displayed.

If you receive a **General Failure** error, this means that there are no valid interfaces to process the request. This could be caused by a hardware issue or a stack issue.

Check whether you see a red "X" character or a yellow exclamation mark on the **Network Connection** icon in the system tray. A red X indicates that Windows is not detecting a network connection. A yellow exclamation mark indicates that the Network Connection Status Indicator (NSCI) failed a probe check.

To troubleshoot this issue, verify that the network adapter reports connectivity. Make sure that the network adapter is plugged in and that the switch port where the node is connected is not in an error state. Changing cables, switch ports, and network adapters can help you narrow down where this issue occurs. However, ultimately, the issue is outside the OS. To investigate further, contact the hardware vendors.

An issue might also occur between the network driver and Windows. This is typically because of a corruption in the stack. Use the following troubleshooting steps:
...
3. Restart the node.
4. Restore the network settings after the restart. (Note: This works only if the interface names have not changed, or the script is updated to use the new names.)
...
7. Try to start the computer in Safe Mode with Networking. If Safe Mode with Networking works, run a "clean boot" process by disabling all third-party apps and services within MSConfig, then re-enabling them one by one until the issue returns. The customer can then contact that vendor for additional support.
   1. If none of these items are successful, the issue is likely a registry corruption.
   2. If the you have a backup copy of the registry (such as a physical backup or a system restore point), you can try to restore the node to a previously working configuration. Catching the root cause of the corruption can be very difficult and extremely time consuming. Even if the corruption is found, knowing what caused it is even more challenging. Modifying the corrupted registry key manually puts the node into an unsupported state. As such, we recommend that the customer restore or reload the node in order to correct the corruption.

If the NSCI fails its probe check (yellow exclamation mark), this does not necessarily indicate a connectivity problem. If the customer is reporting this as their issue, make sure that typical communication is occurring as it should.
...
If the node cannot ping/telnet to nodes on the same subnet/network segment. Note any error messages that are displayed.

1. **Destination host unreachable** error means that the ARP requests that are sent by the node are not getting a response.
2. Gather a two-sided trace from the nodes that you are testing between. make sure that the ARP request that's sent by the source node reaches the destination node and that the destination node replies accordingly. This reply should be seen back in the source trace. If this process fails, the issue is likely a misconfiguration or other issues that affect the infrastructure.
...
3. The **Request timed out** error means that the ARP request got a response, but the ICMP Echo Request that's sent by the node is not getting an ICMP echo response. This, alone, does not indicate an issue. ICMP traffic could be blocked by the network or firewall software on the nodes. It could be beneficial to turn off the firewall profiles (Windows) or disable them through the firewall vendor's supported method for testing ICMP.
   ...
   3. If step 1 is not successful (and if firewall profiles are disabled), gather a two-sided netsh netconnection scenario trace to troubleshoot further.

### Step 5

When the node can ping its default gateway, then external connectivity (such as off-box connectivity) is possible from the source node. Further testing would still be required to understand whether a basic connectivity issue exists. If the node cannot ping or Telnet to its default gateway, this means that the ICMP replies are disabled on the router.

### Step 6

If the source node can ping, Telnet, or PsPing to other nodes on the destination subne,t then basic connectivity and routing within the infrastructure is working. This outcome points to an issue that affects the specific destination node.
...
2. If the connection to the specific port is not successful:
   ...
   2. Temporarily disable all firewall profiles. (Note: Disable only the profiles. Do not disable the service.)  
...
3. If the node cannot ping, Telnet, or PsPing to other nodes on the destination subnet, the issue could likely be infrastructure-related. Again, ICMP could be blocked within the environment. Therefore, verify connectivity by using Telnet or PsPing to connect to known-listening ports. At this point, a two-sided network trace is necessary to show where the packet loss is occurring on the network. Make sure that both traces are running before you try the connectivity test so that the issue is captured.
...
The state of a good TCP connection is usually established while having zero (0) bytes in the send and receive queues. If data is blocked in either queue, or if the state is irregular, the connection is probably at fault. If not, you are probably experiencing a network or user-level software delay.
...
Troubleshooting: If the computer is on the local subnet, verify that the name is spelled correctly and that the target computer is also running TCP/IP. If the computer is not on the local subnet, make sure that its name and IP address mapping are available in the Lmhosts file or the WINS database. If all TCP/IP elements appear to be installed properly, use the **ping** command together with the remote computer to verify that its TCP/IP software is working.

### Issue 4

Cannot connect to a specific server.

Cause: Either NetBIOS name resolution is not resolving the name or the wrong IP address is being resolved.

Troubleshooting: Use the **nbtstat -n** command on the server to determine which names the server registered on the network. The computer name of the computer that you are trying to connect to should be on the displayed list. If the name is not listed, try one of the other unique computer names that are displayed by **nbtstat**.
...
For example, a computer has a single network adapter that's configured with an IP address of 192.168.0.33 and a subnet mask of 255.255.0.0. This requires that the default gateway be of the form "192.168.*y*.*z*" because the network ID portion of the IP interface is 192.168.0.0.
