...
After you enable folder auditing on a Windows Server 2019 file server, audit events don't appear in Event Viewer. This article helps you identify and fix common configuration issues that prevent folder audit events from being recorded.

## Symptoms

You configure folder auditing to monitor a folder on a Windows Server 2019 file server for access and change operations. However, you don't see the audit events that you expect to be logged in Event Viewer. Additionally, you don't see any events or error codes that indicate an auditing issue.
...
1. If you don't configure GPOs, and you use Group Policy at the local level, follow these steps to find the correct settings:
   ...
   1. On the affected computer, select **Start**, type *gpedit.msc*, and then select **Edit group policy** from the search results.
...
1. On the affected computer, open an administrative Command Prompt window.
...
1. On the **Auditing** tab, add the user or group whose access you want to audit.
1. Select the types of access that you want to audit (for example, **Create files/write data** or **Read data/list folder**).
...
- Details of the steps that were used in testing, including the user account that was used and the actions that were performed
